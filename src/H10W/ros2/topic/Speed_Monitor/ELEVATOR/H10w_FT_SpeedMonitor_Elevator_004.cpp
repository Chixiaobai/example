#include <signal.h>
#include "H10wRosSpeedlSaftMonitor.h"
#include "h1_sdk_base.h"

static H10wRosSpeedlSaftMonitor *g_pTester = nullptr;
...
void H10w_FT_SpeedMonitor_Elevator_004() { printf("测试超速阈值为0时，100%速度移动不会触发超速停机\n"); }

// 定义测试实体，多个用例可以关联同一个实体
void h10w_ft_speedmonitor_elevator_004()
{
    ...
    auto node = std::make_shared<H10wRosSpeedlSaftMonitor>(IpPort);
    g_pTester = node.get();
    ...
    msg.right_arm = {0.005,0.0,0.0,0.0,0.0,0};
    msg.torso = {0.0,0.0,0.05,0.0,0.0,0.0};
    msg.left_arm_valid =false;
    msg.right_arm_valid =true;
    msg.torso_valid =false;
    msg.time = 0.001;
    node->speedl(msg,2000);

    // msg.right_arm = {0.0,-0.05,0.0,0.0,0.0,0.0};
    // node->speedl(msg,2000);
    ...
    // 解析测试结果
    Analysis_Test_Task_Result(num, "H10w_FT_SpeedMonitor_Elevator_004");
    if (num.at(num.size() - 1))
    {
        setCaseSucceed("H10w_FT_SpeedMonitor_Elevator_004");
    }
    else
    {
        setCaseFailed("H10w_FT_SpeedMonitor_Elevator_004");
    }
...
// 注册测试用例及测试方法
REGIST_CASE_FUNCTION(h10w_ft_speedmonitor_elevator_004)
REGIST_CASE(H10w_FT_SpeedMonitor_Elevator_004, h10w_ft_speedmonitor_elevator_004, H10w_FT_SpeedMonitor_Elevator_004);