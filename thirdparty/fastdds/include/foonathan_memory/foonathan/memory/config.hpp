...
/// Set to `2` to enable automatic lifetime management of the per-thread stack
/// through nifty counter. Then all memory will be freed upon program
/// termination automatically. Set to `1` to disable automatic lifetime
/// managment of the per-thread stack, requires managing it through the \ref
/// foonathan::memory::temporary_stack_initializer. Set to `0` to disable the
/// per-thread stack completely. \ref get_temporary_stack() will abort the
/// program upon call. \ingroup allocator
