...
namespace foonathan
{
    namespace memory
    {
        ...
        public:
            ...
            /// \effects Creates it by giving it the maximum node size it should
            /// be able to allocate, the size of the initial memory block and
            /// other constructor arguments for the
            /// \concept{concept_blockallocator,BlockAllocator}. The \c
            /// BucketDistribution controls how many free lists are created, but
            /// unlike in \ref memory_pool all free lists are initially empty
            /// and the first memory block queued. \requires \c max_node_size
            /// must be a valid \concept{concept_node,node} size and \c
