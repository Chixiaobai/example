    ...
    namespace memory
    {
        ...
        public:
            ...
            template <
                ...
                // use this to prevent this constructor being chosen instead of
                // move for types inheriting from it
                FOONATHAN_REQUIRES((
            ...
            auto lock() const noexcept
              ...
              return detail::lock_allocator(get_allocator(),
                                            static_cast<actual_mutex &>(*this));
            }
            /// @}.
...
/// \ingroup storage
        ...
        private:
          ...
          class basic_allocator
            ...
            using composable =
                is_composable_allocator<typename traits::allocator_type>;
            using storage = detail::reference_storage_impl<
                typename allocator_traits<RawAllocator>::allocator_type,
                decltype(detail::reference_type(
                    typename allocator_traits<RawAllocator>::is_stateful{},
          ...
          public:
            // non stateful
            basic_allocator(const RawAllocator &alloc) noexcept
                : storage(alloc) {}
          ...
          private:
            ...
            void *try_allocate_impl(std::size_t count, std::size_t size,
              ...
              auto &&alloc = get();
              if (count == 1u)
                return detail::try_allocate_node(composable{}, alloc, size,
                                                 alignment);
              else
                return detail::try_allocate_array(composable{}, alloc, count,
                                                  size, alignment);
            }

            bool try_deallocate_impl(void *ptr, std::size_t count,
                                     std::size_t size,
                                     std::size_t alignment) noexcept override {
              auto &&alloc = get();
              if (count == 1u)
                return detail::try_deallocate_node(composable{}, alloc, ptr,
                                                   size, alignment);
              else
                return detail::try_deallocate_array(composable{}, alloc, ptr,
