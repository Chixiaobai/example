...
/**
 * Class TopicDataType used to provide the DomainRTPSParticipant with the
 * methods to serialize, deserialize and get the key of a specific data type.
 * The user should created a class that inherits from this one, where Serialize
 * and deserialize methods MUST be implemented.
...
public:
  ...
  RTPS_DllAPI TopicDataType()
      : m_typeSize(0), m_isGetKeyDefined(false), auto_fill_type_object_(true),
   ...
   * abstract. It is VERY IMPORTANT that the user sets the SerializedPayload
   * length correctly.
   *
  ...
  serialize(void *data, fastrtps::rtps::SerializedPayload_t *payload) = 0;
   ...
   */
  RTPS_DllAPI virtual bool
  deserialize(fastrtps::rtps::SerializedPayload_t *payload, void *data) = 0;
   ...
   * @return function
   */
  RTPS_DllAPI virtual std::function<uint32_t()>
  getSerializedSizeProvider(void *data) = 0;
   ...
   * @return True if correct.
   */
  RTPS_DllAPI virtual bool getKey(void *data,
   ...
   */
  RTPS_DllAPI inline void setName(const char *nam) {
  ...
  auto_fill_type_information(bool auto_fill_type_information) {
   ...
   */
  RTPS_DllAPI inline void type_identifier(const TypeIdV1 &id) {
   ...
   */
  RTPS_DllAPI inline void type_identifier(const std::shared_ptr<TypeIdV1> id) {
   ...
   */
  RTPS_DllAPI inline void type_object(const TypeObjectV1 &object) {
   ...
   */
  RTPS_DllAPI inline void type_object(std::shared_ptr<TypeObjectV1> object) {
     ...
     */
    RTPS_DllAPI inline const std::shared_ptr<xtypes::TypeInformation>
    type_information() const {
     ...
     */
    RTPS_DllAPI inline void
    type_information(const xtypes::TypeInformation &info) {
     ...
     */
    RTPS_DllAPI inline void
    type_information(std::shared_ptr<xtypes::TypeInformation> info) {
     ...
     */
    RTPS_DllAPI virtual inline bool is_bounded() const { return false; }

    /**
     * Checks if the type is plain.
     */
    RTPS_DllAPI virtual inline bool is_plain() const { return false; }
     ...
     */
    RTPS_DllAPI virtual inline bool construct_sample(void *memory) const {
