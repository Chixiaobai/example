...
private:
  RTPS_DllAPI
  ContentFilteredTopic(const std::string &name, Topic *related_topic,
...
public:
   ...
   *
   * This operation returns filter expression associated with this
   * ContentFilteredTopic. It will return the @c filter_expression specified on
   * the last successful call to @c set_expression or, if that method is never
   * called, the expression specified when the ContentFilteredTopic was created.
   *
   ...
   *
   * This operation returns expression parameters associated with this
   * ContentFilteredTopic. These will be the @c expression_parameters specified
   * on the last successful call to @c set_expression or
   * @c set_expression_parameters.
   * If those methods have never been called, the expression parameters
   * specified when the ContentFilteredTopic was created will be returned.
   *
   * @param [out] expression_parameters  The expression parameters currently
   ...
   *
   * This operation changes expression parameters associated with this
   * ContentFilteredTopic.
   *
   * @param [in] expression_parameters  The expression parameters to set.
   *
   * @return RETCODE_OK             if the expression parameters where correctly
   * updated.
   * @return RETCODE_BAD_PARAMETER  if the expression parameters do not match
   * with the current @c filter_expression.
   */
   ...
   *
   * This operation changes the filter expression and the expression parameters
   * associated with this ContentFilteredTopic.
   *
   * @param [in] filter_expression      The filter expression to set.
   * @param [in] expression_parameters  The expression parameters to set.
   *
   * @return RETCODE_OK             if the expression and parameters where
   * correctly updated.
   * @return RETCODE_BAD_PARAMETER  if @c filter_expression is not valid for
   * this ContentFilteredTopic.
   * @return RETCODE_BAD_PARAMETER  if the expression parameters do not match
   * with the @c filter_expression.
   */
  RTPS_DllAPI ReturnCode_t
  set_filter_expression(const std::string &filter_expression,
                        const std::vector<std::string> &expression_parameters);
