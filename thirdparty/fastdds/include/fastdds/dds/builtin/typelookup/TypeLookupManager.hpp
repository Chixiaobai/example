...
/**
 * Class TypeLookupManager that implements the TypeLookup Service described in
 * the DDS-XTYPES 1.2 specification.
 * @ingroup XTYPES
 */
...
public:
   ...
   */
  bool
  assign_remote_endpoints(const fastrtps::rtps::ParticipantProxyData &pdata);
...
private:
  ...
  bool send_request(TypeLookup_Request &req) const;

  //! Aux method to send replies
  bool send_reply(TypeLookup_Reply &rep) const;

  //! Aux method to received requests
  bool recv_request(fastrtps::rtps::CacheChange_t &change,
                    TypeLookup_Request &req) const;

  //! Aux method to received replies
  bool recv_reply(fastrtps::rtps::CacheChange_t &change,
  ...
  void request_cache_change_acked(fastrtps::rtps::CacheChange_t *change);

  void reply_cache_change_acked(fastrtps::rtps::CacheChange_t *change);
};
