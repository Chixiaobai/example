...
public:
    ...
    create_datareader(TopicDescription *topic, const DataReaderQos &reader_qos,
     ...
     */
    RTPS_DllAPI DataReader *
    create_datareader_with_profile(TopicDescription *topic,
     ...
     */
    RTPS_DllAPI DataReader *
    lookup_datareader(const std::string &topic_name) const;
    ...
    get_datareaders(std::vector<DataReader *> &readers) const;

    /**
     * @brief This operation allows the application to access the DataReader
     * objects that contain samples with the specified sample_states,
     * view_states, and instance_states.
     *
     * @param[out] readers Vector of DataReader where the list of existing
     * readers is returned
     ...
     */
    RTPS_DllAPI ReturnCode_t
    set_default_datareader_qos(const DataReaderQos &qos);
     ...
     */
    RTPS_DllAPI ReturnCode_t
    get_default_datareader_qos(DataReaderQos &qos) const;
     ...
     * @return RETCODE_OK if successful, an error code otherwise
     */
    RTPS_DllAPI static ReturnCode_t
    copy_from_topic_qos(DataReaderQos &reader_qos, const TopicQos &topic_qos);
