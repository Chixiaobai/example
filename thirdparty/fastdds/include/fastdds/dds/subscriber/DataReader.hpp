...
public:
     ...
     */
    RTPS_DllAPI bool
    wait_for_unread_message(const fastrtps::Duration_t &timeout);

    /**
     * NOT YET IMPLEMENTED
     *
     * @brief Method to block the current thread until an unread message is
     * available.
     *
     * @param[in] max_wait Max blocking time for this operation.
     * @return RETCODE_OK if there is new unread message,
     * ReturnCode_t::RETCODE_TIMEOUT if timeout
     */
    RTPS_DllAPI ReturnCode_t
    wait_for_historical_data(const fastrtps::Duration_t &max_wait) const;
     ...
     */
    RTPS_DllAPI ReturnCode_t
    read(LoanableCollection &data_values, SampleInfoSeq &sample_infos,
     ...
     * NOT YET IMPLEMENTED
     *
     * This operation accesses via ‘read’ the samples that match the criteria
     * specified in the ReadCondition. This operation is especially useful in
     * combination with QueryCondition to filter data samples based on the
     * content.
     *
     * The specified ReadCondition must be attached to the DataReader; otherwise
     ...
     * and passing as @c sample_states, @c view_states and @c instance_states
     * the value of the corresponding attributes in @c a_condition. Using this
     * operation the application can avoid repeating the same parameters
     * specified when creating the ReadCondition.
     *
     * The samples are accessed with the same semantics as the read operation.
     * If the DataReader has no samples that meet the constraints, the return
     * value will be RETCODE_NO_DATA.
     *
     * @param[in,out] data_values     A LoanableCollection object where the
     * received data samples will be returned.
     * @param[in,out] sample_infos    A SampleInfoSeq object where the received
     * sample info will be returned.
     * @param[in]     max_samples     The maximum number of samples to be
     * returned.
     * @param[in]     a_condition     A ReadCondition that returned @c
     * sample_states must pass
     *
     * @return Any of the standard return codes.
     */
    RTPS_DllAPI ReturnCode_t read_w_condition(
        LoanableCollection &data_values, SampleInfoSeq &sample_infos,
        int32_t max_samples = LENGTH_UNLIMITED,
     ...
     */
    RTPS_DllAPI ReturnCode_t
    read_instance(LoanableCollection &data_values, SampleInfoSeq &sample_infos,
                  int32_t max_samples = LENGTH_UNLIMITED,
     ...
     * @ref read_next_instance except that all samples returned satisfy the
     * specified condition. In other words, on success all returned samples
     * belong to the same instance, and the instance is the instance with
     * ‘smallest’ @c instance_handle among the ones that verify (a) @c
     * instance_handle >= @c previous_handle and (b) have samples for which the
     * specified ReadCondition evaluates to TRUE.
     *
     * Similar to the operation @ref read_next_instance it is possible to call
     * @ref read_next_instance_w_condition with a @c previous_handle that does
     * not correspond to an instance currently managed by the DataReader.
     *
     * The behavior of the @ref read_next_instance_w_condition operation follows
     * the same rules than the read operation regarding the pre-conditions and
     * post-conditions for the @c data_values and @c sample_infos collections.
     * Similar to read, the @ref read_next_instance_w_condition operation may
     * ‘loan’ elements to the output collections which must then be returned by
     * means of @ref return_loan.
     *
     * If the DataReader has no samples that meet the constraints, the return
     * value will be RETCODE_NO_DATA.
     *
     ...
     * sample info will be returned.
     * @param[in]     max_samples     The maximum number of samples to be
     * returned. If the special value
     *                                @ref LENGTH_UNLIMITED is provided, as many
     * samples will be returned as are available, up to the limits described in
     * the documentation for @ref read().
     * @param[in]     previous_handle The 'next smallest' instance with a value
     * greater than this value that has available samples will be returned.
     * @param[in]     a_condition     A ReadCondition that returned @c
     * sample_states must pass
     *
     * @return Any of the standard return codes.
     ...
     */
    RTPS_DllAPI ReturnCode_t
    take(LoanableCollection &data_values, SampleInfoSeq &sample_infos,
         int32_t max_samples = LENGTH_UNLIMITED,
         SampleStateMask sample_states = ANY_SAMPLE_STATE,
         ViewStateMask view_states = ANY_VIEW_STATE,
         InstanceStateMask instance_states = ANY_INSTANCE_STATE);
     ...
     * NOT YET IMPLEMENTED
     *
     * This operation is analogous to @ref read_w_condition except it accesses
     * samples via the ‘take’ operation.
     *
     * The specified ReadCondition must be attached to the DataReader; otherwise
     * the operation will fail and return RETCODE_PRECONDITION_NOT_MET.
     *
     * The samples are accessed with the same semantics as the @ref take
     * operation.
     *
     ...
     * returned. If the special value
     *                                @ref LENGTH_UNLIMITED is provided, as many
     * samples will be returned as are.
     * @param[in]     a_condition     A ReadCondition that returned @c
     * sample_states must pass
     *
     * @return Any of the standard return codes.
     ...
     *
     * This operation has the same behavior as @ref read_instance, except that
     * the samples are 'taken' from the DataReader such that they are no longer
     * accessible via subsequent 'read' or 'take' operations.
     *
     * The behavior of this operation follows the same rules as the @ref read
     * operation regarding the pre-conditions and post-conditions for the @c
     * data_values and @c sample_infos. Similar to @ref read, this operation may
     * 'loan' elements to the output collections, which must then be returned by
     * means of @ref return_loan.
     *
     * If the DataReader has no samples that meet the constraints, the
     * operations fails with RETCODE_NO_DATA.
     *
     * @param[in,out] data_values     A LoanableCollection object where the
     * received data samples will be returned.
     * @param[in,out] sample_infos    A SampleInfoSeq object where the received
     ...
     * samples will be returned as are available, up to the limits described in
     * the documentation for @ref read().
     * @param[in]     a_handle        The specified instance to return samples
     * for. The method will fail with RETCODE_BAD_PARAMETER if the handle does
     * not correspond to an existing data-object known to the DataReader.
     * @param[in]     sample_states   Only data samples with @c sample_state
     * matching one of these will be returned.
     * @param[in]     view_states     Only data samples with @c view_state
     * matching one of these will be returned.
     * @param[in]     instance_states Only data samples with @c instance_state
     * matching one of these will be returned.
     *
     * @return Any of the standard return codes.
     */
    RTPS_DllAPI ReturnCode_t
    take_instance(LoanableCollection &data_values, SampleInfoSeq &sample_infos,
                  int32_t max_samples = LENGTH_UNLIMITED,
                  const InstanceHandle_t &a_handle = HANDLE_NIL,
                  SampleStateMask sample_states = ANY_SAMPLE_STATE,
                  ViewStateMask view_states = ANY_VIEW_STATE,
     ...
     * and 'removes' them from the DataReader.
     *
     * This operation has the same behavior as @ref read_next_instance, except
     * that the samples are 'taken' from the DataReader such that they are no
     * longer accessible via subsequent 'read' or 'take' operations.
     *
     * Similar to the operation @ref read_next_instance, it is possible to call
     * this operation with a
     * @c previous_handle that does not correspond to an instance currently
     ...
     * belong to the same instance, and the instance is the instance with
     * ‘smallest’
     * @c instance_handle among the ones that verify (a) @c instance_handle >=
     * @c previous_handle and (b) have samples for which the specified
    ...
    /**
     * This operation indicates to the DataReader that the application is done
     * accessing the collection of
     * @c data_values and @c sample_infos obtained by some earlier invocation of
     * @ref read or @ref take on the DataReader.
     *
     * The @c data_values and @c sample_infos must belong to a single related
     * ‘pair’; that is, they should correspond to a pair returned from a single
     * call to read or take. The @c data_values and @c sample_infos must also
     * have been obtained from the same DataReader to which they are returned.
     * If either of these conditions is not met, the operation will fail and
     * return RETCODE_PRECONDITION_NOT_MET.
     *
     * This operation allows implementations of the @ref read and @ref take
     * operations to "loan" buffers from the DataReader to the application and
     * in this manner provide "zero-copy" access to the data. During the loan,
     * the DataReader will guarantee that the data and sample-information are
     * not modified.
     *
     * It is not necessary for an application to return the loans immediately
     * after the read or take calls. However, as these buffers correspond to
     * internal resources inside the DataReader, the application should not
     * retain them indefinitely.
     *
     * The use of the @ref return_loan operation is only necessary if the read
     * or take calls "loaned" buffers to the application. This only occurs if
     * the @c data_values and @c sample_infos collections had <tt> max_len == 0
     * </tt> at the time read or take was called. The application may also
     * examine the @c owns property of the collection to determine if there is
     * an outstanding loan. However, calling @ref return_loan on a collection
     * that does not have a loan is safe and has no side effects.
     *
     * If the collections had a loan, upon return from return_loan the
     * collections will have <tt> max_len == 0 </tt>.
     *
     * @param [in,out] data_values   A LoanableCollection object where the
     * received data samples were obtained from an earlier invocation of read or
     * take on this DataReader.
     * @param [in,out] sample_infos  A SampleInfoSeq object where the received
     * sample infos were obtained from an earlier invocation of read or take on
     * this DataReader.
     *
     * @return Any of the standard return codes.
     */
    RTPS_DllAPI ReturnCode_t return_loan(LoanableCollection &data_values,
     ...
     * to an @c instance_handle. The operation will only fill the fields that
     * form the key inside the key_holder instance.
     *
     * This operation may return BAD_PARAMETER if the InstanceHandle_t a_handle
     ...
     * @return Any of the standard return codes.
     */
    RTPS_DllAPI ReturnCode_t get_key_value(void *key_holder,
     ...
     */
    RTPS_DllAPI ReturnCode_t
    get_requested_deadline_missed_status(RequestedDeadlineMissedStatus &status);
     ...
     */
    RTPS_DllAPI ReturnCode_t
    get_liveliness_changed_status(LivelinessChangedStatus &status) const;
     ...
     */
    RTPS_DllAPI ReturnCode_t
    get_sample_lost_status(SampleLostStatus &status) const;

    /**
     * @brief Get the SAMPLE_REJECTED communication status
     *
     * @param[out] status SampleRejectedStatus object where the status is
     * returned.
     *
     * @return RETCODE_OK
     */
    RTPS_DllAPI ReturnCode_t
    get_sample_rejected_status(SampleRejectedStatus &status) const;
     ...
     */
    RTPS_DllAPI ReturnCode_t
    get_subscription_matched_status(SubscriptionMatchedStatus &status) const;
    ...
    /**
     * @brief Fills the given vector with the InstanceHandle_t of matched
     * DataReaders
     *
     * @param[out] publication_handles Vector where the InstanceHandle_t are
     * returned
     * @return RETCODE_OK
     */
    ...
    /**
     * @brief This operation creates a ReadCondition. The returned ReadCondition
    ...
    create_readcondition(const std::vector<SampleStateKind> &sample_states,
                         const std::vector<ViewStateKind> &view_states,
                         const std::vector<InstanceStateKind> &instance_states);

    /**
     * @brief This operation creates a QueryCondition. The returned
     * QueryCondition will be attached and belong to the DataReader.
     *
     * @param sample_states Vector of SampleStateKind
     * @param view_states Vector of ViewStateKind
     * @param instance_states Vector of InstanceStateKind
     * @param query_expression string containing query
     * @param query_parameters Vector of strings containing parameters of query
     * expression
     * @return QueryCondition pointer
     */
    RTPS_DllAPI QueryCondition *
    create_querycondition(const std::vector<SampleStateKind> &sample_states,
                          const std::vector<ViewStateKind> &view_states,
                          const std::vector<InstanceStateKind> &instance_states,
     ...
     */
    RTPS_DllAPI ReturnCode_t
    delete_readcondition(const ReadCondition *a_condition);
     ...
     */
    RTPS_DllAPI bool is_sample_valid(const void *data,
     ...
     */
    RTPS_DllAPI ReturnCode_t
    get_listening_locators(rtps::LocatorList &locators) const;
