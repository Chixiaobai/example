...
public:
     ...
     * @brief Constructor without parameters
     */
    RTPS_DllAPI EntityFactoryQosPolicy() : autoenable_created_entities(true) {}
     ...
     */
    RTPS_DllAPI EntityFactoryQosPolicy(bool autoenable)
...
typedef enum DurabilityQosPolicyKind : fastrtps::rtps::octet {
   ...
   * of any DataReader that is not known by the DataWriter at the time the
   * instance is written. In other words the Service will only attempt to
   * provide the data to existing subscribers
   */
  VOLATILE_DURABILITY_QOS,
  /**
   * For TRANSIENT_LOCAL, the service is only required to keep the data in the
   * memory of the DataWriter that wrote the data and the data is not required
   * to survive the DataWriter.
   */
  TRANSIENT_LOCAL_DURABILITY_QOS,
  /**
   * For TRANSIENT, the service is only required to keep the data in memory and
   * not in permanent storage; but the data is not tied to the lifecycle of the
   * DataWriter and will, in general, survive it.
   */
...
public:
  ...
  RTPS_DllAPI DurabilityQosPolicy()
      : Parameter_t(PID_DURABILITY, PARAMETER_KIND_LENGTH), QosPolicy(true),
  ...
  inline fastrtps::rtps::DurabilityKind_t durabilityKind() const {
    ...
    case VOLATILE_DURABILITY_QOS:
      return fastrtps::rtps::VOLATILE;
    case TRANSIENT_LOCAL_DURABILITY_QOS:
      return fastrtps::rtps::TRANSIENT_LOCAL;
    case TRANSIENT_DURABILITY_QOS:
      return fastrtps::rtps::TRANSIENT;
    case PERSISTENT_DURABILITY_QOS:
      return fastrtps::rtps::PERSISTENT;
    }
    }
     ...
     */
    inline void
    durabilityKind(const fastrtps::rtps::DurabilityKind_t new_kind) {
      switch (new_kind) {
      default:
      case fastrtps::rtps::VOLATILE:
        kind = VOLATILE_DURABILITY_QOS;
        break;
      case fastrtps::rtps::TRANSIENT_LOCAL:
        kind = TRANSIENT_LOCAL_DURABILITY_QOS;
        break;
      case fastrtps::rtps::TRANSIENT:
        kind = TRANSIENT_DURABILITY_QOS;
        break;
      case fastrtps::rtps::PERSISTENT:
...
public:
  ...
  RTPS_DllAPI DeadlineQosPolicy()
      : Parameter_t(PID_DEADLINE, PARAMETER_TIME_LENGTH), QosPolicy(true),
  ...
  bool operator==(const DeadlineQosPolicy &b) const {
    return (this->period == b.period) && Parameter_t::operator==(b) &&
           QosPolicy::operator==(b);
    }
...
public:
  ...
  RTPS_DllAPI LatencyBudgetQosPolicy()
      : Parameter_t(PID_LATENCY_BUDGET, PARAMETER_TIME_LENGTH), QosPolicy(true),
        duration(0, 0) {}
  ...
  bool operator==(const LatencyBudgetQosPolicy &b) const {
    return (this->duration == b.duration) && Parameter_t::operator==(b) &&
           QosPolicy::operator==(b);
    }
...
public:
  //! Maximum acceptable delay from the time data is written until it is
  //! received. <br> By default, c_TimeZero.
...
typedef enum LivelinessQosPolicyKind : fastrtps::rtps::octet {
  /**
   * The infrastructure will automatically signal liveliness for the DataWriters
   * at least as often as required by the lease_duration.
   */
  AUTOMATIC_LIVELINESS_QOS,
  /**
   * The Service will assume that as long as at least one Entity within the
   * DomainParticipant has asserted its liveliness the other Entities in that
   * same DomainParticipant are also alive.
   */
  MANUAL_BY_PARTICIPANT_LIVELINESS_QOS,
  /**
   * The Service will only assume liveliness of the DataWriter if the
...
public:
  ...
  RTPS_DllAPI LivelinessQosPolicy()
      : Parameter_t(PID_LIVELINESS,
                    PARAMETER_KIND_LENGTH + PARAMETER_TIME_LENGTH),
        QosPolicy(true), kind(AUTOMATIC_LIVELINESS_QOS),
        lease_duration(TIME_T_INFINITE_SECONDS, TIME_T_INFINITE_NANOSECONDS),
        announcement_period(TIME_T_INFINITE_SECONDS,
  ...
  bool operator==(const LivelinessQosPolicy &b) const {
    return (this->kind == b.kind) &&
           ...
           Parameter_t::operator==(b) && QosPolicy::operator==(b);
    }
...
public:
     ...
     *  On a DataWriter it represents the period it commits to signal its
     * liveliness. On a DataReader it represents the period without assertion
     * after which a DataWriter is considered inactive. By default,
     * c_TimeInfinite.
     */
     ...
     *
     * @warning When not infinite, must be < lease_duration, and it is advisable
     * to be less than 0.7*lease_duration.
     */
...
typedef enum ReliabilityQosPolicyKind : fastrtps::rtps::octet {
   ...
   * Presumably new values for the samples are generated often enough that it is
   * not necessary to re-send or acknowledge any samples
   */
   ...
   * communicated via the DataWriter) the middleware guarantees that all samples
   * in the DataWriter history will eventually be delivered to all the
   * DataReader objects. Outside steady state the HistoryQosPolicy and
   * ResourceLimitsQosPolicy will determine how samples become part of the
   * history and whether samples can be discarded from it.
   */
...
public:
  ...
  RTPS_DllAPI ReliabilityQosPolicy()
      : Parameter_t(PID_RELIABILITY,
                    PARAMETER_KIND_LENGTH + PARAMETER_TIME_LENGTH),
        QosPolicy(true) // indicate send always
        ,
        kind(BEST_EFFORT_RELIABILITY_QOS), max_blocking_time{0, 100000000}
...
enum OwnershipQosPolicyKind : fastrtps::rtps::octet {
  /**
   * Indicates shared ownership for each instance. Multiple writers are allowed
   * to update the same instance and all the updates are made available to the
   * readers. In other words there is no concept of an “owner” for the
   * instances.
   */
  SHARED_OWNERSHIP_QOS,
  /**
   * Indicates each instance can only be owned by one DataWriter, but the owner
   * of an instance can change dynamically. The selection of the owner is
   * controlled by the setting of the OwnershipStrengthQosPolicy. The owner is
   * always set to be the highest-strength DataWriter object among the ones
   * currently “active” (as determined by the LivelinessQosPolicy).
   */
...
public:
  ...
  RTPS_DllAPI OwnershipQosPolicy()
      : Parameter_t(PID_OWNERSHIP, PARAMETER_KIND_LENGTH), QosPolicy(true),
  ...
  bool operator==(const OwnershipQosPolicy &b) const {
    return (this->kind == b.kind) && Parameter_t::operator==(b) &&
           QosPolicy::operator==(b);
    }
...
enum DestinationOrderQosPolicyKind : fastrtps::rtps::octet {
   ...
   * Consequently, it is possible for each subscriber to end up with a different
...
public:
  ...
  RTPS_DllAPI DestinationOrderQosPolicy()
      : Parameter_t(PID_DESTINATION_ORDER, PARAMETER_KIND_LENGTH),
        QosPolicy(true), kind(BY_RECEPTION_TIMESTAMP_DESTINATIONORDER_QOS) {}
...
class GenericDataQosPolicy
  ...
  using ResourceLimitedOctetVector =
      fastrtps::ResourceLimitedVector<fastrtps::rtps::octet>;

public:
  RTPS_DllAPI GenericDataQosPolicy(ParameterId_t pid)
      : Parameter_t(pid, 0), QosPolicy(false), ResourceLimitedOctetVector() {}

  RTPS_DllAPI GenericDataQosPolicy(ParameterId_t pid, uint16_t in_length)
      : Parameter_t(pid, in_length), QosPolicy(false),
   ...
   */
  RTPS_DllAPI GenericDataQosPolicy(const GenericDataQosPolicy &data)
      : Parameter_t(data.Pid, data.length), QosPolicy(false),
        ResourceLimitedOctetVector(data) {}

  /**
   * Construct from underlying collection type.
   *
   * Useful to easy integration on old APIs where a traditional container was
   * used. The resulting GenericDataQosPolicy will always be unlimited in size
   *
   ...
   *
   * Useful to easy integration on old APIs where a traditional container was
   * used. The resulting GenericDataQosPolicy will keep the current size limit.
   * If the input data is larger than the current limit size, the elements
   * exceeding that maximum will be silently discarded.
   *
  ...
  GenericDataQosPolicy &operator=(const collection_type &b) {
            ...
            size_t size)
    {
        if (size > 0)
        {
          configuration_ = fastrtps::ResourceLimitedContainerConfig::
              fixed_size_configuration(size);
     ...
     * @return raw data as vector of octets.
     * */
    RTPS_DllAPI inline collection_type &data_vec() { return collection_; }
     ...
     * */
    RTPS_DllAPI inline void data_vec(const collection_type &vec) {
      if (collection_ != vec) {
     ...
     * @param vec raw data to set.
     * */
    RTPS_DllAPI inline void setValue(const collection_type &vec) {
      data_vec(vec);
    }
...
#define TEMPLATE_DATA_QOS_POLICY(TClassName, TPid)                             \
  ...
  public:                                                                      \
    RTPS_DllAPI TClassName() : GenericDataQosPolicy(TPid) {}                   \
                                                                               \
    RTPS_DllAPI TClassName(uint16_t in_length)                                 \
     ...
     * @param data data to copy in the newly created object                    \
     */                                                                        \
    RTPS_DllAPI TClassName(const TClassName &data) = default;                  \
                                                                               \
    /**                                                                        \
     * Construct from underlying collection type.                              \
     *                                                                         \
     * Useful to easy integration on old APIs where a traditional container    \
     * was used. The resulting TClassName will always be unlimited in size     \
     *                                                                         \
     * @param data data to copy in the newly created object                    \
     */                                                                        \
    RTPS_DllAPI TClassName(const collection_type &data)                        \
     ...
     * @return reference to the current object.                                \
     */                                                                        \
    TClassName &operator=(const TClassName &b) = default;                      \
  };
// *INDENT-ON*
...
public:
  ...
  RTPS_DllAPI TimeBasedFilterQosPolicy()
      : Parameter_t(PID_TIME_BASED_FILTER, PARAMETER_TIME_LENGTH),
        QosPolicy(false), minimum_separation(0, 0) {}
...
public:
  //! Minimum interval between samples. By default, c_TimeZero (the DataReader
  //! is interested in all values)
...
enum PresentationQosPolicyAccessScopeKind : fastrtps::rtps::octet {
  /**
   * Scope spans only a single instance. Indicates that changes to one instance
   * need not be coherent nor ordered with respect to changes to any other
   * instance. In other words, order and coherent changes apply to each instance
   * separately.
   */
  INSTANCE_PRESENTATION_QOS,
  /**
   * Scope spans to all instances within the same DataWriter (or DataReader),
   * but not across instances in different DataWriter (or DataReader).
   */
...
public:
  ...
  RTPS_DllAPI PresentationQosPolicy()
      : Parameter_t(PID_PRESENTATION, PARAMETER_PRESENTATION_LENGTH),
        QosPolicy(false), access_scope(INSTANCE_PRESENTATION_QOS),
...
public:
  ...
  RTPS_DllAPI PartitionQosPolicy()
      : Parameter_t(PID_PARTITION, 0), QosPolicy(false), max_size_(0),
        Npartitions_(0) {}
  ...
  RTPS_DllAPI PartitionQosPolicy(uint16_t in_length)
      : Parameter_t(PID_PARTITION, in_length), QosPolicy(false),
        max_size_(in_length), partitions_(in_length), Npartitions_(0) {}
  ...
  RTPS_DllAPI PartitionQosPolicy(const PartitionQosPolicy &b)
      : Parameter_t(b), QosPolicy(b), max_size_(b.max_size_),
        partitions_(b.max_size_ != 0 ? b.partitions_.max_size
    ...
    RTPS_DllAPI inline void push_back(const char *name) {
      ...
      if (max_size_ != 0 &&
          (partitions_.max_size < partitions_.length + size + alignment + 4)) {
        return;
      }
...
enum HistoryQosPolicyKind : fastrtps::rtps::octet {
   ...
   * managed by the DataWriter. On the subscribing side, the DataReader will
   * only attempt to keep the most recent “depth” samples received for each
   * instance (identified by its key) until the application “takes” them via the
   ...
   * its key) managed by the DataWriter until they can be delivered to all
   * subscribers. On the subscribing side, the Service will attempt to keep all
   * samples of each instance of data (identified by its key) managed by the
   * DataReader. These samples are kept until the application “takes” them from
   * the Service via the take operation.
   */
...
public:
  ...
  RTPS_DllAPI HistoryQosPolicy()
      : Parameter_t(PID_HISTORY, PARAMETER_KIND_LENGTH + 4), QosPolicy(true),
  ...
  bool operator==(const HistoryQosPolicy &b) const {
    return (this->kind == b.kind) && (this->depth == b.depth) &&
           Parameter_t::operator==(b) && QosPolicy::operator==(b);
    }
...
public:
  /**
   * @brief Specifies the maximum number of data-samples the DataWriter (or
   * DataReader) can manage across all the instances associated with it.
   * Represents the maximum samples the middleware can store for any one
   * DataWriter (or DataReader). <br> By default, 5000.
   *
   * @warning It is inconsistent for this value to be less than
   * max_samples_per_instance.
   */
  int32_t max_samples;
  /**
   * @brief Represents the maximum number of instances DataWriter (or
   * DataReader) can manage. <br> By default, 10.
   */
  int32_t max_instances;
  /**
   * @brief Represents the maximum number of samples of any one instance a
   * DataWriter(or DataReader) can manage. <br> By default, 400.
   *
  ...
  /**
   * @brief Represents the extra number of samples available once the
   * max_samples have been reached in the history. This makes it possible, for
   * example, to loan samples even with a full history. By default, 1.
   */
  ...
  RTPS_DllAPI ResourceLimitsQosPolicy()
      : Parameter_t(PID_RESOURCE_LIMITS, 4 + 4 + 4), QosPolicy(false),
        max_samples(5000), max_instances(10), max_samples_per_instance(400),
        allocated_samples(100), extra_samples(1) {}
 ...
 * that implements the DurabilityQosPolicy kind of TRANSIENT and PERSISTENT.
 *
 * @warning This QosPolicy can be defined and is transmitted to the rest of the
 * network but is not implemented in this version.
 *
 * @note Immutable Qos Policy
 */
...
public:
  ...
  RTPS_DllAPI DurabilityServiceQosPolicy()
      : Parameter_t(PID_DURABILITY_SERVICE, PARAMETER_TIME_LENGTH +
                                                PARAMETER_KIND_LENGTH + 4 + 4 +
                                                4 + 4),
        QosPolicy(false), history_kind(KEEP_LAST_HISTORY_QOS), history_depth(1),
        max_samples(LENGTH_UNLIMITED), max_instances(LENGTH_UNLIMITED),
...
public:
  /**
   * @brief Control when the service is able to remove all information regarding
   * a data-instance. <br> By default, c_TimeZero.
   */
  fastrtps::Duration_t service_cleanup_delay;
  /**
   * @brief Controls the HistoryQosPolicy of the fictitious DataReader that
   * stores the data within the durability service. <br> By default,
   * KEEP_LAST_HISTORY_QOS.
   */
  HistoryQosPolicyKind history_kind;
  /**
   * @brief Number of most recent values that should be maintained on the
   * History. It only have effect if the history_kind is KEEP_LAST_HISTORY_QOS.
   * <br> By default, 1.
   */
  int32_t history_depth;
  /**
   * @brief Control the ResourceLimitsQos of the implied DataReader that stores
   * the data within the durability service. Specifies the maximum number of
   * data-samples the DataWriter (or DataReader) can manage across all the
   * instances associated with it. Represents the maximum samples the middleware
   * can store for any one DataWriter (or DataReader). It is inconsistent for
   * this value to be less than max_samples_per_instance. <br> By default,
   * LENGTH_UNLIMITED.
   */
  int32_t max_samples;
  /**
   * @brief Control the ResourceLimitsQos of the implied DataReader that stores
   * the data within the durability service. Represents the maximum number of
   * instances DataWriter (or DataReader) can manage. <br> By default,
   * LENGTH_UNLIMITED.
   */
  int32_t max_instances;
  /**
   * @brief Control the ResourceLimitsQos of the implied DataReader that stores
   * the data within the durability service. Represents the maximum number of
   * samples of any one instance a DataWriter(or DataReader) can manage. It is
   * inconsistent for this value to be greater than max_samples. <br> By
...
public:
  ...
  RTPS_DllAPI LifespanQosPolicy()
      : Parameter_t(PID_LIFESPAN, PARAMETER_TIME_LENGTH), QosPolicy(true),
...
public:
  ...
  RTPS_DllAPI OwnershipStrengthQosPolicy()
      : Parameter_t(PID_OWNERSHIP_STRENGTH, 4), QosPolicy(false), value(0) {}
  ...
  bool operator==(const OwnershipStrengthQosPolicy &b) const {
    return (this->value == b.value) && Parameter_t::operator==(b) &&
           QosPolicy::operator==(b);
    }
...
public:
    ...
    RTPS_DllAPI TransportPriorityQosPolicy()
        : Parameter_t(PID_TRANSPORT_PRIORITY, 4), QosPolicy(false), value(0) {}
...
typedef enum PublishModeQosPolicyKind : fastrtps::rtps::octet {
  SYNCHRONOUS_PUBLISH_MODE, //!< Synchronous publication mode (default for
                            //!< writers).
...
public:
    ...
    const char *flow_controller_name =
        fastdds::rtps::FASTDDS_FLOW_CONTROLLER_DEFAULT;
...
public:
  ...
  RTPS_DllAPI DataRepresentationQosPolicy()
      : Parameter_t(PID_DATA_REPRESENTATION, 0), QosPolicy(true) {}
  ...
  bool operator==(const DataRepresentationQosPolicy &b) const {
    return (this->m_value == b.m_value) && Parameter_t::operator==(b) &&
           QosPolicy::operator==(b);
    }
...
public:
    ...
    RTPS_DllAPI TypeConsistencyEnforcementQosPolicy()
        : Parameter_t(PID_TYPE_CONSISTENCY_ENFORCEMENT,
                      8) // 2 + 5 + 1 alignment byte
          ,
          QosPolicy(true) {
...
public:
  ...
  bool operator==(const DisablePositiveACKsQosPolicy &b) const {
    return enabled == b.enabled && duration == b.duration &&
           Parameter_t::operator==(b) && QosPolicy::operator==(b);
    }
...
public:
    ...
    bool enabled;
    //! The duration to keep samples for (not serialized as not needed by
    //! reader). <br> By default, c_TimeInfinite
    fastrtps::Duration_t duration;
};
...
public:
  ...
  RTPS_DllAPI TypeIdV1()
      : Parameter_t(PID_TYPE_IDV1, 0), QosPolicy(false), m_type_identifier() {}
  ...
  RTPS_DllAPI TypeIdV1(const TypeIdV1 &type)
      : Parameter_t(type.Pid, type.length), QosPolicy(type.send_always_),
   ...
   */
  RTPS_DllAPI TypeIdV1(const fastrtps::types::TypeIdentifier &identifier)
      : Parameter_t(PID_TYPE_IDV1, 0), QosPolicy(false),
  ...
  RTPS_DllAPI TypeIdV1 &operator=(const TypeIdV1 &type) {
    ...
    RTPS_DllAPI TypeIdV1 &
    operator=(const fastrtps::types::TypeIdentifier &type_id) {
...
public:
  ...
  RTPS_DllAPI TypeObjectV1()
      : Parameter_t(PID_TYPE_OBJECTV1, 0), QosPolicy(false), m_type_object() {}
   ...
   */
  RTPS_DllAPI TypeObjectV1(const fastrtps::types::TypeObject &type)
      : Parameter_t(PID_TYPE_OBJECTV1, 0), QosPolicy(false),
        m_type_object(type) {}
  ...
  RTPS_DllAPI TypeObjectV1 &operator=(const TypeObjectV1 &type) {
    ...
    RTPS_DllAPI TypeObjectV1 &
    operator=(const fastrtps::types::TypeObject &type_object) {
...
public:
  ...
  RTPS_DllAPI TypeInformation()
      : Parameter_t(PID_TYPE_INFORMATION, 0), QosPolicy(false),
        type_information(), assigned_(false) {}
  ...
  RTPS_DllAPI TypeInformation(const TypeInformation &type)
      : Parameter_t(type.Pid, type.length), QosPolicy(type.send_always_),
        type_information(type.type_information), assigned_(type.assigned_) {}
   ...
   */
  RTPS_DllAPI TypeInformation(const fastrtps::types::TypeInformation &info)
      : Parameter_t(PID_TYPE_INFORMATION, 0), QosPolicy(false),
        type_information(info), assigned_(true) {}
  ...
  RTPS_DllAPI TypeInformation &operator=(TypeInformation &&type) {
    ...
    inline void clear() override { *this = TypeInformation(); }
     ...
     */
    RTPS_DllAPI void assigned(bool value) { assigned_ = value; }

    RTPS_DllAPI TypeInformation &
    operator=(const fastrtps::types::TypeInformation &type_info) {
...
private:
  //! Boolean that states if the TypeInformation has been asigned manually or
  //! not.
  bool assigned_;
};
...
using ParticipantResourceLimitsQos =
    fastrtps::rtps::RTPSParticipantAllocationAttributes;
...
public:
     ...
     * @brief Constructor
     */
  RTPS_DllAPI WireProtocolConfigQos() : QosPolicy(false), participant_id(-1) {}
  ...
  bool operator==(const WireProtocolConfigQos &b) const {
    return (this->prefix == b.prefix) &&
           ...
           (this->default_unicast_locator_list ==
            b.default_unicast_locator_list) &&
           (this->default_multicast_locator_list ==
            b.default_multicast_locator_list) &&
           QosPolicy::operator==(b);
    }
    ...
    /**
     * @brief Throughput controller parameters. Leave default for uncontrolled
     * flow.
     *
...
public:
  ...
  RTPS_DllAPI TransportConfigQos()
      : QosPolicy(false), use_builtin_transports(true),
        send_socket_buffer_size(0), listen_socket_buffer_size(0) {}
...
public:
  ...
  bool operator==(const RTPSEndpointQos &b) const {
    ...
    //! Underlying History memory policy. <br> By default,
...
public:
  ...
  RTPS_DllAPI WriterResourceLimitsQos()
      : matched_subscriber_allocation(), reader_filters_allocation(0, 32u, 1u) {
  }
...
public:
  ...
  RTPS_DllAPI DataSharingQosPolicy()
      : Parameter_t(PID_DATASHARING, 0), QosPolicy(true) {
     ...
     */
    RTPS_DllAPI DataSharingQosPolicy(const DataSharingQosPolicy &b)
        : Parameter_t(b), QosPolicy(b), kind_(b.kind()),
          shm_directory_(b.shm_directory()), max_domains_(b.max_domains()),
          domain_ids_(b.max_domains() != 0 ? b.max_domains()
                                           : b.domain_ids().size()) {
      domain_ids_ = b.domain_ids();
    }

    RTPS_DllAPI DataSharingQosPolicy &operator=(const DataSharingQosPolicy &b) {
      QosPolicy::operator=(b);
      Parameter_t::operator=(b);
      kind_ = b.kind();
      shm_directory_ = b.shm_directory();
      max_domains_ = b.max_domains();
      domain_ids_.reserve(max_domains_ != 0 ? max_domains_
                                            : b.domain_ids().size());
      domain_ids_ = b.domain_ids();
     ...
     */
    RTPS_DllAPI const DataSharingKind &kind() const { return kind_; }
     ...
     */
    RTPS_DllAPI const uint32_t &max_domains() const { return max_domains_; }
     ...
     */
    RTPS_DllAPI void automatic() { setup(AUTO, "", std::vector<uint16_t>()); }
     ...
     */
    RTPS_DllAPI void automatic(const std::vector<uint16_t> &domain_ids) {
     ...
     */
    RTPS_DllAPI void on(const std::string &directory) {
      // TODO [ILG]: This parameter is unused right now. Activate the assert
      // once it is used
     ...
     */
    RTPS_DllAPI void off() { setup(OFF, "", std::vector<uint16_t>()); }
