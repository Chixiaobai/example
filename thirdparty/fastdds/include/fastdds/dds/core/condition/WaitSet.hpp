...
public:
   ...
   * @param cond Condition
   * @return RETCODE_OK if detached correctly, PRECONDITION_NOT_MET if condition
   * was not attached
   */
   ...
   * conditions. If none of the conditions attached to the WaitSet have a
   * trigger_value of true, the wait operation will block suspending the calling
   * thread
   * @param active_conditions Reference to the collection of conditions which
   * trigger_value are true
   * @param timeout Maximum time of the wait
   * @return RETCODE_OK if everything correct, PRECONDITION_NOT_MET if WaitSet
   * already waiting, TIMEOUT if maximum time expired, error code otherwise
   */
  RTPS_DllAPI ReturnCode_t wait(ConditionSeq &active_conditions,
                                const fastrtps::Duration_t timeout) const;

  /**
   * @brief Retrieves the list of attached conditions
   * @param attached_conditions Reference to the collection of attached
   * conditions
   * @return RETCODE_OK if everything correct, error code otherwise
   */
  RTPS_DllAPI ReturnCode_t
  get_conditions(ConditionSeq &attached_conditions) const;
