...
public:
     ...
     * @return RETCODE_OK
     */
    RTPS_DllAPI ReturnCode_t get_qos(DomainParticipantQos &qos) const;
     ...
     */
    RTPS_DllAPI ReturnCode_t set_qos(const DomainParticipantQos &qos) const;
    ...
    /**
     * Modifies the DomainParticipantListener, sets the mask to
     * StatusMask::all()
     *
     * @param listener new value for the DomainParticipantListener
     * @return RETCODE_OK
     */
    RTPS_DllAPI ReturnCode_t set_listener(DomainParticipantListener *listener);
    ...
    create_publisher(const PublisherQos &qos,
     ...
     */
    RTPS_DllAPI Publisher *
    create_publisher_with_profile(const std::string &profile_name,
                                  PublisherListener *listener = nullptr,
                                  const StatusMask &mask = StatusMask::all());
    ...
    create_subscriber(const SubscriberQos &qos,
     ...
     */
    RTPS_DllAPI Subscriber *
    create_subscriber_with_profile(const std::string &profile_name,
                                   SubscriberListener *listener = nullptr,
                                   const StatusMask &mask = StatusMask::all());
     ...
     * RETCODE_OK if it is correctly deleted and RETCODE_ERROR otherwise.
     */
    RTPS_DllAPI ReturnCode_t delete_subscriber(const Subscriber *subscriber);
     ...
     */
    RTPS_DllAPI Topic *create_topic(const std::string &topic_name,
    ...
    RTPS_DllAPI Topic *create_topic_with_profile(
        ...
        const std::string &profile_name, TopicListener *listener = nullptr,
        const StatusMask &mask = StatusMask::all());
     ...
     */
    RTPS_DllAPI ReturnCode_t delete_topic(const Topic *topic);
     ...
     */
    RTPS_DllAPI MultiTopic *
    create_multitopic(const std::string &name, const std::string &type_name,
     ...
     * this participant or if it is referenced by any entity and RETCODE_OK if the Topic was deleted.
     */
    RTPS_DllAPI ReturnCode_t delete_multitopic(const MultiTopic *a_multitopic);
     ...
     */
    RTPS_DllAPI Topic *find_topic(const std::string &topic_name,
    ...
    lookup_topicdescription(const std::string &topic_name) const;
     ...
     * @return RETCODE_OK if the profile exists. RETCODE_BAD_PARAMETER
     * otherwise.
     */
     ...
     */
    RTPS_DllAPI ReturnCode_t
    set_default_subscriber_qos(const SubscriberQos &qos);
     ...
     */
    RTPS_DllAPI ReturnCode_t
    get_default_subscriber_qos(SubscriberQos &qos) const;
     ...
     * @return RETCODE_INCONSISTENT_POLICY if the Qos is not self consistent and RETCODE_OK if the qos is changed correctly.
     */
    RTPS_DllAPI ReturnCode_t set_default_topic_qos(const TopicQos &qos);
     ...
     * @return RETCODE_OK
     */
    RTPS_DllAPI ReturnCode_t get_default_topic_qos(TopicQos &qos) const;
    ...
    /**
     * Retrieves the list of DomainParticipants that have been discovered in the
     * domain and are not "ignored".
     *
     * @param[out]  participant_handles Reference to the vector where discovered
    ...
    /**
     * Retrieves the DomainParticipant data of a discovered not ignored
     * participant.
     *
     * @param[out]  participant_data Reference to the
     * ParticipantBuiltinTopicData object to return the data
     * @param participant_handle InstanceHandle of DomainParticipant to retrieve
     * the data from
     * @return RETCODE_OK if everything correct, PRECONDITION_NOT_MET if
    ...
    /**
     * Retrieves the list of topics that have been discovered in the domain and
     * are not "ignored".
     *
     * @param[out]  topic_handles Reference to the vector where discovered
     * topics will be returned
     * @return RETCODE_OK if everything correct, error code otherwise
     */
    RTPS_DllAPI ReturnCode_t
    get_discovered_topics(std::vector<InstanceHandle_t> &topic_handles) const;

    /**
     * Retrieves the Topic data of a discovered not ignored topic.
     *
     * @param[out]  topic_data Reference to the TopicBuiltinTopicData object to
     * return the data
     * @param topic_handle InstanceHandle of Topic to retrieve the data from
     * @return RETCODE_OK if everything correct, PRECONDITION_NOT_MET if topic
     * does not exist
     */
    RTPS_DllAPI ReturnCode_t
    get_discovered_topic_data(builtin::TopicBuiltinTopicData &topic_data,
     ...
     */
    RTPS_DllAPI bool contains_entity(const InstanceHandle_t &a_handle,
     ...
     */
    RTPS_DllAPI ReturnCode_t
    get_current_time(fastrtps::Time_t &current_time) const;
     ...
     */
    RTPS_DllAPI ReturnCode_t register_type(TypeSupport type,
                                           const std::string &type_name);
     ...
     */
    RTPS_DllAPI ReturnCode_t unregister_type(const std::string &typeName);
     ...
     */
    RTPS_DllAPI bool
    new_remote_endpoint_discovered(const fastrtps::rtps::GUID_t &partguid,
                                   uint16_t userId,
    ...
    /**
     * When a DomainParticipant receives an incomplete list of TypeIdentifiers
     * in a PublicationBuiltinTopicData or SubscriptionBuiltinTopicData, it may
     * request the additional type dependencies by invoking the
     * getTypeDependencies operation.
     *
     * @param in TypeIdentifier sequence
     * @return SampleIdentity
     */
    RTPS_DllAPI fastrtps::rtps::SampleIdentity
    get_type_dependencies(const fastrtps::types::TypeIdentifierSeq &in) const;

    /**
     * A DomainParticipant may invoke the operation getTypes to retrieve the
     * TypeObjects associated with a list of TypeIdentifiers.
     *
     * @param in TypeIdentifier sequence
     * @return SampleIdentity
     */
    RTPS_DllAPI fastrtps::rtps::SampleIdentity
    get_types(const fastrtps::types::TypeIdentifierSeq &in) const;

    /**
     * Helps the user to solve all dependencies calling internally to the type
     * lookup service and registers the resulting dynamic type. The registration
     * may be perform asynchronously, case in which the user will be notified
     * through the given callback, which receives the type_name as unique
     * argument.
     *
     * @param type_information
     * @param type_name
     * @param callback
     * @return RETCODE_OK If the given type_information is enough to build the
     * type without using the typelookup service (callback will not be called).
     * @return RETCODE_OK if the given type is already available (callback will
     * not be called).
     * @return RETCODE_NO_DATA if type is not available yet (the callback will
     * be called if negotiation is success, and ignored in other case).
     * @return RETCODE_NOT_ENABLED if the DomainParticipant is not enabled.
     * @return RETCODE_PRECONDITION_NOT_MET if the DomainParticipant type lookup
     * service is disabled.
     */
     ...
     */
    RTPS_DllAPI IContentFilterFactory *
    lookup_content_filter_factory(const char *filter_class_name);
     ...
     */
    RTPS_DllAPI ReturnCode_t
    unregister_content_filter_factory(const char *filter_class_name);
