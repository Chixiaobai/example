...
public:
  ...
  create_participant(DomainId_t domain_id, const DomainParticipantQos &qos,
   ...
   */
  RTPS_DllAPI DomainParticipant *
  create_participant_with_profile(DomainId_t domain_id,
   ...
   */
  RTPS_DllAPI DomainParticipant *
  create_participant_with_profile(const std::string &profile_name,
                                  DomainParticipantListener *listener = nullptr,
                                  const StatusMask &mask = StatusMask::all());

  /**
   * This operation retrieves a previously created DomainParticipant belonging
   * to specified domain_id. If no such DomainParticipant exists, the operation
   * will return 'nullptr'. If multiple DomainParticipant entities belonging to
   * that domain_id exist, then the operation will return one of them. It is not
   * specified which one.
   *
   * @param domain_id
   ...
   */
  RTPS_DllAPI std::vector<DomainParticipant *>
  lookup_participants(DomainId_t domain_id) const;

  /**
   * @brief This operation retrieves the default value of the DomainParticipant
   * QoS, that is, the QoS policies which will be used for newly created
   * DomainParticipant entities in the case where the QoS policies are defaulted
   * in the create_participant operation. The values retrieved
   * get_default_participant_qos will match the set of values specified on the
   * last successful call to set_default_participant_qos, or else, if the call
   * was never made, the default values.
   *
   * @param qos DomainParticipantQos where the qos is returned
   * @return RETCODE_OK
   */
  RTPS_DllAPI ReturnCode_t
  get_default_participant_qos(DomainParticipantQos &qos) const;
  ...
  /**
   * @brief This operation sets a default value of the DomainParticipant QoS
   * policies which will be used for newly created DomainParticipant entities in
   * the case where the QoS policies are defaulted in the create_participant
   * operation.
   *
   * This operation will check that the resulting policies are self consistent;
   * if they are not, the operation will have no effect and return
   * INCONSISTENT_POLICY.
   *
   * The special value PARTICIPANT_QOS_DEFAULT may be passed to this operation
   * to indicate that the default QoS should be reset back to the initial values
   * the factory would use, that is the values that would be used if the
   * set_default_participant_qos operation had never been called.
   *
   * @param qos DomainParticipantQos to be set
   * @return RETCODE_INCONSISTENT_POLICY if the Qos is not self consistent and
   * RETCODE_OK if the qos is changed correctly.
   */
  RTPS_DllAPI ReturnCode_t
  set_default_participant_qos(const DomainParticipantQos &qos);
   ...
   * @return RETCODE_PRECONDITION_NOT_MET if the participant has active
   * entities, RETCODE_OK if the participant is correctly deleted and
   ...
   */
  RTPS_DllAPI ReturnCode_t
  load_XML_profiles_file(const std::string &xml_profile_file);
   ...
   * @return RETCODE_OK if it is correctly loaded, RETCODE_ERROR otherwise.
   */
  RTPS_DllAPI ReturnCode_t load_XML_profiles_string(const char *data,
                                                    size_t length);
   ...
   * RETCODE_INCONSISTENT_POLICY if the Qos is not self consistent and
   * RETCODE_OK if the qos is changed correctly.
   */
