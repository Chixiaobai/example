...
public:
    ...
    RegisterConsumer(std::unique_ptr<LogConsumer> &&consumer);
     ...
     *  * logError(cat, msg);
     */
    RTPS_DllAPI static void QueueLog(const std::string &message,
                                     const Log::Context &, Log::Kind);

  private:
    // Applies transformations to the entries compliant with the options
    // selected (such as erasure of certain context information, or filtering by
    // category. Returns false if the log entry is blacklisted.
...
protected:
  void print_timestamp(std::ostream &stream, const Log::Entry &,
                       bool color) const;

  void print_header(std::ostream &stream, const Log::Entry &, bool color) const;

  void print_context(std::ostream &stream, const Log::Entry &,
                     bool color) const;

  void print_message(std::ostream &stream, const Log::Entry &,
                     bool color) const;

  void print_new_line(std::ostream &stream, bool color) const;
};
...
#define logError_(cat, msg)                                                    \
  {                                                                            \
    ...
    Log::QueueLog(fastdds_log_ss_tmp__.str(),                                  \
                  Log::Context{__FILE__, __LINE__, __func__, #cat},            \
                  Log::Kind::Error);                                           \
  }
