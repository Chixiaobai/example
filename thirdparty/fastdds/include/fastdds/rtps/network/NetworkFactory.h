...
public:
  NetworkFactory(const RTPSParticipantAttributes &PParam);

  /**
   * Allow registration of a transport statically, by specifying the transport
   * type and its associated descriptor type. This is particularly useful for
   * user-defined transports.
   */
  template <class T, class D> void RegisterTransport(const D &descriptor) {
   ...
   *
   * @param descriptor Structure that defines all initial configuration for a
   * given transport.
   * @param properties Optional policy to specify additional parameters for the
   * created transport.
   */
  ...
  /**
   * Walk over the list of transports, opening every possible channel that can
   * send through the given locator and returning a vector of Sender Resources
   * associated with it.
   * @param locator Locator through which to send.
   */
  bool build_send_resources(fastdds::rtps::SendResourceList &,
                            const Locator_t &locator);

  /**
   * Walk over the list of transports, opening every possible channel that we
   * can listen to from the given locator, and returns a vector of Receiver
   * Resources for this goal.
   * @param local Locator from which to listen.
   * @param returned_resources_list List that will be filled with the created
   * ReceiverResources.
   * @param receiver_max_message_size Max message size allowed by the message
   * receiver.
   */
  ...
  /**
   * Transform a remote locator into a locator optimized for local
   * communications.
   *
   ...
   *
   * @return false if the input locator is not supported/allowed by any of the
   * registered transports, true otherwise.
   */
  bool transform_remote_locator(const Locator_t &remote_locator,
   ...
   *   - the transport selection algorithm is called for each registered
   * transport
   *
  ...
  /**
   * Fill ret_locators with the list of all possible locators in the local
   * machine at the given physical_port of the locator_kind. Return if found
   * any.
   * */
  bool generate_locators(uint16_t physical_port, int locator_kind,
  ...
  bool getDefaultMetatrafficMulticastLocators(
      LocatorList_t &locators, uint32_t metatraffic_multicast_port) const;

  /**
   * Add locators to the metatraffic unicast list.
   * */
  bool
  getDefaultMetatrafficUnicastLocators(LocatorList_t &locators,
   ...
   * */
  bool
  fillMetatrafficMulticastLocator(Locator_t &locator,
                                  uint32_t metatraffic_multicast_port) const;

  /**
   * Fill the locator with the metatraffic unicast configuration.
   * */
  bool fillMetatrafficUnicastLocator(Locator_t &locator,
                                     uint32_t metatraffic_unicast_port) const;

  /**
   * Configure the locator with the initial peer configuration.
   * */
  bool configureInitialPeerLocator(uint32_t domain_id, Locator_t &locator,
                                   RTPSParticipantAttributes &m_att) const;

  /**
   * Add locators to the default unicast configuration.
   * */
  bool getDefaultUnicastLocators(uint32_t domain_id, LocatorList_t &locators,
                                 const RTPSParticipantAttributes &m_att) const;

  /**
   * Fill the locator with the default unicast configuration.
   * */
  bool fill_default_locator_port(uint32_t domain_id, Locator_t &locator,
   ...
   *
   * @param send_resource_list List of send resources associated to the local
   * participant.
   * @param remote_participant_locators List of locators associated to the
   * remote participant.
   * @param participant_initial_peers List of locators of the initial peers of
...
private:
  std::vector<std::unique_ptr<fastdds::rtps::TransportInterface>>
   ...
   */
  uint16_t calculate_well_known_port(uint32_t domain_id,
