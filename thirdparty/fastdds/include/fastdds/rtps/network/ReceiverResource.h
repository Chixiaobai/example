...
/**
 * RAII object that encapsulates the Receive operation over one channel in an
 * unknown transport. A Receiver resource is always univocally associated to a
 * transport channel; the act of constructing a Receiver Resource opens the
 * channel and its destruction closes it.
 * @ingroup NETWORK_MODULE
 */
class ReceiverResource : public fastdds::rtps::TransportReceiverInterface {
  //! Only NetworkFactory is ever allowed to construct a ReceiverResource from
  //! scratch. In doing so, it guarantees the transport and channel are in a
  //! valid state for this resource to exist.
...
public:
  ...
  /**
   * Reports whether this resource supports the given local locator (i.e., said
   * locator maps to the transport channel managed by this resource).
   */
  ...
  inline uint32_t max_message_size() const { return max_message_size_; }
