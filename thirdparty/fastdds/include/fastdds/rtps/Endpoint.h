 ...
 * Although the RTPSParticipant is also defined as an endpoint in the RTPS
 * specification, in this implementation the RTPSParticipant class **does not**
 * inherit from the endpoint class. Each Endpoint object owns a pointer to the
...
protected:
  ...
  Endpoint(RTPSParticipantImpl *pimpl, const GUID_t &guid,
           const EndpointAttributes &att)
      : mp_RTPSParticipant(pimpl), m_guid(guid), m_att(att) {}
