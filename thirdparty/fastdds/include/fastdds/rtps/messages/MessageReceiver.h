...
public:
   ...
   */
  void processCDRMsg(const Locator_t &source_locator,
                     const Locator_t &reception_locator, CDRMessage_t *msg);
...
private:
  ...
  //! GuidPrefix of the entity that receives the message. GuidPrefix of the
  //! RTPSParticipant.
  ...
  std::function<void(const EntityId_t &, CacheChange_t &, bool)>
      process_data_message_function_;
  //! Function used to process a received fragment message
  std::function<void(const EntityId_t &, CacheChange_t &, uint32_t, uint32_t,
                     uint16_t, bool)>
   ...
   */
  bool readSubmessageHeader(CDRMessage_t *msg, SubmessageHeader_t *smh) const;

  /**
   * Find if there is a reader (in associated_readers_) that will accept a msg
   * directed to the given entity ID.
   */
  bool willAReaderAcceptMsgDirectedTo(const EntityId_t &readerID,
                                      RTPSReader *&first_reader) const;

  /**
   * Find all readers (in associated_readers_), with the given entity ID, and
   * call the callback provided.
   */
  template <typename Functor>
  void findAllReaders(const EntityId_t &readerID,
   ...
   * @param[in] was_decoded  Whether the submessage being processed came from
   * decoding a secured submessage
   ...
   *
   * @param[in] reader_id    The ID of the reader to which the changes is
   * addressed
  ...
  ///@{
#if HAVE_SECURITY
  void process_data_message_with_security(const EntityId_t &reader_id,
                                          CacheChange_t &change,
                                          bool was_decoded);
#endif // HAVE_SECURITY

  void process_data_message_without_security(const EntityId_t &reader_id,
                                             CacheChange_t &change,
                                             bool was_decoded);
  ///@}
   ...
   * @param[in] fragment_starting_num   The index of the first fragment in the
   * message
   * @param[in] fragments_in_submessage The number of fragments in the message
   * @param[in] was_decoded             Whether the submessage being processed
   * came from decoding a secured submessage
   */
  ///@{
#if HAVE_SECURITY
  void process_data_fragment_message_with_security(
      const EntityId_t &reader_id, CacheChange_t &change, uint32_t sample_size,
      uint32_t fragment_starting_num, uint16_t fragments_in_submessage,
      bool was_decoded);
#endif // HAVE_SECURITY
  ...
  /**
   * Looks for the statistics specific submessage and notifies statistics
   ...
   */
  void notify_network_statistics(const Locator_t &source_locator,
