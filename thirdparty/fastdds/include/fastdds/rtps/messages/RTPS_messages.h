...
struct Header_t {
  ...
  Header_t() : version(c_ProtocolVersion), vendorId(c_VendorId_eProsima) {}
  ~Header_t() {}
};
...
inline std::ostream &operator<<(std::ostream &output, const Header_t &h) {
  output << "RTPS HEADER of Version: " << (int)h.version.m_major << "."
         << (int)h.version.m_minor;
  output << "  || VendorId: " << std::hex << (int)h.vendorId[0] << "."
         << (int)h.vendorId[1] << std::dec;
  output << "GuidPrefix: " << h.guidPrefix;
  return output;
}

//!@brief Structure SubmessageHeader_t, used to contain the header information
//! of a submessage.
struct SubmessageHeader_t {
  ...
  SubmessageHeader_t()
      : submessageId(0), submessageLength(0), flags(0), is_last(false) {}
};
...
inline std::ostream &operator<<(std::ostream &output,
  ...
  output << "Submessage Header, ID: " << std::hex << (int)sh.submessageId
         << std::dec;
  output << " length: " << (int)sh.submessageLength << " flags "
         << (bitset<8>)sh.flags;
  return output;
}
} // namespace rtps
