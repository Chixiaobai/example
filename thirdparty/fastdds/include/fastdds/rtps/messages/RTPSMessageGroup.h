...
public:
  ...
  /*!
   * Exception thrown when a operation exceeds the maximum bytes this object can
   * process in the current period of time.
   */
   ...
   * @param internal_buffer true indicates this object to allocate its own
   * buffer. false indicates to get a buffer from the participant.
   */
  RTPSMessageGroup(RTPSParticipantImpl *participant,
   ...
   * @param max_blocking_time_point Future time point where blocking send should
   * end.
   */
  RTPSMessageGroup(
      RTPSParticipantImpl *participant, Endpoint *endpoint,
   ...
   * @param expects_inline_qos True when one destination is expecting inline
   * QOS.
   ...
   * @return True when message was added to the group.
   */
  bool add_data_frag(const CacheChange_t &change,
                     const uint32_t fragment_number, bool expects_inline_qos);
   ...
   * @return True when message was added to the group.
   */
  bool add_heartbeat(const SequenceNumber_t &first_seq,
                     const SequenceNumber_t &last_seq, Count_t count,
   ...
   * @return True when message was added to the group.
   */
  bool add_gap(const SequenceNumber_t &gap_initial_sequence,
   ...
   * @return True when message was added to the group.
   */
  bool add_acknack(const SequenceNumberSet_t &seq_num_set, int32_t count,
                   bool final_flag);
   ...
   * @return True when message was added to the group.
   */
  bool add_nackfrag(const SequenceNumber_t &seq_number,
                    FragmentNumberSet_t fn_state, int32_t count);

  /**
   * To be used whenever destination locators/guids change between two add_xxx
   * calls. Automatically called inside add_xxx calls if
   * destinations_have_changed() method of RTPSMessageSenderInterface returns
   * true. May become private again with a refactor of
   * RTPSMessageSenderInterface, adding a group_has_been_flushed() method.
   */
   ...
   *
   * @param endpoint Pointer to next Endpoint sender. nullptr resets object to
   * initial state.
   * @param msg_sender Pointer to the RTPSMessageSenderInterface will be used to
   * send next RTPS messages.. nullptr resets object to initial state.
   * @pre (endpoint != nullptr && msg_sender != nullptr) || (endpoint == nullptr
   * && msg_sender == nullptr)
   */
  void sender(Endpoint *endpoint, RTPSMessageSenderInterface *msg_sender) {
    assert((endpoint != nullptr && msg_sender != nullptr) ||
           (endpoint == nullptr && msg_sender == nullptr));
  ...
  static inline constexpr uint32_t get_max_fragment_payload_size() {
    // Max fragment is 64KBytes_max - header - inlineqos - 3(for better
    // alignment)
    return std::numeric_limits<uint16_t>::max() - data_frag_header_size_ -
           max_inline_qos_size_ - 3;
  }

  void set_sent_bytes_limitation(uint32_t limit) {
    sent_bytes_limitation_ = limit;
  }

  void reset_current_bytes_processed() { current_sent_bytes_ = 0; }
...
private:
  ...
  void check_and_maybe_flush(const GuidPrefix_t &destination_guid_prefix);
  ...
  bool insert_submessage(const GuidPrefix_t &destination_guid_prefix,
  ...
  bool create_gap_submessage(const SequenceNumber_t &gap_initial_sequence,
                             const SequenceNumberSet_t &gap_bitmap,
