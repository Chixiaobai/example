...
inline bool CDRMessage::initCDRMsg(CDRMessage_t *msg, uint32_t payload_size) {
  if (msg->buffer == nullptr) {
    msg->buffer =
        (octet *)malloc(payload_size + RTPSMESSAGE_COMMON_RTPS_PAYLOAD_SIZE);
...
inline bool CDRMessage::wrapVector(CDRMessage_t *msg,
...
inline bool CDRMessage::read_array_with_max_size(CDRMessage_t *msg, octet *arr,
                                                 size_t max_size) {
  if (msg->pos + 4 > msg->length) {
    return false;
  }
...
inline bool CDRMessage::readDataReversed(CDRMessage_t *msg, octet *o,
                                         uint32_t length) {
...
inline bool CDRMessage::readUInt64(CDRMessage_t *msg, uint64_t *ulolo) {
  if (msg->pos + 8 > msg->length) {
    return false;
  }
...
inline bool CDRMessage::readSequenceNumber(CDRMessage_t *msg,
                                           SequenceNumber_t *sn) {
  if (msg->pos + 8 > msg->length) {
    return false;
  }
...
inline SequenceNumberSet_t
CDRMessage::readSequenceNumberSet(CDRMessage_t *msg) {
  bool valid = true;
  ...
  if (valid && std::numeric_limits<int32_t>::max() == seqNum.high) {
    numBits = (std::min)(numBits,
                         (std::numeric_limits<uint32_t>::max)() - seqNum.low);
  }
...
inline bool CDRMessage::readFragmentNumberSet(CDRMessage_t *msg,
...
inline bool CDRMessage::readOctetVector(CDRMessage_t *msg,
...
inline bool CDRMessage::hasSpace(CDRMessage_t *msg, const uint32_t length) {
  return msg && (msg->pos + length <= msg->max_size);
}

inline void CDRMessage::copyToBuffer(CDRMessage_t *msg, const octet *data,
                                     const uint32_t length, bool reverse) {
  if (reverse) {
    for (uint32_t i = 0; i < length; i++) {
...
inline bool CDRMessage::addData(CDRMessage_t *msg, const octet *data,
...
inline bool CDRMessage::addDataReversed(CDRMessage_t *msg, const octet *data,
                                        const uint32_t length) {
...
template <typename T>
inline bool CDRMessage::addPrimitive(CDRMessage_t *msg, T value) {
...
inline bool CDRMessage::addUInt64(CDRMessage_t *msg, uint64_t ulolo) {
  return addPrimitive(msg, ulolo);
}

inline bool CDRMessage::addOctetVector(CDRMessage_t *msg,
  ...
  valid &=
      CDRMessage::addData(msg, (octet *)ocvec->data(), (uint32_t)ocvec->size());
...
inline bool CDRMessage::add_string(CDRMessage_t *msg,
...
inline bool CDRMessage::addProperty(CDRMessage_t *msg,
...
inline bool CDRMessage::addBinaryProperty(CDRMessage_t *msg,
...
inline bool CDRMessage::readBinaryProperty(CDRMessage_t *msg,
...
inline bool CDRMessage::addPropertySeq(CDRMessage_t *msg,
  ...
  if (msg->pos + 4 <= msg->max_size) {
    ...
    if (CDRMessage::addUInt32(msg, number_to_serialize)) {
      ...
      for (auto it = properties.begin();
           returnedValue && it != properties.end(); ++it) {
        if (it->propagate()) {
...
inline bool CDRMessage::readPropertySeq(CDRMessage_t *msg,
  ...
  // Length should be at least 16 times the number of elements, since each
  // property contains 2 empty strings, each with 4 bytes for its length + at
  // least 4 bytes of data (single NUL character + padding)
...
inline bool
CDRMessage::addBinaryPropertySeq(CDRMessage_t *msg,
  ...
  if (msg->pos + 4 <= msg->max_size) {
    ...
    if (CDRMessage::addUInt32(msg, number_to_serialize)) {
      returnedValue = true;
      for (auto it = binary_properties.begin();
           returnedValue && it != binary_properties.end(); ++it) {
        if (it->propagate()) {
          --number_to_serialize;
          returnedValue = CDRMessage::addBinaryProperty(
              msg, *it, add_final_padding || (number_to_serialize != 0));
        }
      }
    }
  }
...
inline bool CDRMessage::addBinaryPropertySeq(
    CDRMessage_t *msg, const BinaryPropertySeq &binary_properties,
    const std::string &name_start, bool add_final_padding) {
  assert(msg);
...
inline bool
CDRMessage::readBinaryPropertySeq(CDRMessage_t *msg,
  ...
  // Length should be at least 12 times the number of elements, since each each
  // property contains at least 1 empty string with 4 bytes for its length + at
  // least 4 bytes of data (single NUL character + padding) and 1 empty byte
  // sequence with 4 bytes for its length
  ...
  for (uint32_t i = 0; returnedValue && i < length; ++i) {
    returnedValue =
        CDRMessage::readBinaryProperty(msg, binary_properties.at(i));
  }

  return returnedValue;
}

inline bool CDRMessage::addDataHolder(CDRMessage_t *msg,
...
inline bool CDRMessage::readDataHolder(CDRMessage_t *msg,
...
inline bool CDRMessage::addDataHolderSeq(CDRMessage_t *msg,
  ...
  if (msg->pos + 4 <= msg->max_size) {
    if (CDRMessage::addUInt32(msg,
                              static_cast<uint32_t>(data_holders.size()))) {
      returnedValue = true;
      for (auto data_holder = data_holders.begin();
           returnedValue && data_holder != data_holders.end(); ++data_holder) {
...
inline bool CDRMessage::readDataHolderSeq(CDRMessage_t *msg,
  ...
  // Length should be at least 16 times the number of elements, since each
  // DataHolder contains at least 1 empty string with 4 bytes for its length +
  // at least 4 bytes of data (single NUL character + padding) and 2 empty
  // property sequences, each with 4 bytes for its length
  ...
  for (uint32_t i = 0; returnedValue && i < length; ++i) {
    ...
    returnedValue =
        CDRMessage::readDataHolder(msg, data_holders.at(i), remaining_length);
  }
...
inline bool CDRMessage::addMessageIdentity(
    CDRMessage_t *msg, const security::MessageIdentity &message_identity) {
  assert(msg);

  if (!CDRMessage::addData(msg, message_identity.source_guid().guidPrefix.value,
                           GuidPrefix_t::size)) {
    return false;
  }
  if (!CDRMessage::addData(msg, message_identity.source_guid().entityId.value,
                           EntityId_t::size)) {
    return false;
  }
...
inline bool
CDRMessage::readMessageIdentity(CDRMessage_t *msg,
                                security::MessageIdentity &message_identity) {
  assert(msg);

  if (!CDRMessage::readData(msg,
                            message_identity.source_guid().guidPrefix.value,
                            GuidPrefix_t::size)) {
    return false;
  }
  if (!CDRMessage::readData(msg, message_identity.source_guid().entityId.value,
                            EntityId_t::size)) {
    return false;
  }
...
inline bool CDRMessage::addParticipantGenericMessage(
    CDRMessage_t *msg, const security::ParticipantGenericMessage &message) {
  assert(msg);

  if (!CDRMessage::addMessageIdentity(msg, message.message_identity())) {
    return false;
  }
  if (!CDRMessage::addMessageIdentity(msg,
                                      message.related_message_identity())) {
    return false;
  }
  if (!CDRMessage::addData(
          msg, message.destination_participant_key().guidPrefix.value,
          GuidPrefix_t::size)) {
    return false;
  }
  if (!CDRMessage::addData(msg,
                           message.destination_participant_key().entityId.value,
                           EntityId_t::size)) {
    return false;
  }
  if (!CDRMessage::addData(msg,
                           message.destination_endpoint_key().guidPrefix.value,
                           GuidPrefix_t::size)) {
    return false;
  }
  if (!CDRMessage::addData(msg,
                           message.destination_endpoint_key().entityId.value,
                           EntityId_t::size)) {
    return false;
  }
  if (!CDRMessage::addData(msg, message.source_endpoint_key().guidPrefix.value,
                           GuidPrefix_t::size)) {
    return false;
  }
  if (!CDRMessage::addData(msg, message.source_endpoint_key().entityId.value,
                           EntityId_t::size)) {
    return false;
  }
...
inline bool CDRMessage::readParticipantGenericMessage(
    CDRMessage_t *msg, security::ParticipantGenericMessage &message) {
  assert(msg);

  if (!CDRMessage::readMessageIdentity(msg, message.message_identity())) {
    return false;
  }
  if (!CDRMessage::readMessageIdentity(msg,
                                       message.related_message_identity())) {
    return false;
  }
  if (!CDRMessage::readData(
          msg, message.destination_participant_key().guidPrefix.value,
          GuidPrefix_t::size)) {
    return false;
  }
  if (!CDRMessage::readData(
          msg, message.destination_participant_key().entityId.value,
          EntityId_t::size)) {
    return false;
  }
  if (!CDRMessage::readData(msg,
  ...
  if (!CDRMessage::readData(msg, message.source_endpoint_key().guidPrefix.value,
                            GuidPrefix_t::size)) {
    return false;
  }
  if (!CDRMessage::readData(msg, message.source_endpoint_key().entityId.value,
                            EntityId_t::size)) {
    return false;
  }
...
inline bool CDRMessage::skip(CDRMessage_t *msg, uint32_t length) {
  // Validate input
