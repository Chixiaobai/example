...
/**
 * @brief Class RTPSMessageCreator, allows the generation of serialized CDR RTPS
 * Messages.
...
public:
   ...
   * @return True if correct.
   */
  static bool addCustomContent(CDRMessage_t *msg, const octet *content,
   ...
   * @return True if correct.
   */
  static bool addSubmessageHeader(CDRMessage_t *msg, octet id, octet flags,
                                  uint16_t size);

  /** @name CDR messages creation methods.
   * These methods create a CDR message for different types
   * Depending on the function a complete message (with RTPS Header is created)
   * or only the submessage.
   * @param[out] msg Pointer to where the message is going to be created and
   * stored.
  ...
  static bool addMessageData(CDRMessage_t *msg, GuidPrefix_t &guidprefix,
                             const CacheChange_t *change, TopicKind_t topicKind,
                             const EntityId_t &readerId, bool expectsInlineQos,
  ...
  static bool
  addMessageDataFrag(CDRMessage_t *msg, GuidPrefix_t &guidprefix,
                     const CacheChange_t *change, uint32_t fragment_number,
                     TopicKind_t topicKind, const EntityId_t &readerId,
                     bool expectsInlineQos, InlineQosWriter *inlineQos);
  static bool
  addSubmessageDataFrag(CDRMessage_t *msg, const CacheChange_t *change,
                        uint32_t fragment_number,
                        const SerializedPayload_t &data, TopicKind_t topicKind,
                        const EntityId_t &readerId, bool expectsInlineQos,
                        InlineQosWriter *inlineQos);
  ...
  static bool addSubmessageGap(CDRMessage_t *msg,
                               const SequenceNumber_t &seqNumFirst,
                               const SequenceNumberSet_t &seqNumList,
                               const EntityId_t &readerId,
                               const EntityId_t &writerId);

  static bool addMessageHeartbeat(CDRMessage_t *msg,
  ...
  static bool addSubmessageHeartbeat(
      CDRMessage_t *msg, const EntityId_t &readerId, const EntityId_t &writerId,
      const SequenceNumber_t &firstSN, const SequenceNumber_t &lastSN,
      Count_t count, bool isFinal, bool livelinessFlag);

  static bool addMessageHeartbeatFrag(CDRMessage_t *msg,
                                      const GuidPrefix_t &guidprefix,
                                      const EntityId_t &readerId,
                                      const EntityId_t &writerId,
  ...
  static bool addMessageAcknack(CDRMessage_t *msg,
                                const GuidPrefix_t &guidprefix,
                                const GuidPrefix_t &remoteGuidPrefix,
                                const EntityId_t &readerId,
                                const EntityId_t &writerId,
                                const SequenceNumberSet_t &SNSet, int32_t count,
                                bool finalFlag);
  ...
  static bool addMessageNackFrag(CDRMessage_t *msg,
                                 const GuidPrefix_t &guidprefix,
                                 const GuidPrefix_t &remoteGuidPrefix,
                                 const EntityId_t &readerId,
                                 const EntityId_t &writerId,
  ...
  static bool addSubmessageInfoTS(CDRMessage_t *msg, const Time_t &time,
                                  bool invalidateFlag);
  static bool addSubmessageInfoTS_Now(CDRMessage_t *msg, bool invalidateFlag);

  static bool addSubmessageInfoSRC(CDRMessage_t *msg,
                                   const ProtocolVersion_t &version,
                                   const VendorId_t &vendorId,
                                   const GuidPrefix_t &guidP);
  static bool addSubmessageInfoDST(CDRMessage_t *msg,
                                   const GuidPrefix_t &guidP);
};
} // namespace rtps
} // namespace fastrtps
