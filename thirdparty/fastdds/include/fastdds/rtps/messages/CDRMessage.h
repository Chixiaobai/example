...
/**
 * Namespace CDRMessage, contains inline methods to initialize CDRMessage_t and
 add or read different data types.
   @ingroup COMMON_MODULE
 */
namespace CDRMessage {

/** @name Read from a CDRMessage_t.
 * Methods to read different data types from a CDR message. Pointers to the
 * message and to the data types are provided. The read position is updated in
 * the message. It fails if you attempt to read outside the boundaries of the
 * message.
...
inline bool read_array_with_max_size(CDRMessage_t *msg, octet *arr,
                                     size_t max_size);
...
inline bool readUInt64(CDRMessage_t *msg, uint64_t *lolo);
...
inline bool readFragmentNumberSet(CDRMessage_t *msg,
...
inline bool readBinaryProperty(CDRMessage_t *msg,
...
inline bool readBinaryPropertySeq(CDRMessage_t *msg,
...
inline bool readMessageIdentity(CDRMessage_t *msg,
                                security::MessageIdentity &message_identity);

inline bool
readParticipantGenericMessage(CDRMessage_t *msg,
                              security::ParticipantGenericMessage &message);
///@}

/**
 * Initialize given CDR message with default size. It frees the memory already
 * allocated and reserves new one.
 * @param[in,out] msg Pointer to the message to initialize.
 * @param data_size Size of the data the message is suppose to carry
 * @return True if correct.
 */
inline bool
initCDRMsg(CDRMessage_t *msg,
...
/**
 * Append given CDRMessage to existing CDR Message. Joins two messages into the
 *first one if it has space.
 ...
 * Methods to add different data types to a CDR message. Pointers to the message
 * and to the data types are provided. The write position is updated in the
 * message. It fails if you attempt to write outside the boundaries of the
 * message.
...
template <typename T> inline bool addPrimitive(CDRMessage_t *msg, T value);

inline bool hasSpace(CDRMessage_t *msg, const uint32_t length);

inline void copyToBuffer(CDRMessage_t *msg, const octet *data,
                         const uint32_t length, bool reverse = false);

inline bool addData(CDRMessage_t *, const octet *, const uint32_t number_bytes);
inline bool addDataReversed(CDRMessage_t *, const octet *,
...
inline bool addUInt64(CDRMessage_t *msg, uint64_t lo);
...
inline bool addSequenceNumberSet(CDRMessage_t *msg,
                                 const SequenceNumberSet_t *sns);

inline bool addFragmentNumberSet(CDRMessage_t *msg, FragmentNumberSet_t *fns);

inline bool addLocator(CDRMessage_t *msg, const Locator_t &loc);

inline bool add_string(CDRMessage_t *msg, const char *in_str);

inline bool add_string(CDRMessage_t *msg, const std::string &in_str);

inline bool add_string(CDRMessage_t *msg, const string_255 &in_str);
...
inline bool addBinaryProperty(CDRMessage_t *msg,
...
inline bool addDataHolderSeq(CDRMessage_t *msg,
                             const DataHolderSeq &data_holders);

inline bool
addMessageIdentity(CDRMessage_t *msg,
 ...
 */
inline bool skip(CDRMessage_t *msg, uint32_t length);
