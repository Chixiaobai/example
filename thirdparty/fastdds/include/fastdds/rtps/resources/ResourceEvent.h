...
public:
  ...
  /*!
   * @brief This method removes a TimedEventImpl object in case it is waiting to
   * be processed by ResourceEvent's internal thread.
   *
   * This method has to be called before deleting the TimedEventImpl object.
   * This method cancels any operation of the timer.
   * Then it avoids the situation of the execution thread calling the event
   * handler when it was previously removed.
   * @param event TimedEventImpl object that will be deleted and we have to be
   * sure all its operations are cancelled.
   */
  void unregister_timer(TimedEventImpl *event);

  /*!
   * @brief This method notifies to ResourceEvent that the TimedEventImpl object
   * has operations to be scheduled.
   *
   * These operations can be the cancellation of the timer or starting another
   * async_wait.
...
private:
  ...
  //! Flag used to allow a thread to manipulate the timer collections when the
  //! execution thread is not using them.
  ...
  //! Prevents iterator invalidation when active_timers are manipulated inside
  //! loops
  ...
  /*!
   * @brief Registers a new TimedEventImpl object in the internal queue to be
   * processed. Non thread safe.
   * @param event Event to be added in the queue.
   * @return True value if the insertion was successful. In other case, it
   * return False.
   */
  ...
  //! Ensures internal collections can accommodate current total number of
  //! timers.
