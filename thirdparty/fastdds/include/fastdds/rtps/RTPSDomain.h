...
public:
   ...
   *
   * \post After this call, all the pointers to RTPS entities are invalidated
   * and their use may result in undefined behaviour.
   */
  RTPS_DllAPI static void stopAll();

  /**
   * @brief Create a RTPSParticipant.
   * @param domain_id DomainId to be used by the RTPSParticipant (80 by
   * default).
   ...
   *
   * \warning The returned pointer is invalidated after a call to
   * removeRTPSParticipant() or stopAll(), so its use may result in undefined
   * behaviour.
   */
  RTPS_DllAPI static RTPSParticipant *
  createParticipant(uint32_t domain_id, const RTPSParticipantAttributes &attrs,
   ...
   * @param domain_id DomainId to be used by the RTPSParticipant (80 by
   * default).
   * @param enabled True if the RTPSParticipant should be enabled on creation.
   * False if it will be enabled later with RTPSParticipant::enable()
   * @param attrs RTPSParticipant Attributes.
   * @param plisten Pointer to the ParticipantListener.
   * @return Pointer to the RTPSParticipant.
   *
   ...
   */
  RTPS_DllAPI static RTPSParticipant *
  createParticipant(uint32_t domain_id, bool enabled,
   ...
   *
   * \warning The returned pointer is invalidated after a call to
   * removeRTPSWriter() or stopAll(), so its use may result in undefined
   * behaviour.
   */
  RTPS_DllAPI static RTPSWriter *
  createRTPSWriter(RTPSParticipant *p, WriterAttributes &watt,
   ...
   */
  RTPS_DllAPI static RTPSWriter *
  createRTPSWriter(RTPSParticipant *p, const EntityId_t &entity_id,
   ...
   *
   * \warning The returned pointer is invalidated after a call to
   * removeRTPSReader() or stopAll(), so its use may result in undefined
   * behaviour.
   */
  RTPS_DllAPI static RTPSReader *
  createRTPSReader(RTPSParticipant *p, ReaderAttributes &ratt,
   ...
   */
  RTPS_DllAPI static RTPSReader *
  createRTPSReader(RTPSParticipant *p, const EntityId_t &entity_id,
  ...
  /**
   * Creates a RTPSParticipant as default server or client if ROS_MASTER_URI
  ...
  static RTPSParticipant *clientServerEnvironmentCreationOverride(
      uint32_t domain_id, bool enabled, const RTPSParticipantAttributes &attrs,
      RTPSParticipantListener *listen /*= nullptr*/);

private:
  typedef std::pair<RTPSParticipant *, RTPSParticipantImpl *>
      t_p_RTPSParticipant;
  ...
  static void removeRTPSParticipant_nts(t_p_RTPSParticipant &);
