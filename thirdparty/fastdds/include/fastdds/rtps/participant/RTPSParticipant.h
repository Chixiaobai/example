...
public:
  ...
  //	//!Method to loose the next change (ONLY FOR TEST). //TODO remove this
  // method because is only for testing 	void loose_next_change();

  //! Stop the RTPSParticipant announcement period. //TODO remove this method
  //! because is only for testing
  void stopRTPSParticipantAnnouncement();

  //! Reset the RTPSParticipant announcement period. //TODO remove this method
  //! because is only for testing
   ...
   */
  bool registerWriter(RTPSWriter *Writer, const TopicAttributes &topicAtt,
   ...
   */
  bool registerReader(
      RTPSReader *Reader, const TopicAttributes &topicAtt,
   ...
   */
  bool updateWriter(RTPSWriter *Writer, const TopicAttributes &topicAtt,
                    const WriterQos &wqos);
  ...
  /**
   * @brief Fills a new entityId if set to unknown, or checks if a entity
   * already exists with that entityId in other case.
   * @param entityId to check of fill. If filled, EntityKind will be
  ...
  /**
   * Allows setting a function to check if a type is already known by the top
   * level API participant.
   */
  ...
  bool
  is_security_enabled_for_writer(const WriterAttributes &writer_attributes);
  ...
  bool
  is_security_enabled_for_reader(const ReaderAttributes &reader_attributes);
   ...
   * @param listener
   * @param kind combination of fastdds::statistics::EventKind flags used as a
   * mask. Events to notify.
   * @return true if successfully added
   */
  bool add_statistics_listener(
      std::shared_ptr<fastdds::statistics::IListener> listener, uint32_t kind);

  /*
   * Remove a listener from receiving statistics backend callbacks
   * @param listener
   * @param kind combination of fastdds::statistics::EventKind flags used as a
   * mask. Events to ignore.
