...
public:
  //!  Constructor
  RTPS_DllAPI ChainingTransport(const ChainingTransportDescriptor &t)
  ...
  /*!
   * Initialize the low-level transport. This method will prepare all the
   * internals of the transport.
   * @param properties Optional policy to specify additional parameters of the
   * created transport.
   * @return True when the transport was correctly initialized.
   */
  RTPS_DllAPI bool
  init(const fastrtps::rtps::PropertyPolicy *properties = nullptr) override {
   ...
   * Channels must either be fully closed or fully open, so that "open" and
   * "close" operations are whole and definitive.
   */
  RTPS_DllAPI bool
  IsInputChannelOpen(const fastrtps::rtps::Locator_t &loc) const override {
    return low_level_transport_->IsInputChannelOpen(loc);
  }

  /*!
   * Call the low-level transport `IsLocatorSupported()`.
   * Must report whether the given locator is supported by this transport
   * (typically inspecting its "kind" value).
   */
  RTPS_DllAPI bool
  IsLocatorSupported(const fastrtps::rtps::Locator_t &loc) const override {
    return low_level_transport_->IsLocatorSupported(loc);
  }

  /*!
   * Call the low-level transport `RemoteToMainLocal()`.
   * Returns the locator describing the main (most general) channel that can
   * write to the provided remote locator.
   */
  RTPS_DllAPI fastrtps::rtps::Locator_t
  RemoteToMainLocal(const fastrtps::rtps::Locator_t &loc) const override {
   ...
   */
  RTPS_DllAPI bool
  OpenInputChannel(const fastrtps::rtps::Locator_t &loc,
   ...
   * Must open the channel that maps to/from the given locator. This method must
   * allocate, reserve and mark any resources that are needed for said channel.
   */
  RTPS_DllAPI bool
  OpenOutputChannel(SendResourceList &sender_resource_list,
   ...
   * IMPORTANT: It MUST be safe to call this method even during a Receive
   * operation on another thread. You must implement any necessary mutual
   * exclusion and timeout mechanisms to make sure the channel can be closed
   * without damage.
   */
  RTPS_DllAPI bool
  CloseInputChannel(const fastrtps::rtps::Locator_t &loc) override {
   ...
   */
  RTPS_DllAPI fastrtps::rtps::LocatorList_t
  NormalizeLocator(const fastrtps::rtps::Locator_t &locator) override {
   ...
   */
  RTPS_DllAPI bool
  is_local_locator(const fastrtps::rtps::Locator_t &locator) const override {
   ...
   */
  RTPS_DllAPI bool
  is_locator_allowed(const fastrtps::rtps::Locator_t &locator) const override {
   ...
   */
  RTPS_DllAPI void
  select_locators(fastrtps::rtps::LocatorSelector &selector) const override {
   ...
   */
  RTPS_DllAPI void
  AddDefaultOutputLocator(fastrtps::rtps::LocatorList_t &defaultList) override {
  ...
  RTPS_DllAPI bool getDefaultMetatrafficMulticastLocators(
    ...
    return low_level_transport_->getDefaultMetatrafficMulticastLocators(
        locators, metatraffic_multicast_port);
  }
  ...
  RTPS_DllAPI bool getDefaultMetatrafficUnicastLocators(
    ...
    return low_level_transport_->getDefaultMetatrafficUnicastLocators(
        locators, metatraffic_unicast_port);
  }
   ...
   */
  RTPS_DllAPI bool
  getDefaultUnicastLocators(fastrtps::rtps::LocatorList_t &locators,
                            uint32_t unicast_port) const override {
    return low_level_transport_->getDefaultUnicastLocators(locators,
                                                           unicast_port);
  }

  /*!
   * Call the low-level transport `fillMetatrafficMulticastLocator()`.
   * Assign port to the given metatraffic multicast locator if not already
   * defined
   */
  RTPS_DllAPI bool fillMetatrafficMulticastLocator(
      fastrtps::rtps::Locator_t &locator,
      uint32_t metatraffic_multicast_port) const override {
    return low_level_transport_->fillMetatrafficMulticastLocator(
        locator, metatraffic_multicast_port);
  }
  ...
  RTPS_DllAPI bool fillMetatrafficUnicastLocator(
      fastrtps::rtps::Locator_t &locator,
      uint32_t metatraffic_unicast_port) const override {
    return low_level_transport_->fillMetatrafficUnicastLocator(
        locator, metatraffic_unicast_port);
  }
  ...
  RTPS_DllAPI bool configureInitialPeerLocator(
      fastrtps::rtps::Locator_t &locator,
      const fastrtps::rtps::PortParameters &port_params, uint32_t domainId,
      fastrtps::rtps::LocatorList_t &list) const override {
    return low_level_transport_->configureInitialPeerLocator(
        locator, port_params, domainId, list);
  }
   ...
   */
  RTPS_DllAPI bool fillUnicastLocator(fastrtps::rtps::Locator_t &locator,
  ...
  //! Transforms a remote locator into a locator optimized for local
  //! communications.
  RTPS_DllAPI bool transform_remote_locator(
    ...
    return low_level_transport_->transform_remote_locator(remote_locator,
                                                          result_locator);
  }
  ...
  /**
   * Blocking Send through the specified channel. It may perform operations on
   the output buffer.
   * At the end the function must call to the low-level transport's `send()`
   function.
   * @code{.cpp}
         // Example of calling the low-level transport `send()` function.
         return low_sender_resource->send(send_buffer, send_buffer_size,
   ...
   * @param send_buffer_size Size of the raw data. It will be used as a bounds
   check for the previous argument.
   * It must not exceed the \c sendBufferSize fed to this class during
   construction.
   * @param destination_locators_begin First iterator of the list of Locators
   describing the remote destinations
   * we're sending to.
   * @param destination_locators_end End iterator of the list of Locators
   describing the remote destinations
   * we're sending to.
   * @param timeout Maximum blocking time.
   */
  RTPS_DllAPI virtual bool
  send(fastrtps::rtps::SenderResource *low_sender_resource,
       const fastrtps::rtps::octet *send_buffer, uint32_t send_buffer_size,
       fastrtps::rtps::LocatorsIterator *destination_locators_begin,
       fastrtps::rtps::LocatorsIterator *destination_locators_end,
       const std::chrono::steady_clock::time_point &timeout) = 0;

  /*!
   * Blocking Receive from the specified channel. It may perform operations on
   the input buffer.
   * At the end the function must call to the `next_receiver`'s `OnDataReceived`
   function.
   * @code{.cpp}
         // Example of calling the `next_receiver`'s `OnDataReceived` function.
         next_receiver->OnDataReceived(receive_buffer, receive_buffer_size,
   local_locator, remote_locator);
     @endcode
   * @param next_receiver Next resource receiver to be called.
   * @param receive_buffer vector with enough capacity (not size) to accommodate
   a full receive buffer. That
   * capacity must not be less than the \c receiveBufferSize supplied to this
   class during construction.
   * @param receive_buffer_size Size of the raw data. It will be used as bounds
   check for the previous argument.
   * It must not exceed the \c receiveBufferSize fed to this class during
   construction.
   * @param local_locator Locator mapping to the local channel we're listening
   to.
   * @param[out] remote_locator Locator describing the remote destination we
   received a packet from.
   */
  RTPS_DllAPI virtual void
  receive(TransportReceiverInterface *next_receiver,
...
private:
  std::map<fastrtps::rtps::Locator_t, ChainingReceiverResource *>
      receiver_resources_;
};
