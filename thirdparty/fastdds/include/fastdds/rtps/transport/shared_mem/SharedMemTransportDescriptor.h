 ...
 * The kind value for SharedMemTransportDescriptor is given by
...
struct SharedMemTransportDescriptor : public TransportDescriptorInterface {
  ...
  uint32_t min_send_buffer_size() const override { return segment_size_; }
  ...
  //! Copy assignment
  RTPS_DllAPI SharedMemTransportDescriptor &
  operator=(const SharedMemTransportDescriptor &t) = default;

  //! Return the size of the shared memory segment
  RTPS_DllAPI uint32_t segment_size() const { return segment_size_; }
  ...
  virtual uint32_t max_message_size() const override { return maxMessageSize; }

  //! Set the maximum size of a single message in the transport (in octets)
  RTPS_DllAPI void max_message_size(uint32_t max_message_size) {
  ...
  RTPS_DllAPI std::string rtps_dump_file() const { return rtps_dump_file_; }
