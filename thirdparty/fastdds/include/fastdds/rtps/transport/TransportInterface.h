...
FASTDDS_DEPRECATED_UNTIL(
    3, s_minimumSocketBuffer,
    "Minimum socket buffer is now taken from the maximum msg size")
...
using SendResourceList =
    std::vector<std::unique_ptr<fastrtps::rtps::SenderResource>>;

/**
 * Interface against which to implement a transport layer, decoupled from
 * FastRTPS internals. TransportInterface expects the user to implement a
 * logical equivalence between Locators and protocol-specific "channels". This
 * equivalence can be narrowing: For instance in UDP/IP, a port would take the
 * role of channel, and several different locators can map to the same port, and
...
public:
  /**
   * Aside from the API defined here, an user-defined Transport must define a
   * descriptor data type and a constructor that expects a constant reference to
   * such descriptor. e.g:
   *
   * class MyTransport:
   * public:
  ...
  /**
   * Initialize this transport. This method will prepare all the internals of
   * the transport.
   ...
   */
  virtual bool
  init(const fastrtps::rtps::PropertyPolicy *properties = nullptr) = 0;
   ...
   * communications.
   *
   * If the remote locator corresponds to one of the local interfaces, it is
   * converted to the corresponding local address.
   *
   * @param [in]  remote_locator Locator to be converted.
   * @param [out] result_locator Converted locator.
   *
   * @return false if the input locator is not supported/allowed by this
   * transport, true otherwise.
   */
  ...
  //! Must open the channel that maps to/from the given locator. This method
  //! must allocate, reserve and mark any resources that are needed for said
  //! channel.
   ...
   */
  virtual bool OpenInputChannel(const Locator &, TransportReceiverInterface *,
                                uint32_t) = 0;
   ...
   *   - if a locator from an entry is selected, selector.select is called for
   * that entry
   *
   * @param [in, out] selector Locator selector.
   */
  virtual void
  select_locators(fastrtps::rtps::LocatorSelector &selector) const = 0;
  ...
  virtual bool getDefaultMetatrafficMulticastLocators(
      LocatorList &locators, uint32_t metatraffic_multicast_port) const = 0;

  //! Add metatraffic unicast locator with the given port
  virtual bool getDefaultMetatrafficUnicastLocators(
      LocatorList &locators, uint32_t metatraffic_unicast_port) const = 0;
  ...
  virtual bool fillMetatrafficMulticastLocator(
      Locator &locator, uint32_t metatraffic_multicast_port) const = 0;

  //! Assign port to the given metatraffic unicast locator if not already
  //! defined
  virtual bool
  fillMetatrafficUnicastLocator(Locator &locator,
                                uint32_t metatraffic_unicast_port) const = 0;

  //! Configure the initial peer locators list
  virtual bool
  configureInitialPeerLocator(Locator &locator,
                              const fastrtps::rtps::PortParameters &port_params,
                              uint32_t domainId, LocatorList &list) const = 0;
