...
/**
 * Virtual base class for the data type used to define configuration of
 * transports using sockets.
 *
 * - \c sendBufferSize: size of the sending buffer of the socket (in octets).
 *
 * - \c receiveBufferSize: size of the receiving buffer of the socket (in
 * octets).
 *
...
struct SocketTransportDescriptor : public TransportDescriptorInterface {
  //! Constructor
  SocketTransportDescriptor(uint32_t maximumMessageSize,
                            uint32_t maximumInitialPeersRange)
      : TransportDescriptorInterface(maximumMessageSize,
                                     maximumInitialPeersRange),
        sendBufferSize(0), receiveBufferSize(0), TTL(s_defaultTTL) {}
  ...
  operator=(const SocketTransportDescriptor &t) = default;
  ...
  //! Comparison operator
  bool operator==(const SocketTransportDescriptor &t) const {
    return (this->sendBufferSize == t.min_send_buffer_size() &&
            ...
            this->TTL == t.TTL && TransportDescriptorInterface::operator==(t));
  }
