 ...
 *
 * - \c keep_alive_timeout_ms: time since sending the last keep alive request to
 * consider a connection as broken (in ms).
 *
 * - \c max_logical_port: maximum number of logical ports to try during RTCP
 * negotiation.
 *
 * - \c logical_port_range: maximum number of logical ports per request to try
 * during RTCP negotiation.
 *
 * - \c logical_port_increment: increment between logical ports to try during
 * RTCP negotiation.
 *
 ...
 *
 * - \c tcp_negotiation_timeout: time to wait for logical port negotiation (in
 * ms).
 *
...
struct TCPTransportDescriptor : public SocketTransportDescriptor {
   ...
   * TLS Configuration
   *
   * - \c password: password of the \c private_key_file or \c
  ...
  struct TLSConfig {
     ...
     * Several options can be combined in the same TransportDescriptor using the
     * add_option() member function.
     *
     ...
     * Several verification options can be combined in the same
     * TransportDescriptor using the add_verify_mode() member function.
     *
     * - VERIFY_NONE: perform no verification.
     *
     * - VERIFY_PEER: perform verification of the peer.
     *
     * - VERIFY_FAIL_IF_NO_PEER_CERT: fail verification if the peer has no
     * certificate. Ignored unless VERIFY_PEER is also set.
     *
    ...
    //! Maximum allowed depth for verifying intermediate certificates. Do not
    //! override
    ...
    void add_verify_mode(const TLSVerifyMode verify) { verify_mode |= verify; }
    ...
    void add_option(const TLSOptions option) { options |= option; }
    ...
    bool operator==(const TLSConfig &t) const {
      return (this->password == t.password && this->options == t.options &&
  ...
  //! Time since sending the last keep alive request to consider a connection as
  //! broken (ms)
  ...
  /**
   * Time to wait for logical port negotiation (ms). If a logical port is under
   * negotiation, it waits for the negotiation to finish up to this timeout
   * before trying to send a message to that port. Zero value means no waiting
   * (default).
   */
  ...
  FASTDDS_TODO_BEFORE(
      3, 0, "Eliminate wait_for_tcp_negotiation, variable not in use.")
  ...
  //! Copy assignment
  RTPS_DllAPI TCPTransportDescriptor &
  operator=(const TCPTransportDescriptor &t);
