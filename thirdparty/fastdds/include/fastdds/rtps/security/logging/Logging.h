...
public:
   ...
   */
  bool set_log_options(const LogOptions &log_options,
   ...
   * @return
   */
  bool get_log_options(LogOptions &log_options,
   ...
   * @param exception
   */
  void log(const LoggingLevel log_level, const std::string &message,
           const std::string &category, SecurityException &exception) const;
...
protected:
   ...
   * @return
   */
  virtual bool enable_logging_impl(SecurityException & /*exception*/) {
    return true;
  }
   ...
   * @return
   */
  virtual bool convert(const LoggingLevel log_level, const std::string &message,
                       const std::string &category,
                       BuiltinLoggingType &builtin_msg,
                       SecurityException &exception) const;

  template <typename Stream>
  bool compose_header(Stream &header, const BuiltinLoggingType &builtin_msg,
                      SecurityException &exception) const;
...
#define __FASTRTPS_SECURITY_LOGGING(LEVEL, CLASS, MESSAGE, EXCEPTION)          \
  do {                                                                         \
    ...
    if (logger) {                                                              \
      logger->log(LEVEL, MESSAGE, std::string(CLASS ",") + __func__,           \
                  EXCEPTION);                                                  \
    } else {                                                                   \
      switch (LEVEL) {                                                         \
...
#define SECURITY_LOGGING(...)                                                  \
  __FASTRTPS_EXPAND(__FASTRTPS_MACRO_SELECTOR(                                 \
      __VA_ARGS__, __FASTRTPS_SECURITY_LOGGING,                                \
      __FASTRTPS_SECURITY_LOGGING_EX, _UNUSED)(__VA_ARGS__))

#define EMERGENCY_SECURITY_LOGGING(...)                                        \
  SECURITY_LOGGING(LoggingLevel::EMERGENCY_LEVEL, __VA_ARGS__)
#define ALERT_SECURITY_LOGGING(...)                                            \
  SECURITY_LOGGING(LoggingLevel::ALERT_LEVEL, __VA_ARGS__)
#define CRITICAL_SECURITY_LOGGING(...)                                         \
  SECURITY_LOGGING(LoggingLevel::CRITICAL_LEVEL, __VA_ARGS__)
#define ERROR_SECURITY_LOGGING(...)                                            \
  SECURITY_LOGGING(LoggingLevel::ERROR_LEVEL, __VA_ARGS__)
#define WARNING_SECURITY_LOGGING(...)                                          \
  SECURITY_LOGGING(LoggingLevel::WARNING_LEVEL, __VA_ARGS__)
#define NOTICE_SECURITY_LOGGING(...)                                           \
  SECURITY_LOGGING(LoggingLevel::NOTICE_LEVEL, __VA_ARGS__)
#define INFORMATIONAL_SECURITY_LOGGING(...)                                    \
  SECURITY_LOGGING(LoggingLevel::INFORMATIONAL_LEVEL, __VA_ARGS__)
#define DEBUG_SECURITY_LOGGING(...)                                            \
  SECURITY_LOGGING(LoggingLevel::DEBUG_LEVEL, __VA_ARGS__)
