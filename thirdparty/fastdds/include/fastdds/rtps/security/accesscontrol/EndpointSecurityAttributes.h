...
#define PLUGIN_ENDPOINT_SECURITY_ATTRIBUTES_FLAG_IS_SUBMESSAGE_ENCRYPTED       \
  (0x00000001UL << 0)
#define PLUGIN_ENDPOINT_SECURITY_ATTRIBUTES_FLAG_IS_PAYLOAD_ENCRYPTED          \
  (0x00000001UL << 1)
#define PLUGIN_ENDPOINT_SECURITY_ATTRIBUTES_FLAG_IS_SUBMESSAGE_ORIGIN_AUTHENTICATED \
  (0x00000001UL << 2)
#define PLUGIN_ENDPOINT_SECURITY_ATTRIBUTES_FLAG_IS_VALID (0x00000001UL << 31)

#define PLUGIN_ENDPOINT_SECURITY_ATTRIBUTES_MASK_DEFAULT                       \
...
struct PluginEndpointSecurityAttributes {
  PluginEndpointSecurityAttributes()
      : is_submessage_encrypted(false),
        is_submessage_origin_authenticated(false), is_payload_encrypted(false) {
  }
  ...
  inline PluginEndpointSecurityAttributesMask mask() const {
    PluginEndpointSecurityAttributesMask rv =
        PLUGIN_ENDPOINT_SECURITY_ATTRIBUTES_FLAG_IS_VALID;
...
struct EndpointSecurityAttributes {
  EndpointSecurityAttributes()
      : is_read_protected(false), is_write_protected(false),
        ...
        is_key_protected(false), plugin_endpoint_attributes(0UL) {}

  explicit EndpointSecurityAttributes(const EndpointSecurityAttributesMask mask)
      : is_read_protected(
            (mask & ENDPOINT_SECURITY_ATTRIBUTES_FLAG_IS_READ_PROTECTED) != 0),
        is_write_protected(
            (mask & ENDPOINT_SECURITY_ATTRIBUTES_FLAG_IS_WRITE_PROTECTED) != 0),
        is_discovery_protected(
            (mask & ENDPOINT_SECURITY_ATTRIBUTES_FLAG_IS_DISCOVERY_PROTECTED) !=
            0),
        is_liveliness_protected(
            (mask &
             ENDPOINT_SECURITY_ATTRIBUTES_FLAG_IS_LIVELINESS_PROTECTED) != 0),
        is_submessage_protected(
            (mask &
             ENDPOINT_SECURITY_ATTRIBUTES_FLAG_IS_SUBMESSAGE_PROTECTED) != 0),
        is_payload_protected(
            (mask & ENDPOINT_SECURITY_ATTRIBUTES_FLAG_IS_PAYLOAD_PROTECTED) !=
            0),
        is_key_protected(
            (mask & ENDPOINT_SECURITY_ATTRIBUTES_FLAG_IS_KEY_PROTECTED) != 0),
  ...
  inline EndpointSecurityAttributesMask mask() const {
    EndpointSecurityAttributesMask rv =
  ...
  inline bool
  match(const EndpointSecurityAttributesMask remoteMask,
