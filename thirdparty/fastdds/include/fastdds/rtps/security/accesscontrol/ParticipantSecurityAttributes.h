...
#define PLUGIN_PARTICIPANT_SECURITY_ATTRIBUTES_FLAG_IS_RTPS_ENCRYPTED          \
  (0x00000001UL << 0)
#define PLUGIN_PARTICIPANT_SECURITY_ATTRIBUTES_FLAG_BUILTIN_IS_DISCOVERY_ENCRYPTED \
  (0x00000001UL << 1)
#define PLUGIN_PARTICIPANT_SECURITY_ATTRIBUTES_FLAG_IS_LIVELINESS_ENCRYPTED    \
  (0x00000001UL << 2)
#define PLUGIN_PARTICIPANT_SECURITY_ATTRIBUTES_FLAG_IS_RTPS_ORIGIN_AUTHENTICATED \
  (0x00000001UL << 3)
#define PLUGIN_PARTICIPANT_SECURITY_ATTRIBUTES_FLAG_IS_DISCOVERY_ORIGIN_AUTHENTICATED \
  (0x00000001UL << 4)
#define PLUGIN_PARTICIPANT_SECURITY_ATTRIBUTES_FLAG_IS_LIVELINESS_ORIGIN_AUTHENTICATED \
  (0x00000001UL << 5)
#define PLUGIN_PARTICIPANT_SECURITY_ATTRIBUTES_FLAG_IS_VALID                   \
  (0x00000001UL << 31)

struct PluginParticipantSecurityAttributes {
  PluginParticipantSecurityAttributes()
      : is_rtps_encrypted(false), is_discovery_encrypted(false),
        is_liveliness_encrypted(false), is_rtps_origin_authenticated(false),
  ...
  inline PluginParticipantSecurityAttributesMask mask() const {
    PluginParticipantSecurityAttributesMask rv =
...
#define PARTICIPANT_SECURITY_ATTRIBUTES_FLAG_IS_RTPS_PROTECTED                 \
  (0x00000001UL << 0)
#define PARTICIPANT_SECURITY_ATTRIBUTES_FLAG_IS_DISCOVERY_PROTECTED            \
  (0x00000001UL << 1)
#define PARTICIPANT_SECURITY_ATTRIBUTES_FLAG_IS_LIVELINESS_PROTECTED           \
  (0x00000001UL << 2)
...
struct ParticipantSecurityAttributes {
  ParticipantSecurityAttributes()
      : allow_unauthenticated_participants(false), is_access_protected(true),
        is_rtps_protected(false), is_discovery_protected(false),
        is_liveliness_protected(false), plugin_participant_attributes(0UL) {}

  explicit ParticipantSecurityAttributes(
      const ParticipantSecurityAttributesMask mask)
      : allow_unauthenticated_participants(false), is_access_protected(true),
        is_rtps_protected(
            (mask & PARTICIPANT_SECURITY_ATTRIBUTES_FLAG_IS_RTPS_PROTECTED) !=
            0),
        is_discovery_protected(
            (mask &
             PARTICIPANT_SECURITY_ATTRIBUTES_FLAG_IS_DISCOVERY_PROTECTED) != 0),
  ...
  inline ParticipantSecurityAttributesMask mask() const {
    ParticipantSecurityAttributesMask rv =
