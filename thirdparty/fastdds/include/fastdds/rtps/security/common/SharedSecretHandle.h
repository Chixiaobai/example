...
public:
  ...
  public:
    ...
    BinaryData(const BinaryData &data)
        : name_(data.name_), value_(data.value_) {}

    BinaryData(BinaryData &&data)
        : name_(std::move(data.name_)), value_(std::move(data.value_)) {}

    BinaryData(const std::string &name, const std::vector<uint8_t> &value)
        : name_(name), value_(value) {}

    BinaryData(std::string &&name, std::vector<uint8_t> &&value)
...
public:
  ...
  find_data_value(const SecretHandle &sharedsecret, const std::string &name);
};
