...
public:
   ...
   * @param extra_inline_qos (out) Contains additional parameters to be added to
   * the inlineQos of the submessage
   * @param payload Plain input buffer
   * @param sending_datawriter_crypto Returned by a prior call to
   * register_local_datawriter
   * @param exception (out) Security exception
   * @return TRUE if successful
   */
  virtual bool
  encode_serialized_payload(SerializedPayload_t &encoded_payload,
   ...
   * @param sending_datawriter_crypto Crypto of the datawriter that sends the
   * message
   * @param receiving_datareader_crypto_list Crypto of the datareaders the
   * message is aimed at
   * @param exception (out) Security exception
   ...
   * @param receiving_datawriter_crypto_list List with Crypto of the intended
   * datawriter recipients
   * @param exception (out) Security exception
   * @return TRUE if successful
   */
   ...
   * @param sending_crypto Crypto of the Participant where the message
   * originates from
   * @param receiving_crypto_list Crypto of the Partipants the message is
   * intended towards
   * @param exception (out) Security expcetion
   * @return TRUE if successful
   */
  virtual bool
  encode_rtps_message(CDRMessage_t &encoded_rtps_message,
  ...
  /**
   * Reverses the transformation performed by encode_rtps_message. Decrypts the
   * contents and veryfies MACs or digital signatures.
   ...
   */
  virtual bool
  decode_rtps_message(CDRMessage_t &plain_buffer,
  ...
  /**
   * Determines whether the secure submessage comes from a datawriter or a data
   * reader and extracts the required CryptoHandle to decode it.
   * @param datawriter_crypto (out) Crypto of the sending datawriter, if
   * applicable
   * @param datareader_crypto (out) Crypto of the sending datareader, if
   * applicable
   * @param secure_submessage_category (out) Specifies whether the message comes
   * from a datawriter or from a datareader
  ...
  /**
   * Called after prprocess_secure_submessage when the submessage category is
   ...
   * @param sending_datawriter_crypto Crypto of the datawriter that sent the
   * message
   * @param exception (out) Security exception
   * @return TRUE if successful
   */
  ...
  /**
   * Called after preprocess_secure_submessage when the submessage category is
   * DATAREADER_SUBMESSAGE
   * @param plain_rtps_submessage (out) Result of the decryption
   * @param encoded_rtps_submessage Encoded message
   * @param receiving_datawriter_crypto Crypto of the target datawriter
   * @param sending_datareader_crypto Crypto of the datareader that sent the
   * message
   * @param exception (out) Security exception
   * @return TRUE if successful
   */
   ...
   * @param inline_qos Coming from the data message that carries the target
   * payload
   * @param receiving_datareader_crypto Crypto of the target datareader
   * @param sending_datawriter_crypto Crypto of the datawriter that sent the
   * message
   ...
   */
  virtual bool
  decode_serialized_payload(SerializedPayload_t &plain_payload,
