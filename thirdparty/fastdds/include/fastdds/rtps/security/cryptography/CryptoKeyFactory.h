...
public:
  ...
  /**
   * Register a local, already authenticated Participant with the Cryptographic
   * Plugin. Creates Crypto material needed to encrypt messages directed to
   * other Participants
   * @param participant_identity Made by a prior call to validate_local_identity
   * @param participant_permissions Made by a prior call to
   * validate_local_permissions
   * @param participant_properties Combination of PropertyQoSPolicy and contents
   * of AccessControl
  ...
  /**
   * Register a remote, already authenticated Participant with the Cryptographic
   * Plugin. Creates key material to decrypt messages coming from and aimed at
   * it.
   * @param local_participant_crypto_handle Returned by a prior call to
   * register_local_participant
   * @param remote_participant_identity Returned by a prior call to
   * validate_remote_identity
   * @param remote_participant_permissions Returned by a prior call to
   * validate_remote_permissions
   * @param shared_secret Returned by a prior call to get_shared_secret (Auth
   * Handshake)
   * @param exception (out) Security exception
   * @return ParticipantCryptoHandle with generated key material
   */
  virtual std::shared_ptr<ParticipantCryptoHandle>
  register_matched_remote_participant(
      ...
      const SecretHandle &shared_secret, SecurityException &exception) = 0;
   ...
   * @param participant_crypto returned by a prior call to
   * register_local_participant
   * @param datawriter_prop Combination of PropertyWosPolicy and contents of
   * AccessControl
  ...
  /**
   * Register a remote DataReader that has been granted permission to match with
   * the local DataWriter. Creates cryptographic material to encript/decrypt
   * messages from and towards that DataReader.
   * @param local_datawriter_crypto_handle Returned by a prior call to
   * register_local_datawriter
   * @param remote_participant_crypto Returned by a prior call to
   * register_matched_remote_participant.
   * @param shared_secret Obtained as a result of the Authentication Handshake.
   * @param relay_only If FALSE it generates material for both a submessage and
   * serialized payload. Submessages only if TRUE.
   * @param exception (out) Security exception.
  ...
  virtual DatareaderCryptoHandle *register_matched_remote_datareader(
      ...
      const SecretHandle &shared_secret, const bool relay_only,
      SecurityException &exception) = 0;

  /**
   * Register a local DataReader (belonging to an authenticated and authorized
   * Participant) with the Cryptographic Plugin. Creates crypto material to
   * encode messages when the encryption is independent of the targeted
   * DataWriter
   * @param participant_crypto Returned by a prior call to
   * register_local_participant
   * @param datareader_properties Combination of PropertyQosPolicy and the
   * contents of AccessControl
  ...
  /**
   * Register a remote DataWriter that has been granted permission to match with
   * a local DataReader. Creates crypto material to decrypt messages coming from
   * and encode messages going towards that datareader
  ...
  /**
   * Releases resources associated with a Participant. The Crypto Handle becomes
   * unusable after this
   * @param participant_crypto_handle Belonging to the Participant that awaits
   * termination
   * @param exception (out) Security exception
  ...
  /**
   * Releases resources associated with a DataWriter. The Crypto Handle may
   * become unusable after this
   * @param datawriter_crypto_handle Belonging to the DataWriter that awaits
   * termination
   * @param exception (out) Security exception
   * @return TRUE is successful
   */
   ...
   * @return TRUE is successful
   */
  bool unregister_datawriter(DatawriterCryptoHandle *datawriter_crypto_handle,
  ...
  /**
   * Releases resources associated with a DataReader. The Crypto Handle may
   * become unusable after this
   * @param datareader_crypto_handle Belonging to the DataReader that awaits
   * termination
   * @param exception (out) Security exception
   * @return TRUE is successful
   */
  ...
  /**
   * Convenient override for raw pointers arguments.
   * @param datareader_crypto_handle Belonging to the DataWriter that awaits
   * termination
   * @param exception (out) Security exception
   * @return TRUE is successful
   */
  bool unregister_datareader(DatareaderCryptoHandle *datareader_crypto_handle,
                             SecurityException &exception) {
