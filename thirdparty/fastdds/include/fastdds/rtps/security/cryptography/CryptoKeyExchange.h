...
public:
  ...
  /**
   * Creates Crypto Tokens containing the info to decrypt text encoded by the
   * local Participant. To be sent to the remote participant.
   * @param local_participant_crypto_tokens (out) Returned CryptoTokenSeq.
   * @param local_participant_crypto CryptoHandle returned by a previous call to
   * register_local_participant.
   * @param remote_participant_crypto CryptoHangle returned by a previous call
   * to register_remote_participant.
  ...
  /**
   * Configures the Cryptographic Plugin with the material needed to interpret
   * messages coming from the remote crypto.
   * @param local_participant_crypto CryptoHandle returned by a previous call to
   * register_local_participant.
   * @param remote_participant_crypto CryptoHandle returned by a previous call
   * to register_matched_remote_participant.
   * @param remote_participant_tokens CryptoToken sequence received from the
   * remote Participant
   * @param exception (out) Security exception
  ...
  /**
   * Creates CryptoTokens containing the info to decrypt text encoded by the
   * local DataWriter.
   * @param local_datawriter_crypto_tokens (out) Returned CryptoSeq
   * @param local_datawriter_crypto CryptoHandle returned by a previous call to
   * register_local_datawriter.
   * @param remote_datareader_crypto CryptoHandle returned by a previous call to
  ...
  /**
   * Configures the Cryptographic Plugin with the material needed to interpret
   * messages coming from the remote DataReader.
  ...
  /**
   * Configures the Cryptographic Plugin with the material needed to interpret
   * messages coming from the remote DataWriter.
   * @param local_datareader_crypto
   * @param remote_datawriter_crypto
   ...
   * @return TRUE if successful
   */
  virtual bool return_crypto_tokens(const CryptoTokenSeq &crypto_tokens,
                                    SecurityException &exception) = 0;
};
