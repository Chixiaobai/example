...
/**
 * Class RTPSWriter, manages the sending of data to the readers. Is always
 * associated with a HistoryCache.
...
protected:
  RTPSWriter(RTPSParticipantImpl *impl, const GUID_t &guid,
...
public:
  ...
  template <typename T>
  CacheChange_t *
  new_change(T &data, ChangeKind_t changeKind,
  ...
  RTPS_DllAPI CacheChange_t *
  new_change(const std::function<uint32_t()> &dataCdrSerializedSize,
             ChangeKind_t changeKind,
             InstanceHandle_t handle = c_InstanceHandle_Unknown);

  RTPS_DllAPI CacheChange_t *
  new_change(ChangeKind_t changeKind,
             InstanceHandle_t handle = c_InstanceHandle_Unknown);
   ...
   *     @li @c change points to a cache change obtained from a call to @c
   * this->new_change
   *
  ...
  matched_reader_is_matched(const GUID_t &reader_guid) = 0;

  /**
   * @brief Set a content filter to perform content filtering on this writer.
   *
   * This method sets a content filter that will be used to check whether a
   * cache change is relevant for a reader or not.
   *
   * @param filter  The content filter to use on this writer. May be @c nullptr
   * to remove the content filter (i.e. treat all samples as relevant).
   */
  RTPS_DllAPI virtual void
  reader_data_filter(fastdds::rtps::IReaderDataFilter *filter) = 0;

  /**
   * @brief Get the content filter used to perform content filtering on this
   * writer.
   *
   * @return The content filter used on this writer.
   */
  RTPS_DllAPI virtual const fastdds::rtps::IReaderDataFilter *
  reader_data_filter() const = 0;

  /**
   * Check if a specific change has been acknowledged by all Readers.
   * Is only useful in reliable Writer. In BE Writers returns false when pending
   * to be sent.
   * @return True if acknowledged by all.
   */
  RTPS_DllAPI virtual bool
  is_acked_by_all(const CacheChange_t * /*a_change*/) const {
    return false;
  }
   ...
   */
  RTPS_DllAPI virtual bool wait_for_all_acked(const Duration_t & /*max_wait*/) {
    return true;
  }
  ...
  RTPS_DllAPI inline bool set_listener(WriterListener *listener) {
   ...
   * @param[in] writer_guid      GUID of the writer the submessage is directed
   * to.
   ...
   * @param[out] result          true if the writer could process the
   * submessage. Only valid when returned value is true.
   * @return true when the submessage was destinated to this writer, false
   * otherwise.
   */
  virtual bool process_acknack(const GUID_t &writer_guid,
                               const GUID_t &reader_guid, uint32_t ack_count,
                               const SequenceNumberSet_t &sn_set,
                               bool final_flag, bool &result) {
    (void)reader_guid;
    (void)ack_count;
    (void)sn_set;
    (void)final_flag;
   ...
   * otherwise.
   */
  virtual bool process_nack_frag(const GUID_t &writer_guid,
                                 const GUID_t &reader_guid, uint32_t ack_count,
   ...
   *
   * @param cache_change Pointer to the CacheChange_t that represents the sample
   * which can be sent.
   * @param group RTPSMessageGroup reference uses for generating the RTPS
   * message.
   * @param locator_selector RTPSMessageSenderInterface reference uses for
   * selecting locators. The reference has to be a member of this RTPSWriter
   * object.
   * @param max_blocking_time Future timepoint where blocking send should end.
   * @return Return code.
   * @note Must be non-thread safe.
   */
  virtual DeliveryRetCode
  deliver_sample_nts(CacheChange_t *cache_change, RTPSMessageGroup &group,
   ...
   * selecting locators. The reference has to be a member of this RTPSWriter
   * object.
   * @param max_blocking_time_point Future timepoint where blocking send should
   * end.
   */
  virtual bool send_nts(
      CDRMessage_t *message, const LocatorSelectorSender &locator_selector,
      std::chrono::steady_clock::time_point &max_blocking_time_point) const;

protected:
  //! Is the data sent directly or announced by HB and THEN sent to the ones who
  //! ask for it?.
  bool m_pushMode = true;

  //! Flow controller.
  fastdds::rtps::FlowController *flow_controller_;
  //! Maximum number of bytes allowed for an RTPS datagram generated by this
  //! writer.
  ...
  void add_guid(LocatorSelectorSender &locator_selector,
  ...
  /**
   * Indicate the writer that a change has been removed by the history due to
   * some HistoryQos requirement.
  ...
  bool send_data_or_fragments(RTPSMessageGroup &group, CacheChange_t *change,
                              bool inline_qos, Functor sent_fun) {
    bool sent_ok = true;
    ...
    if (n_fragments > 0) {
      ...
      if (sent_ok) {
        ...
        logError(RTPS_WRITER,
                 "Error sending change " << change->sequenceNumber);
      }
    }
  ...
  void add_statistics_sent_submessage(CacheChange_t *change,
                                      size_t num_locators);

  void deinit();

private:
  RecursiveTimedMutex &get_mutex() { return mp_mutex; }

  RTPSWriter &operator=(const RTPSWriter &) = delete;

  void init(const std::shared_ptr<IPayloadPool> &payload_pool,
            const std::shared_ptr<IChangePool> &change_pool,
            const WriterAttributes &att);
