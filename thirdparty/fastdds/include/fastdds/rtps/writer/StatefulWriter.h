...
/**
 * Class StatefulWriter, specialization of RTPSWriter that maintains information
 * of each matched Reader.
...
protected:
  //! Constructor
  StatefulWriter(RTPSParticipantImpl *impl, const GUID_t &guid,
  ...
  virtual void print_inconsistent_acknack(
      const GUID_t &writer_guid, const GUID_t &reader_guid,
...
private:
  ...
  //! A timed event to mark samples as acknowledget (used only if disable
  //! positive ACKs QoS is enabled)
  ...
  using ReaderProxyConstIterator =
      ResourceLimitedVector<ReaderProxy *>::const_iterator;
...
public:
  ...
  void unsent_change_added_to_history(
      CacheChange_t *p, const std::chrono::time_point<std::chrono::steady_clock>
                            &max_blocking_time) override;
  ...
  bool intraprocess_gap(ReaderProxy *reader_proxy,
  ...
  bool intraprocess_heartbeat(ReaderProxy *reader_proxy,
                              bool liveliness = false);

  //! Increment the HB count.
  inline void incrementHBCount() { on_heartbeat(++m_heartbeatCount); }
  ...
  Function for_each_reader_proxy(Function f) const {
    // we cannot directly pass iterators neither const_iterators to
    // matched_readers_ because then the functor would be able to modify
    // ReaderProxy elements
  ...
  void send_heartbeat_to_nts(ReaderProxy &remoteReaderProxy,
                             bool liveliness = false, bool force = false);
   ...
   * otherwise.
   */
  bool process_acknack(const GUID_t &writer_guid, const GUID_t &reader_guid,
                       uint32_t ack_count, const SequenceNumberSet_t &sn_set,
                       bool final_flag, bool &result) override;
   ...
   */
  void reader_data_filter(fastdds::rtps::IReaderDataFilter *filter) final;
...
private:
  ...
  void update_reader_info(LocatorSelectorSender &locator_selector,
                          bool create_sender_resources);

  void select_all_readers_nts(RTPSMessageGroup &group,
                              LocatorSelectorSender &locator_selector);
  ...
  void send_heartbeat_nts_(size_t number_of_readers,
  ...
  void deliver_sample_to_datasharing(CacheChange_t *change);
  ...
  void prepare_datasharing_delivery(CacheChange_t *change);

  /**
   * Check the StatefulWriter's sequence numbers and add the required GAP
   * messages to the provided message group.
   *
   * @param group     Reference to the Message Group to which the GAP messages
   * are to be added.
   */
  ...
  //! Last acknowledged cache change (only used if using disable positive ACKs
  //! QoS)
