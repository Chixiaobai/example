...
/*!
 * Class used by writers to inform a RTPSMessageGroup object which remote
 * participants will be addressees of next RTPS submessages.
 */
...
public:
  LocatorSelectorSender(
      ...
      : locator_selector(matched_readers_allocation),
        ...
        all_remote_participants(matched_readers_allocation), writer_(writer) {}
   ...
   *
   * @return If only one remote participant is an addressee, return its
   * GUIDPrefix_t. c_GuidPrefix_Unknown otherwise.
   */
  GuidPrefix_t destination_guid_prefix() const override {
    return all_remote_participants.size() == 1 ? all_remote_participants.at(0)
   ...
   * Lock the object.
   *
   * This kind of object needs to be locked because could be used outside the
   * writer's mutex.
   */
