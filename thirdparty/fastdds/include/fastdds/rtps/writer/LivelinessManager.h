...
using LivelinessCallback = std::function<void(
    const GUID_t &, const LivelinessQosPolicyKind &, const Duration_t &,
    int32_t alive_change, int32_t not_alive_change)>;

/**
 * @brief A class managing the liveliness of a set of writers. Writers are
 * represented by their LivelinessData
 * @details Uses a shared timed event and informs outside classes on liveliness
 * changes
...
public:
  /**
   * @brief Constructor
   * @param callback A callback that will be invoked when a writer changes its
   * liveliness status
   * @param service ResourceEvent object that will operate with the events.
   * @param manage_automatic True to manage writers with automatic liveliness,
   * false otherwise
   */
  LivelinessManager(const LivelinessCallback &callback, ResourceEvent &service,
   ...
   */
  bool add_writer(GUID_t guid, LivelinessQosPolicyKind kind,
   ...
   */
  bool remove_writer(GUID_t guid, fastdds::dds::LivelinessQosPolicyKind kind,
                     Duration_t lease_duration,
   ...
   */
  bool assert_liveliness(GUID_t guid, LivelinessQosPolicyKind kind,
                         Duration_t lease_duration);

  /**
   * @brief Asserts liveliness of writers with given liveliness kind and
   * GuidPrefix
   * @param kind Liveliness kind
...
private:
  /**
   * @brief A method responsible for invoking the callback when liveliness is
   * asserted
  ...
  /**
   * @brief A method to calculate the time when the next writer is going to lose
   * liveliness
   * @details This method is public for testing purposes but it should not be
   * used from outside this class
   * @pre std::mutex_ should not be taken on calling this method to avoid
   * deadlock.
  ...
  //! A callback to inform outside classes that a writer changed its liveliness
  //! status
  const LivelinessCallback callback_;

  //! A boolean indicating whether we are managing writers with automatic
  //! liveliness
  ...
  //! A timed callback expiring when a writer (the timer owner) loses its
  //! liveliness
  TimedEvent timer_;
};
