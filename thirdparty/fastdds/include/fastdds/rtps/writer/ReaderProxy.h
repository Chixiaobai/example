...
/**
 * ReaderProxy class that helps to keep the state of a specific Reader with
 * respect to the RTPSWriter.
...
public:
   ...
   * @param loc_alloc Maximum number of remote locators to keep in the
   * ReaderProxy.
   * @param writer Pointer to the StatefulWriter creating the reader proxy.
   */
  ReaderProxy(const WriterTimes &times,
              const RemoteLocatorsAllocationAttributes &loc_alloc,
              StatefulWriter *writer);

  /**
   * Activate this proxy associating it to a remote reader.
   * @param reader_attributes ReaderProxyData of the reader for which to keep
   * state.
   * @param is_datasharing whether the reader is datasharing compatible with the
   * writer or not.
   */
   ...
   * @param reader_attributes ReaderProxyData with updated information of the
   * reader.
   ...
   * @param restart_nack_supression Whether nack-supression event should be
   * restarted.
   */
  void add_change(const ChangeForReader_t &change, bool is_relevant,
   ...
   * @return true when the change is irrelevant or has been already
   ...
   * @param[out] gap_seq Return, when it is its first delivery (should be
   * relevant seq_num), the sequence number of the first sequence of the gap
   * [first, seq_num). Otherwise return SequenceNumber_t::unknown().
   * @param[in]  min_seq Minimum sequence number managed by the History. It
   * could be SequenceNumber_t::unknown() if history is empty.
   * @param[out] need_reactivate_periodic_heartbeat Indicates if the heartbeat
   * period event has to be restarted.
   *
   * @return true if the change is marked to be sent. False otherwise.
   */
  bool change_is_unsent(const SequenceNumber_t &seq_num,
   ...
   * @param seq_num Sequence number of the first change not to be marked as
   * acknowledged.
   */
   ...
   * @param gap_builder RTPSGapBuilder reference uses for adding  each requested
   * change that is irrelevant for the requester.
   * @param[in] min_seq_in_history Minimum SequenceNumber_t in the writer's
   * history. If writer's history is empty, SequenceNumber_t::unknown() is
   * expected.
   * @return true if at least one change has been marked as REQUESTED, false
   * otherwise.
   */
   ...
   * @param func functor called, if the requester is a local reader, for each
   * changes moved to UNSENT status.
   ...
   * @param restart_nack_supression Whether nack supression event should be
   * restarted or not.
   * @param delivered true if change was able to be delivered to its addressees.
   * false otherwise.
   */
  void from_unsent_to_status(const SequenceNumber_t &seq_num,
   ...
   * @param[out] was_last_fragment Indicates if the fragment was the last one
   * pending.
   * @return true when the change was found, false otherwise.
   */
  bool mark_fragment_as_sent_for_change(const SequenceNumber_t &seq_num,
   ...
   */
  inline const GUID_t &guid() const { return locator_info_.remote_guid(); }

  /**
   * Get the durability of the reader represented by this proxy.
   * @return the durability of the reader represented by this proxy.
   */
  inline DurabilityKind_t durability_kind() const { return durability_kind_; }

  /**
   * Check if the reader represented by this proxy expexts inline QOS to be
   * received.
   * @return true if the reader represented by this proxy expexts inline QOS to
   * be received.
   */
  inline bool expects_inline_qos() const { return expects_inline_qos_; }

  /**
   * Check if the reader represented by this proxy is reliable.
   * @return true if the reader represented by this proxy is reliable.
   */
  inline bool is_reliable() const { return is_reliable_; }

  inline bool disable_positive_acks() const { return disable_positive_acks_; }

  /**
   * Check if the reader represented by this proxy is remote and reliable.
   * @return true if the reader represented by this proxy is remote and
   * reliable.
   */
  inline bool is_remote_and_reliable() const {
    return !locator_info_.is_local_reader() &&
           !locator_info_.is_datasharing_reader() && is_reliable_;
  }
   ...
   */
  inline bool is_local_reader() { return locator_info_.is_local_reader(); }

  /**
   * Get the local reader on the same process (if any).
   * @return The local reader on the same process.
   */
  inline RTPSReader *local_reader() { return locator_info_.local_reader(); }

  /**
   * Called when an ACKNACK is received to set a new value for the minimum count
   * accepted for following received ACKNACKs.
   *
   ...
   */
  bool process_nack_frag(const GUID_t &reader_guid, uint32_t nack_count,
   ...
   */
  SequenceNumber_t changes_low_mark() const { return changes_low_mark_; }
  ...
  RTPSMessageSenderInterface *message_sender() { return &locator_info_; }
  ...
  void datasharing_notify() { locator_info_.datasharing_notify(); }

  size_t locators_size() const { return locator_info_.locators_size(); }
...
private:
  ...
  //! Timed Event to manage the delay to mark a change as UNACKED after sending
  //! it.
  ...
  using ChangeIterator =
      ResourceLimitedVector<ChangeForReader_t, std::true_type>::iterator;
  using ChangeConstIterator =
      ResourceLimitedVector<ChangeForReader_t, std::true_type>::const_iterator;
  ...
  uint32_t convert_status_on_all_changes(
      ChangeForReaderStatus_t previous, ChangeForReaderStatus_t next,
      const std::function<void(ChangeForReader_t &change)> &func = {});

  /*!
   * @brief Adds requested fragments. These fragments will be sent in next
   * NackResponseDelay.
   * @param[in] seq_num Sequence number to be paired with the requested
   * fragments.
   * @param[in] frag_set set containing the requested fragments to be sent.
   * @return True if there is at least one requested fragment. False in other
   * case.
   */
  bool requested_fragment_set(const SequenceNumber_t &seq_num,
   ...
   * @param exact When false, the first change with a sequence number not less
   * than seq_num will be returned. When true, the change with a sequence number
   * value of seq_num will be returned.
   * @return Iterator pointing to the change, changes_for_reader_.end() if not
   * found.
   */
  ChangeIterator find_change(const SequenceNumber_t &seq_num, bool exact);
