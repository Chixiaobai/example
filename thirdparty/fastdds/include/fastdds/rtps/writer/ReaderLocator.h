...
/**
 * Class ReaderLocator, contains information about a remote reader, without
 * saving its state. It also implements RTPSMessageSenderInterface, so it can be
 * used when separate sending is enabled.
...
public:
   ...
   *
   * @param owner                   Pointer to the RTPSWriter creating this
   * object.
   * @param max_unicast_locators    Maximum number of unicast locators to hold.
   * @param max_multicast_locators  Maximum number of multicast locators to
   * hold.
   */
  ReaderLocator(RTPSWriter *owner, size_t max_unicast_locators,
                size_t max_multicast_locators);

  bool expects_inline_qos() const { return expects_inline_qos_; }

  bool is_local_reader() const { return is_local_reader_; }

  RTPSReader *local_reader();

  void local_reader(RTPSReader *local_reader) { local_reader_ = local_reader; }

  const GUID_t &remote_guid() const { return locator_info_.remote_guid; }

  LocatorSelectorEntry *locator_selector_entry() { return &locator_info_; }
   ...
   * @param expects_inline_qos  Whether remote reader expects to receive inline
   * QoS.
   * @param is_datasharing      Whether remote reader can be reached through
   * datasharing.
   *
   * @return false when this object was already started, true otherwise.
   */
  bool start(const GUID_t &remote_guid,
             const ResourceLimitedVector<Locator_t> &unicast_locators,
             const ResourceLimitedVector<Locator_t> &multicast_locators,
             bool expects_inline_qos, bool is_datasharing = false);
   ...
   */
  bool update(const ResourceLimitedVector<Locator_t> &unicast_locators,
              const ResourceLimitedVector<Locator_t> &multicast_locators,
   ...
   *
   * @return When all the destinations share the same prefix (i.e. belong to the
   * same participant) that prefix is returned. When there are no destinations,
   * or they belong to different participants, c_GuidPrefix_Unknown is returned.
   */
   ...
   *
   * @return a const reference to a vector with the GUID prefix of all
   ...
   * end.
   */
  bool send(CDRMessage_t *message,
  ...
  /**
   * @return The datasharing notifier for this reader or nullptr if the reader
   * is not datasharing.
   */
  IDataSharingNotifier *datasharing_notifier() { return datasharing_notifier_; }
  ...
  /**
   * Performs datasharing notification of changes on the state of a writer to
   * the reader represented by this class.
   */
