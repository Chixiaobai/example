...
/**
 * Enum ChangeForReaderStatus_t, possible states for a CacheChange_t in a
 * ReaderProxy.
...
/**
 * Struct ChangeForReader_t used to represent the state of a specific change
 * with respect to a specific reader, as well as its relevance.
 *  @ingroup COMMON_MODULE
 */
...
public:
  explicit ChangeForReader_t(CacheChange_t *change)
      : status_(UNSENT), seq_num_(change->sequenceNumber), change_(change) {
  ...
  void setStatus(const ChangeForReaderStatus_t status) { status_ = status; }

  ChangeForReaderStatus_t getStatus() const { return status_; }

  const SequenceNumber_t getSequenceNumber() const { return seq_num_; }
  ...
  FragmentNumberSet_t getUnsentFragments() const { return unsent_fragments_; }
  ...
  void markFragmentsAsSent(const FragmentNumber_t &sentFragment) {
    ...
    // We only use the running window mechanism during the first stage, until
    // all fragments have been delivered once, and we consider the whole change
    // as delivered.
    if (!delivered_ && !unsent_fragments_.empty() &&
        (unsent_fragments_.max() < change_->getFragmentCount())) {
  ...
  void markFragmentsAsUnsent(const FragmentNumberSet_t &unsentFragments) {
    // Ignore NACK_FRAG messages during the first stage, until all fragments
    // have been delivered once, and we consider the whole change as delivered.
    if (delivered_) {
      if (unsent_fragments_.empty()) {
  ...
  bool has_been_delivered() const { return delivered_; }

  void set_delivered() { delivered_ = true; }
...
struct ChangeForReaderCmp {
  bool operator()(const ChangeForReader_t &a,
