...
/**
 * Class History, container of the different CacheChanges and the methods to
 * access them.
...
public:
   ...
   */
  FASTRTPS_DEPRECATED(
      "Use new_change on RTPSWriter or reserveCache on RTPSReader")
  RTPS_DllAPI inline bool
  reserve_Cache(CacheChange_t **change,
  ...
  FASTRTPS_DEPRECATED(
      "Use new_change on RTPSWriter or reserveCache on RTPSReader")
  RTPS_DllAPI inline bool reserve_Cache(CacheChange_t **change,
                                        uint32_t dataSize) {
   ...
   */
  FASTRTPS_DEPRECATED(
      "Use release_change on RTPSWriter or releaseCache on RTPSReader")
  ...
  /**
   * Find a specific change in the history using the matches_change method
   * criteria. No Thread Safe
   ...
   */
  RTPS_DllAPI virtual iterator remove_change_nts(const_iterator removal,
                                                 bool release = true);
   ...
   * @return an iterator if a suitable change is found
   */
  RTPS_DllAPI const_iterator find_change(CacheChange_t *ch) {
    std::lock_guard<RecursiveTimedMutex> guard(*mp_mutex);
   ...
   * Derived classes have more info on how to identify univocally a change and
   * should override.
   ...
   */
  RTPS_DllAPI virtual bool matches_change(const CacheChange_t *ch_inner,
   ...
   * @return iterator to the next CacheChange_t or end iterator.
   */
  RTPS_DllAPI iterator remove_change(const_iterator removal,
                                     bool release = true) {
    std::lock_guard<RecursiveTimedMutex> guard(*mp_mutex);
   ...
   */
  RTPS_DllAPI iterator changesBegin() { return m_changes.begin(); }

  RTPS_DllAPI reverse_iterator changesRbegin() { return m_changes.rbegin(); }

  /**
   * Get the end of the changes history iterator.
   * @return Iterator to the end of the vector.
   */
  RTPS_DllAPI iterator changesEnd() { return m_changes.end(); }

  RTPS_DllAPI reverse_iterator changesRend() { return m_changes.rend(); }
  ...
  RTPS_DllAPI bool get_change(const SequenceNumber_t &seq, const GUID_t &guid,
                              CacheChange_t **change) const;

  const_iterator get_change_nts(const SequenceNumber_t &seq, const GUID_t &guid,
...
protected:
  ...
  //! Variable to know if the history is full without needing to block the
  //! History mutex.
  ...
  //! Print the seqNum of the changes in the History (for debuggisi, mng
  //! purposes).
  void print_changes_seqNum2();

  RTPS_DllAPI virtual bool do_reserve_cache(CacheChange_t **change,
                                            uint32_t size) = 0;

  RTPS_DllAPI virtual void do_release_cache(CacheChange_t *ch) = 0;
};
