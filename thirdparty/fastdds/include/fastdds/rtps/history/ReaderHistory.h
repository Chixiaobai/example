...
public:
  ...
  RTPS_DllAPI virtual bool can_change_be_added_nts(
      const GUID_t &writer_guid, uint32_t total_payload_size,
      size_t unknown_missing_changes_up_to, bool &will_never_be_accepted) const;

  /**
   * Virtual method that is called when a new change is received.
   * In this implementation this method just calls add_change. The user can
   * overload this method in case he needs to perform additional checks before
   * adding the change.
  ...
  received_change(CacheChange_t *change, size_t unknown_missing_changes_up_to);
   ...
   * @return
   */
  RTPS_DllAPI bool virtual completed_change(rtps::CacheChange_t *change) {
    (void)change;
    return true;
  }
   ...
   */
  RTPS_DllAPI iterator remove_change_nts(const_iterator removal,
   ...
   */
  RTPS_DllAPI bool matches_change(const CacheChange_t *inner,
  ...
  /**
   * Remove all fragmented changes from certain writer up to certain sequence
   * number.
   * @param seq_num First SequenceNumber_t not to be removed.
   * @param writer_guid GUID of the writer for which changes should be looked
   * for.
   * @return True if successful, even if no changes have been removed.
   */
  ...
  RTPS_DllAPI bool get_min_change_from(CacheChange_t **min_change,
                                       const GUID_t &writerGuid);

  /**
   * Called when a writer is unmatched from the reader holding this history.
   *
   * This method will remove all the changes on the history that came from the
   * writer being unmatched and which have not yet been notified to the user.
   *
   * @param writer_guid        GUID of the writer being unmatched.
   * @param last_notified_seq  Last sequence number from the specified writer
   * that was notified to the user.
   */
  RTPS_DllAPI virtual void
  writer_unmatched(const GUID_t &writer_guid,
                   const SequenceNumber_t &last_notified_seq);

protected:
  RTPS_DllAPI bool do_reserve_cache(CacheChange_t **change,
                                    uint32_t size) override;

  RTPS_DllAPI void do_release_cache(CacheChange_t *ch) override;

  template <typename Pred> inline void remove_changes_with_pred(Pred pred) {
