...
public:
  ...
  remove_change_and_reuse(const SequenceNumber_t &sequence_number);
...
protected:
   ...
   *                                Its @c sequenceNumber and sourceTimestamp
   * will be filled by this method. Its @c wparams will be filled from parameter
   * @c wparams.
   * @param [in,out] wparams        On input, it holds the WriteParams to be
   * copied into @c a_change. On output, will be filled with the sample identity
   * assigned to @c a_change.
   * @param [in] max_blocking_time  Maximum time point the writer is allowed to
   * be blocked till the change is put into the wire or the sending queue.
   *
   * @return whether @c a_change could be added to the history.
   */
  bool add_change_(
      CacheChange_t *a_change, WriteParams &wparams,
   ...
   * copied into @c a_change. On output, will be filled with the sample identity
   * assigned to @c a_change.
   * @param [in] pre_commit         Functor called after @c a_change has been
   * added to the history, and its information has been filled, but before the
   * writer is notified of the insertion.
   * @param [in] max_blocking_time  Maximum time point the writer is allowed to
   * be blocked till the change is put into the wire or the sending queue.
   *
   * @return whether @c a_change could be added to the history.
  ...
  bool add_change_with_commit_hook(
      CacheChange_t *a_change, WriteParams &wparams, PreCommitHook pre_commit,
      std::chrono::time_point<std::chrono::steady_clock> max_blocking_time) {
    if (mp_writer == nullptr || mp_mutex == nullptr) {
      logError(RTPS_WRITER_HISTORY, "You need to create a Writer with this "
                                    "History before adding any changes");
      return false;
    }
...
private:
   ...
   *
   * @param [in,out] a_change  The change to be added.
   *                           Its @c sequenceNumber and sourceTimestamp will be
   * filled by this method. Its @c wparams will be filled from parameter @c
   * wparams.
   * @param [in,out] wparams   On input, it holds the WriteParams to be copied
   * into @c a_change. On output, will be filled with the sample identity
   * assigned to @c a_change.
   *
   * @return whether @c a_change could be added to the history.
   */
  bool prepare_and_add_change(CacheChange_t *a_change, WriteParams &wparams);

  /**
   * Notifies the RTPS writer associated with this history that a change has
   * been added. Depending on the publish mode, it will be directly sent to the
   * wire, or put into a sending queue.
   *
   * @param [in] a_change           The change that has just been added to the
   * history.
   * @param [in] max_blocking_time  Maximum time point the writer is allowed to
   * be blocked till the change is put into the wire or the sending queue.
   */
  void notify_writer(CacheChange_t *a_change,
                     const std::chrono::time_point<std::chrono::steady_clock>
                         &max_blocking_time);

  void set_fragments(CacheChange_t *change);
};
