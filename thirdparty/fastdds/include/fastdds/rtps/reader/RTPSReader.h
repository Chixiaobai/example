...
protected:
  RTPSReader(RTPSParticipantImpl *pimpl, const GUID_t &guid,
             const ReaderAttributes &att, ReaderHistory *hist,
...
public:
   ...
   * @param removed_by_lease Whether the writer is being unmatched due to a
   * participant drop.
   * @return True if correctly removed.
   */
  RTPS_DllAPI virtual bool
  matched_writer_remove(const GUID_t &writer_guid,
   ...
   */
  RTPS_DllAPI virtual bool
  matched_writer_is_matched(const GUID_t &writer_guid) = 0;

  /**
   * Processes a new DATA message. Previously the message must have been
   * accepted by function acceptMsgDirectedTo.
   *
   ...
   * @param fragmentsInSubmessage Number of fragments on this particular
   * message.
   * @return true if the reader accepts message.
   */
  RTPS_DllAPI virtual bool
  processDataFragMsg(CacheChange_t *change, uint32_t sampleSize,
   ...
   * @return true if the reader accepts messages from the.
   */
  RTPS_DllAPI virtual bool processHeartbeatMsg(const GUID_t &writerGUID,
                                               uint32_t hbCount,
   ...
   */
  RTPS_DllAPI virtual bool
  processGapMsg(const GUID_t &writerGUID, const SequenceNumber_t &gapStart,
   ...
   */
  RTPS_DllAPI virtual bool
  change_removed_by_history(CacheChange_t *change,
                            WriterProxy *prox = nullptr) = 0;

  /**
   * Get the associated listener, secondary attached Listener in case it is of
   * compound type
   ...
   * If the RTPSReader does not belong to the built-in protocols it switches out
   * the old one. If it belongs to the built-in protocols, it sets the new
   * ReaderListener callbacks to be called after the built-in ReaderListener
   * ones.
   ...
   * @return True if read.
   */
  RTPS_DllAPI virtual bool nextUnreadCache(CacheChange_t **change,
                                           WriterProxy **wp) = 0;
   ...
   * @return True if read.
   */
  RTPS_DllAPI virtual bool nextUntakenCache(CacheChange_t **change,
                                            WriterProxy **wp) = 0;

  RTPS_DllAPI bool
  wait_for_unread_cache(const eprosima::fastrtps::Duration_t &timeout);
  ...
  RTPS_DllAPI eprosima::fastdds::rtps::IReaderDataFilter *
  ...
  RTPS_DllAPI void
  set_content_filter(eprosima::fastdds::rtps::IReaderDataFilter *filter) {
    std::lock_guard<RecursiveTimedMutex> guard(mp_mutex);
    data_filter_ = filter;
  }
   ...
   * It occurs when the Reader received all samples sent by Writers. In other
   * words, its WriterProxies are up to date.
   ...
   * @param [out] is_future_change  Whether the change is in the future (i.e.
   * there are earlier unreceived changes from the same writer).
   *
   * @return Whether the change is still valid or not.
   */
  virtual bool begin_sample_access_nts(CacheChange_t *change, WriterProxy *&wp,
   ...
   * @param [in] mark_as_read  Whether the @c change should be marked as read or
   * not.
   */
  virtual void end_sample_access_nts(CacheChange_t *change, WriterProxy *&wp,
                                     bool mark_as_read) = 0;
   ...
   */
  virtual void change_read_by_user(CacheChange_t *change, WriterProxy *writer,
   ...
   */
  RTPS_DllAPI bool is_sample_valid(const void *data, const GUID_t &writer,
...
protected:
   ...
   * @param guid GUID of the remote writer
   * @param persistence_guid Persistence GUID of the remote writer
   * @param removed_by_lease Whether the GUIDs are being removed due to a
   * participant drop.
   */
  void remove_persistence_guid(const GUID_t &guid,
   ...
   * @remarks Takes persistence_guid into consideration
   */
  SequenceNumber_t update_last_notified(const GUID_t &guid,
   ...
   */
  virtual void set_last_notified(const GUID_t &persistence_guid,
                                 const SequenceNumber_t &seq);

  /*!
   * @brief Search if there is a CacheChange_t, giving SequenceNumber_t and
   * writer GUID_t, waiting to be completed because it is fragmented.
   ...
   * @param change If a CacheChange_t was found, this argument will fill with
   * its pointer. In other case nullptr is returned.
  ...
  History::const_iterator findCacheInFragmentedProcess(
      const SequenceNumber_t &sequence_number, const GUID_t &writer_guid,
      CacheChange_t **change, History::const_iterator hint) const;
