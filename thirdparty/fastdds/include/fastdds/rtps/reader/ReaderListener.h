...
/**
 * Class ReaderListener, to be used by the user to override some of is virtual
 * method to program actions to certain events.
...
public:
  ...
  /**
   * This method is called when a new CacheChange_t is added to the
   * ReaderHistory.
   * @param reader Pointer to the reader.
   * @param change Pointer to the CacheChange_t. This is a const pointer to
   * const data to indicate that the user should not dispose of this data
   * himself. To remove the data call the remove_change method of the
   * ReaderHistory. reader->getHistory()->remove_change((CacheChange_t*)change).
   */
  virtual void onNewCacheChangeAdded(RTPSReader *reader,
   ...
   * matches that of a local reader, but with an offered QoS that is
   * incompatible with the one requested by the local reader
   * @param reader Pointer to the RTPSReader.
   * @param qos A mask with the bits of all incompatible Qos activated.
   */
  virtual void
  on_requested_incompatible_qos(RTPSReader *reader,
  ...
  /**
   * This method is called when the reader detects that one or more samples have
   * been lost.
   *
   * @param reader                         Pointer to the RTPSReader.
   * @param sample_lost_since_last_update  The number of samples that were lost
   * since the last time this method was called for the same reader.
   */
  ...
  /**
   * @brief Method called when the discovery information of a writer regarding a
   * reader changes.
   *
   * @param reader       The reader.
   * @param reason       The reason motivating this method to be called.
   * @param writer_guid  The GUID of the writer for which the discovery
   * information changed.
   * @param writer_info  Discovery information about the writer. Will be @c
   * nullptr for reason @c REMOVED_WRITER.
   */
