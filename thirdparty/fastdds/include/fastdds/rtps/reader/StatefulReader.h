...
/**
 * Class StatefulReader, specialization of RTPSReader than stores the state of
 * the matched writers.
...
protected:
  StatefulReader(RTPSParticipantImpl *pimpl, const GUID_t &guid,
...
public:
   ...
   * @param removed_by_lease true it the writer was removed due to lease
   * duration.
   * @return True if correct.
   */
  bool matched_writer_remove(const GUID_t &writer_guid,
   ...
   */
  bool processDataFragMsg(CacheChange_t *change, uint32_t sampleSize,
   ...
   * @return true if the reader accepts messages.
   */
  bool processHeartbeatMsg(const GUID_t &writerGUID, uint32_t hbCount,
                           const SequenceNumber_t &firstSN,
                           const SequenceNumber_t &lastSN, bool finalFlag,
  ...
  /**
   * This method is called when a new change is received. This method calls the
   * received_change of the History and depending on the implementation performs
   * different actions.
   * @param a_change Pointer of the change to add.
   * @param prox Pointer to the WriterProxy that adds the Change.
   * @param unknown_missing_changes_up_to The number of changes from the same
   * writer with a lower sequence number that could potentially be received in
   * the future.
   ...
   */
  void send_acknack(const WriterProxy *writer, const SequenceNumberSet_t &sns,
                    RTPSMessageSenderInterface *sender, bool is_final);

  /**
   * Sends an acknack message from this reader in response to a heartbeat.
   * @param writer Pointer to the proxy representing the writer to send the
   * acknack to.
   * @param sender Message sender interface.
   * @param heartbeat_was_final Final flag of the last received heartbeat.
   */
  void send_acknack(const WriterProxy *writer,
   ...
   * @param max_blocking_time_point Future time point where any blocking should
   * end.
   */
  bool
  send_sync_nts(CDRMessage_t *message, const Locators &locators_begin,
   ...
   */
  void assert_writer_liveliness(const GUID_t &writer) override;
   ...
   */
  bool begin_sample_access_nts(CacheChange_t *change, WriterProxy *&wp,
   ...
   * not.
   */
  void end_sample_access_nts(CacheChange_t *change, WriterProxy *&wp,
   ...
   */
  void change_read_by_user(CacheChange_t *change, WriterProxy *writer,
                           bool mark_as_read = true) override;

private:
  void init(RTPSParticipantImpl *pimpl, const ReaderAttributes &att);

  bool acceptMsgFrom(const GUID_t &entityGUID, WriterProxy **wp) const;

  /*!
   * @remarks Non thread-safe.
   */
  bool findWriterProxy(const GUID_t &writerGUID, WriterProxy **wp) const;

  void NotifyChanges(WriterProxy *wp);

  void remove_changes_from(const GUID_t &writerGUID,
  ...
  //! Vector containing pointers to all the inactive, ready for reuse,
  //! WriterProxies.
