...
public:
  ...
  EndpointAttributes()
      : endpointKind(WRITER), topicKind(NO_KEY), reliabilityKind(BEST_EFFORT),
        durabilityKind(VOLATILE), persistence_guid(), m_userDefinedID(-1),
        m_entityID(-1) {
    datasharing_.off();
  }
   ...
   */
  inline void setUserDefinedID(int16_t id) { m_userDefinedID = id; }

  /**
   * Set the entity ID
   * @param id Entity ID to be set
   */
  inline void setEntityID(int16_t id) { m_entityID = id; }
