...
typedef enum DiscoveryProtocol {
  ...
  /*!<
      ...
      Publisher and Subscriber defined with the same topic name would NOT be
     linked. All matching must be done manually through the addReaderLocator,
     addReaderProxy, addWriterProxy methods.
   */
  SIMPLE,
  /*!<
      Discovery works according to 'The Real-time Publish-Subscribe
     Protocol(RTPS) DDS Interoperability Wire Protocol Specification'
   */
  EXTERNAL,
  /*!<
      A user defined PDP subclass object must be provided in the attributes that
     deals with the discovery. Framework is not responsible of this object
     lifetime.
   */
  CLIENT,      /*!< The participant will behave as a client concerning discovery
                  operation.      Server locators should be specified as attributes.
                */
  SERVER,      /*!< The participant will behave as a server concerning discovery
                  operation.      Discovery operation is volatile (discovery
                  handshake      must take place if shutdown). */
  BACKUP,      /*!< The participant will behave as a server concerning discovery
                  operation.      Discovery operation persist on a file (discovery
                  handshake wouldn't repeat if shutdown). */
  SUPER_CLIENT /*!< The participant will behave as a client concerning all
                  internal behaviour. Remote servers will treat it as a server
                  and will share every discovery information. */
...
public:
  ...
  bool operator==(const SimpleEDPAttributes &b) const {
    return (this->use_PublicationWriterANDSubscriptionReader ==
            b.use_PublicationWriterANDSubscriptionReader) &&
#if HAVE_SECURITY
...
#endif // if HAVE_SECURITY
           (this->use_PublicationReaderANDSubscriptionWriter ==
            b.use_PublicationReaderANDSubscriptionWriter);
  }
};

/**
 * Struct InitialAnnouncementConfig defines the behavior of the RTPSParticipant
...
/**
 * Class DiscoverySettings, to define the attributes of the several discovery
 * protocols available
 * @ingroup RTPS_ATTRIBUTES_MODULE
 */
...
public:
   ...
   * indicating how much time remote RTPSParticipants should consider this
  ...
  Duration_t discoveryServer_client_syncperiod = {
      0, 450 * 1000000}; // 450 milliseconds

  //! Discovery Server settings, only needed if
  //! use_CLIENT_DiscoveryProtocol=true
  eprosima::fastdds::rtps::RemoteServerList_t m_DiscoveryServers;

  //! Filtering participants out depending on location
  ParticipantFilteringFlags_t ignoreParticipantFlags =
      ParticipantFilteringFlags::NO_FILTER;
  ...
  bool operator==(const DiscoverySettings &b) const {
    return (this->discoveryProtocol == b.discoveryProtocol) &&
           (this->use_SIMPLE_EndpointDiscoveryProtocol ==
            b.use_SIMPLE_EndpointDiscoveryProtocol) &&
           (this->use_STATIC_EndpointDiscoveryProtocol ==
            b.use_STATIC_EndpointDiscoveryProtocol) &&
           (this->discoveryServer_client_syncperiod ==
            b.discoveryServer_client_syncperiod) &&
           (this->m_PDPfactory == b.m_PDPfactory) &&
           (this->leaseDuration == b.leaseDuration) &&
           (this->leaseDuration_announcementperiod ==
            b.leaseDuration_announcementperiod) &&
   ...
   * The string could contain a filename (file://) or the XML content directly
   * (data://).
   */
  void static_edp_xml_config(const char *str) { static_edp_xml_config_ = str; }
...
private:
  //! URI specifying the static EDP XML configuration, only necessary if
  //! use_STATIC_EndpointDiscoveryProtocol=true This string could contain a
  //! filename or the XML content directly.
...
public:
  ...
  bool operator==(const BuiltinAttributes &b) const {
    return (this->discovery_config == b.discovery_config) &&
           (this->use_WriterLivelinessProtocol ==
            b.use_WriterLivelinessProtocol) &&
           (typelookup_config.use_client == b.typelookup_config.use_client) &&
           (typelookup_config.use_server == b.typelookup_config.use_server) &&
           (this->metatrafficUnicastLocatorList ==
            b.metatrafficUnicastLocatorList) &&
           (this->metatrafficMulticastLocatorList ==
            b.metatrafficMulticastLocatorList) &&
...
public:
  ...
  bool operator==(const RTPSParticipantAttributes &b) const {
    return (this->name == b.name) &&
           ...
           (this->defaultMulticastLocatorList ==
            b.defaultMulticastLocatorList) &&
           ...
           (this->properties == b.properties) && (this->prefix == b.prefix) &&
           (this->flow_controllers == b.flow_controllers);
  }

  /**
   * Provides a way of easily configuring transport related configuration on
   * certain pre-defined scenarios.
   *
   * @param transports Defines the transport configuration scenario to setup.
   */
  RTPS_DllAPI void
  setup_transports(fastdds::rtps::BuiltinTransports transports);

  /**
   * Default list of Unicast Locators to be used for any Endpoint defined inside
   * this RTPSParticipant in the case that it was defined with NO
  ...
  /*!
   * @brief Send socket buffer size for the send resource. Zero value indicates
   * to use default system buffer size. Default value: 0.
   */
  uint32_t sendSocketBufferSize = 0;

  /*! Listen socket buffer for all listen resources. Zero value indicates to use
   * default system buffer size. Default value: 0.
   */
