...
/**
 * Struct WriterTimes, defining the times associated with the Reliable Writers
 * events.
...
struct WriterTimes {
  ...
  //! This time allows the RTPSWriter to ignore nack messages too soon after the
  //! data as sent, default value 0s.
...
public:
  WriterAttributes()
      : liveliness_kind(AUTOMATIC_LIVELINESS_QOS),
        liveliness_lease_duration(TIME_T_INFINITE_SECONDS,
                                  TIME_T_INFINITE_NANOSECONDS),
        liveliness_announcement_period(TIME_T_INFINITE_SECONDS,
                                       TIME_T_INFINITE_NANOSECONDS),
        mode(SYNCHRONOUS_WRITER), disable_heartbeat_piggyback(false),
