...
/**
 * Class RemoteServerAttributes, to define the attributes of the Discovery
 * Server Protocol.
...
template <class charT> struct server_ostream_separators {
...
std::basic_ostream<charT> &operator<<(std::basic_ostream<charT> &output,
  ...
  if (!sa.metatrafficUnicastLocatorList.empty()) {
    output << server_ostream_separators<charT>::locator_separator
           << sa.metatrafficUnicastLocatorList;
  }
...
std::basic_ostream<charT> &operator<<(std::basic_ostream<charT> &output,
  ...
  typename std::basic_ostream<charT>::sentry s(output);
  std::ostream_iterator<RemoteServerAttributes> os_iterator(
      output, server_ostream_separators<charT>::list_separator);
...
const char *const DEFAULT_ROS2_SERVER_GUIDPREFIX =
...
/* Environment variable to specify a semicolon-separated list of UDPv4 locators
 * (ip:port) that define remote server locators. The position in the list is
 * used as a "server id" to extrapolate the server's GUID prefix. For the
 * variable to take any effect, the following pre-conditions must be met:
 *    1. The server's GUID prefix must be compliant with the schema
 *       "44.53.<server_id_in_hex>.5f.45.50.52.4f.53.49.4d.41", which is the
 * schema followed by the prefixes generated when creating server using fastdds
 * cli, being DEFAULT_ROS2_SERVER_GUIDPREFIX the prefix for ID=0.
 *    1. The discovery protocol must be either SIMPLE or SERVER.
 *       1. In the case of SIMPLE, the participant is created as a CLIENT
 * instead.
 *       1. In the case of SERVER, the participant is created as a SERVER, using
 * the DEFAULT_ROS2_MASTER_URI list to expand the list of remote servers.
 */
 ...
 *    - FALSE, false, False, 0
 *    - TRUE, true, True, 1
 * If the variable is not set, the program will behave like the variable is set
 * to false.
 */
 ...
 */
RTPS_DllAPI bool load_environment_server_info(const std::string &list,
                                              RemoteServerList_t &attributes);

/**
 * Retrieves a semicolon-separated list of locators from DEFAULT_ROS2_MASTER_URI
 * environment variable, and populates a RemoteServerList_t mapping list
 * position to default guid.
 *
 * The environment variable can be read from an environment file (which allows
 * runtime modification of the remote servers list) or directly from the
 * environment. The value contained in the file takes precedence over the
 * environment value (if both are set). This is to avoid conflicts because only
 * new servers can be added to the list (containing thus all the previously
 * known servers).
 *
 * @param[out] attributes reference to a RemoteServerList_t to populate.
 * @return true if parsing succeeds, false otherwise
 */
RTPS_DllAPI bool load_environment_server_info(RemoteServerList_t &attributes);

/**
 * Get the value of environment variable DEFAULT_ROS2_MASTER_URI
 * @return The value of environment variable DEFAULT_ROS2_MASTER_URI. Empty
 ...
 * @return The value of environment variable ROS_SUPER_CLIENT. False if the
 ...
 */
RTPS_DllAPI bool
get_server_client_default_guidPrefix(int id,
