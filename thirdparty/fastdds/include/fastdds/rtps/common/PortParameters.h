...
public:
  PortParameters()
      : portBase(7400), domainIDGain(250), participantIDGain(2), offsetd0(0),
        offsetd1(10), offsetd2(1), offsetd3(11) {}
  ...
  bool operator==(const PortParameters &b) const {
    return (this->portBase == b.portBase) &&
           ...
           (this->offsetd0 == b.offsetd0) && (this->offsetd1 == b.offsetd1) &&
           (this->offsetd2 == b.offsetd2) && (this->offsetd3 == b.offsetd3);
  }
  ...
  inline uint32_t getMulticastPort(uint32_t domainId) const {
    ...
    if (port > 65535) {
      logError(RTPS, "Calculated port number is too high. Probably the "
                     "domainId is over 232 "
                         << "or portBase is too high.");
      std::cout << "Calculated port number is too high. Probably the domainId "
                   "is over 232 "
   ...
   * @return Unicast port
   */
  inline uint32_t getUnicastPort(uint32_t domainId,
                                 uint32_t RTPSParticipantID) const {
    uint32_t port = portBase + domainIDGain * domainId + offsetd1 +
                    participantIDGain * RTPSParticipantID;

    if (port > 65535) {
      logError(RTPS,
               "Calculated port number is too high. Probably the domainId is "
               "over 232, there are "
                   << "too much participants created or portBase is too high.");
      std::cout << "Calculated port number is too high. Probably the domainId "
                   "is over 232, there are "
                << "too much participants created or portBase is too high."
                << std::endl;
      std::cout.flush();
      exit(EXIT_FAILURE);
