...
public:
  ...
  /// Copy constructor
  RTPS_DllAPI LocatorList(const LocatorList &list)
      : m_locators(list.m_locators) {}

  /// Move constructor
  RTPS_DllAPI LocatorList(LocatorList &&list)
  ...
  RTPS_DllAPI LocatorList &operator=(const LocatorList &list) {
    ...
    /// Move assignment
    RTPS_DllAPI LocatorList &operator=(LocatorList &&list) {
      m_locators = std::move(list.m_locators);
      return *this;
    }

    /// Equal to operator
    RTPS_DllAPI bool operator==(const LocatorList &locator_list) const {
      if (locator_list.m_locators.size() == m_locators.size()) {
        bool returnedValue = true;

        for (auto it = locator_list.m_locators.begin();
             returnedValue && it != locator_list.m_locators.end(); ++it) {
          returnedValue = false;

          for (auto it2 = m_locators.begin();
               !returnedValue && it2 != m_locators.end(); ++it2) {
            if (*it == *it2) {
              returnedValue = true;
            }
          }
     ...
     */
    RTPS_DllAPI LocatorListIterator begin() { return m_locators.begin(); }
     ...
     */
    RTPS_DllAPI LocatorListIterator end() { return m_locators.end(); }
     ...
     */
    RTPS_DllAPI size_t size() const { return m_locators.size(); }
     ...
     */
    RTPS_DllAPI LocatorList &assign(const LocatorList &list) {
      if (!(*this == list)) {
        m_locators = list.m_locators;
      }
      return *this;
     ...
     */
    RTPS_DllAPI void push_back(const Locator &loc) {
      bool already = false;
     ...
     */
    RTPS_DllAPI void push_back(const LocatorList &locList) {
      for (auto it = locList.m_locators.begin(); it != locList.m_locators.end();
           ++it) {
        this->push_back(*it);
      }
    }
     ...
     */
    RTPS_DllAPI bool empty() const { return m_locators.empty(); }
     ...
     */
    RTPS_DllAPI void erase(const Locator &loc) {
    ...
    FASTDDS_DEPRECATED_UNTIL(
        3, "eprosima::fastrtps::rtps::LocatorList::contains(const Locator&)",
        "Unused method.")
    RTPS_DllAPI bool contains(const Locator &loc) {
      for (LocatorListIterator it = this->begin(); it != this->end(); ++it) {
    ...
    RTPS_DllAPI bool isValid() const {
      for (LocatorListConstIterator it = this->begin(); it != this->end();
           ++it) {
     ...
     */
    RTPS_DllAPI void swap(LocatorList &locatorList) {
