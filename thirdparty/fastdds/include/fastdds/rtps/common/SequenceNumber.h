...
//!@brief Structure SequenceNumber_t, different for each change in the same
//! writer.
...
struct RTPS_DllAPI SequenceNumber_t {
   ...
   * @param u
   */
  explicit SequenceNumber_t(uint64_t u) noexcept
      : high(static_cast<int32_t>(u >> 32u)), low(static_cast<uint32_t>(u)) {}
  ...
  SequenceNumber_t &operator+=(int inc) noexcept {
    ...
    if (low < aux_low) {
      // Being the type of the parameter an 'int', the increment of 'high' will
      // be as much as 1.
      assert(std::numeric_limits<decltype(high)>::max() > high);
      ++high;
    }
  ...
  static SequenceNumber_t unknown() noexcept { return {-1, 0}; }
};
 ...
 */
inline bool operator==(const SequenceNumber_t &sn1,
 ...
 */
inline bool operator>(const SequenceNumber_t &seq1,
 ...
 * @param seq1 First SequenceNumber_t to compare
 * @param seq2 Second SequenceNumber_t to compare
 * @return True if the first SequenceNumber_t is greater or equal than the
 * second
 */
inline bool operator>=(const SequenceNumber_t &seq1,
                       const SequenceNumber_t &seq2) noexcept {
 ...
 */
inline SequenceNumber_t operator-(const SequenceNumber_t &seq,
                                  const uint32_t inc) noexcept {
  SequenceNumber_t res(seq.high, seq.low - inc);

  if (inc > seq.low) {
    // Being the type of the parameter an 'uint32_t', the decrement of 'high'
    // will be as much as 1.
    assert(0 < res.high);
    --res.high;
  }
...
inline SequenceNumber_t operator+(const SequenceNumber_t &seq,
  ...
  if (res.low < seq.low) {
    // Being the type of the parameter an 'uint32_t', the increment of 'high'
    // will be as much as 1.
 ...
 * @return Result of the subtraction
 */
inline SequenceNumber_t operator-(const SequenceNumber_t &minuend,
                                  const SequenceNumber_t &subtrahend) noexcept {
  assert(minuend >= subtrahend);
  SequenceNumber_t res(minuend.high - subtrahend.high,
 ...
 */
inline bool sort_seqNum(const SequenceNumber_t &s1,
...
struct SequenceNumberDiff {
  uint32_t operator()(const SequenceNumber_t &a,
...
//!@ingroup COMMON_MODULE
using SequenceNumberSet_t =
    BitmapRange<SequenceNumber_t, SequenceNumberDiff, 256>;
...
inline std::ostream &operator<<(std::ostream &output,
  ...
  sns.for_each(
      [&output](SequenceNumber_t it) { output << it.to64long() << "-"; });
 ...
 * @return
 */
inline std::istream &operator>>(std::istream &input, SequenceNumber_t &seqNum) {
  uint64_t aux;
