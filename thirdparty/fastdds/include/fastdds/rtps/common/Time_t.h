...
struct RTPS_DllAPI Time_t {
  ...
  inline bool is_infinite() const noexcept { return is_infinite(*this); }
  ...
  static inline constexpr bool is_infinite(const Time_t &t) noexcept {
    return (INFINITE_SECONDS == t.seconds) ||
...
private:
  ...
  void set_fraction(uint32_t frac);

  void set_nanosec(uint32_t nanos);
};
...
inline std::istream &operator>>(std::istream &input, Time_t &t) {
...
//! Time_t (Duration_t) representing an infinite time. DONT USE IT IN
//! CONSTRUCTORS
const Time_t c_TimeInfinite{Time_t::INFINITE_SECONDS,
                            Time_t::INFINITE_NANOSECONDS};
//! Time_t (Duration_t) representing a zero time. DONT USE IT IN CONSTRUCTORS
const Time_t c_TimeZero{0, 0};
//! Time_t (Duration_t) representing an invalid time. DONT USE IT IN
//! CONSTRUCTORS
...
#define TIME_T_INFINITE_NANOSECONDS                                            \
  (eprosima::fastrtps::Time_t::INFINITE_NANOSECONDS)
