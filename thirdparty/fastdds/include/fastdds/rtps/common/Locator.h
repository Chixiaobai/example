...
/**
 * @brief Class Locator_t, uniquely identifies a communication channel for a
 * particular transport. For example, an address + port combination in the case
 * of UDP.
...
public:
  ...
  /// Default constructor
  Locator_t() : kind(LOCATOR_KIND_UDPv4) {
    port = 0;
    LOCATOR_ADDRESS_INVALID(address);
  }

  /// Move constructor
  Locator_t(Locator_t &&loc) : kind(loc.kind) {
    port = loc.port;
    std::memcpy(address, loc.address, 16 * sizeof(octet));
  }

  /// Copy constructor
  Locator_t(const Locator_t &loc) : kind(loc.kind) {
    port = loc.port;
    std::memcpy(address, loc.address, 16 * sizeof(octet));
  }

  /// Port constructor
  Locator_t(uint32_t portin) : kind(LOCATOR_KIND_UDPv4) {
    port = portin;
    LOCATOR_ADDRESS_INVALID(address);
  }

  /// Kind and port constructor
  Locator_t(int32_t kindin, uint32_t portin) : kind(kindin) {
    port = portin;
    LOCATOR_ADDRESS_INVALID(address);
  }
   ...
   *
   * @param other Locator which IP address is used to set this locator IP
   * address.
   * @return always true.
   */
   ...
   */
  octet *get_address() { return address; }
   ...
   */
  octet get_address(uint16_t field) const { return address[field]; }

  /**
   * @brief Automatic setter for setting locator IP address to invalid address
   * (0).
   */
  void set_Invalid_Address() { LOCATOR_ADDRESS_INVALID(address); }
};
...
inline bool IsAddressDefined(const Locator_t &loc) {
  if (loc.kind == LOCATOR_KIND_UDPv4 ||
      loc.kind == LOCATOR_KIND_TCPv4) // WAN addr in TCPv4 is optional, isn't?
  {
...
/**
 * @brief Auxiliary method to check that locator kind is not
 ...
 * @return false otherwise.
 */
inline bool IsLocatorValid(const Locator_t &loc) { return (0 <= loc.kind); }
 ...
 * @return \c std::ostream& Reference to the output stream with the serialized
 * locator appended.
 */
inline std::ostream &operator<<(std::ostream &output, const Locator_t &loc) {
  ...
  if (loc.kind == LOCATOR_KIND_TCPv4 || loc.kind == LOCATOR_KIND_TCPv6) {
    output << "]:" << std::to_string(IPLocator::getPhysicalPort(loc)) << "-"
           << std::to_string(IPLocator::getLogicalPort(loc));
  } else {
    output << "]:" << loc.port;
  }
 ...
 *            - TCPv6
 *            - SHM
 *        \c address must be either a name which can be resolved by DNS or the
 * IP address unless \c kind is SHM \c port number
 *
 ...
 * @return \c std::istream& Reference to the input stream after extracting the
 * locator.
 */
inline std::istream &operator>>(std::istream &input, Locator_t &loc) {
