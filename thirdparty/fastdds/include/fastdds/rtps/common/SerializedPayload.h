...
/*!
 * @brief Maximum payload is maximum of UDP packet size minus 536bytes
 * (RTPSMESSAGE_COMMON_RTPS_PAYLOAD_SIZE) With those 536 bytes
 * (RTPSMESSAGE_COMMON_RTPS_PAYLOAD_SIZE) bytes is posible to send RTPS Header
 * plus RTPS Data submessage plus RTPS Heartbeat submessage.
 */
...
struct RTPS_DllAPI SerializedPayload_t {
  //! Size in bytes of the representation header as specified in the RTPS 2.3
  //! specification chapter 10.
  static constexpr size_t representation_header_size = 4u;

  //! Encapsulation of the data as suggested in the RTPS 2.1 specification
  //! chapter 10.
  ...
  //! Default constructor
  SerializedPayload_t()
      : encapsulation(CDR_BE), length(0), data(nullptr), max_size(0), pos(0) {}

  /**
   * @param len Maximum size of the payload
   */
  explicit SerializedPayload_t(uint32_t len) : SerializedPayload_t() {
    this->reserve(len);
  }

  ~SerializedPayload_t() { this->empty(); }
   ...
   * @param with_limit if true, the function will fail when providing a payload
   * too big
