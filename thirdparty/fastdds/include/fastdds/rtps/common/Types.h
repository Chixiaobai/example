...
//!@ingroup COMMON_MODULE
typedef enum ReliabilityKind_t { RELIABLE, BEST_EFFORT } ReliabilityKind_t;
...
//! Endpoint kind
//!@ingroup COMMON_MODULE
typedef enum EndpointKind_t { READER, WRITER } EndpointKind_t;

//! Topic kind
typedef enum TopicKind_t { NO_KEY, WITH_KEY } TopicKind_t;
...
struct RTPS_DllAPI ProtocolVersion_t {
  ...
  ProtocolVersion_t(octet maj, octet min) : m_major(maj), m_minor(min) {}
...
inline std::ostream &operator<<(std::ostream &output,
  ...
  return output << static_cast<int>(pv.m_major) << "."
                << static_cast<int>(pv.m_minor);
}
