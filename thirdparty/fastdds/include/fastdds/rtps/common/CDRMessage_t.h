...
#define RTPSMESSAGE_COMMON_RTPS_PAYLOAD_SIZE                                   \
  536 // common payload a rtps message has TODO(Ricardo) It is necessary?
...
#define RTPSMESSAGE_OCTETSTOINLINEQOS_DATASUBMSG                               \
  16 // may change in future versions
#define RTPSMESSAGE_OCTETSTOINLINEQOS_DATAFRAGSUBMSG                           \
  28 // may change in future versions
...
struct RTPS_DllAPI CDRMessage_t final {
  ...
  CDRMessage_t() : CDRMessage_t(RTPSMESSAGE_DEFAULT_SIZE) {}
   ...
   */
  explicit CDRMessage_t(uint32_t size) {
    wraps = false;
    pos = 0;
    length = 0;
   ...
   */
  explicit CDRMessage_t(const SerializedPayload_t &payload) : wraps(true) {
