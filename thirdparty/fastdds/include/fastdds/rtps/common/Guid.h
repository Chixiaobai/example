...
struct RTPS_DllAPI GUID_t {
  ...
  GUID_t(const GuidPrefix_t &guid_prefix, uint32_t id) noexcept
      : guidPrefix(guid_prefix), entityId(id) {}
  ...
  GUID_t(const GuidPrefix_t &guid_prefix, const EntityId_t &entity_id) noexcept
      : guidPrefix(guid_prefix), entityId(entity_id) {}
  ...
  /**
   * Checks whether this guid is for an entity on the same host and process as
   * another guid.
   *
   * @param other_guid GUID_t to compare to.
   *
   * @return true when this guid is on the same host and process, false
   * otherwise.
   */
   ...
   *
   * @return true when this guid corresponds to a builtin entity, false
   * otherwise.
   */
  bool is_builtin() const { return entityId.value[3] >= 0xC0; }

  static GUID_t unknown() noexcept { return GUID_t(); }
