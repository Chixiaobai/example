...
struct RTPS_DllAPI InstanceHandleValue_t {
   ...
   */
  template <typename T> octet &operator[](T i) noexcept {
    has_been_set_ = true;
    return value_[i];
  }
   ...
   *         Use method has_been_set() instead.
   */
  template <typename T> octet operator[](T i) const noexcept {
    return value_[i];
  }
   ...
   *         Use method has_been_set() instead.
   */
  operator const octet *() const noexcept { return value_.data(); }

  /**
   * Return whether any of the write access operators of this value has been
   * used.
   */
  bool has_been_set() const noexcept { return has_been_set_; }
...
private:
  //! Hash value
  KeyHash_t value_{};
  //! Flag indicating if value_ has been modified since the creation of this
  //! object
...
struct RTPS_DllAPI InstanceHandle_t {
  ...
  InstanceHandle_t(const GUID_t &guid) noexcept { *this = guid; }
   ...
   */
  InstanceHandle_t &
  operator=(const InstanceHandle_t &ihandle) noexcept = default;

  /**
   * Assignment operator
   * @param guid GUID to copy the data from
   */
  InstanceHandle_t &operator=(const GUID_t &guid) noexcept {
    octet *dst = value;
   ...
   */
  bool isDefined() const noexcept { return value.has_been_set(); }

  void clear() noexcept { value.clear(); }

  // TODO Review this conversion once InstanceHandle_t is implemented as DDS
  // standard defines
 ...
 */
inline bool operator==(const InstanceHandle_t &ihandle1,
 ...
 */
inline void iHandle2GUID(GUID_t &guid,
 ...
 * @return GUID_t
 */
inline GUID_t iHandle2GUID(const InstanceHandle_t &ihandle) noexcept {
  GUID_t guid;
...
/**
 * @brief Comparison operator: checks if a InstanceHandle_t is less than
 * another.
 *
 ...
 */
inline bool operator<(const InstanceHandle_t &h1,
