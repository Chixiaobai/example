...
public:
  ...
  void value(std::vector<uint8_t> &&value) { value_ = std::move(value); }
...
public:
  static size_t serialized_size(const BinaryProperty &binary_property,
    ...
    if (binary_property.propagate()) {
      ...
      current_alignment +=
          4 + alignment(current_alignment, 4) + binary_property.value().size();
  ...
  static size_t serialized_size(const BinaryPropertySeq &binary_properties,
                                size_t current_alignment = 0) {
    size_t initial_alignment = current_alignment;

    current_alignment += 4 + alignment(current_alignment, 4);
    for (auto binary_property = binary_properties.begin();
         binary_property != binary_properties.end(); ++binary_property)
