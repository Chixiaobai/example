...
struct RTPS_DllAPI EntityId_t {
  ...
  EntityId_t() { *this = ENTITYID_UNKNOWN; }
...
{
  output << std::hex;
  output << (int)enI.value[0] << "." << (int)enI.value[1] << "."
         << (int)enI.value[2] << "." << (int)enI.value[3];
...
namespace std {
template <> struct hash<eprosima::fastrtps::rtps::EntityId_t> {
