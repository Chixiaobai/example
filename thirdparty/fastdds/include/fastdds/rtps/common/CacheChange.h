...
struct CacheChangeWriterInfo_t {
  //! Number of DATA / DATA_FRAG submessages sent to the transport (only used in
  //! Writers)
...
struct RTPS_DllAPI CacheChange_t {
   ...
   */
  CacheChange_t() : writer_info() {
    inline_qos.encapsulation =
        DEFAULT_ENDIAN == LITTLEEND ? PL_CDR_LE : PL_CDR_BE;
  }
  ...
  CacheChange_t(uint32_t payload_size, bool is_untyped = false)
      : serializedPayload(payload_size), is_untyped_(is_untyped) {}

  /*!
   * Copy a different change into this one. All the elements are copied,
   * included the data, allocating new memory.
   ...
   * @param create_fragment_list Whether to create missing fragments list or
   * not.
   *
   * @remarks Parameter create_fragment_list should only be true when receiving
   * the first fragment of a change.
   */
  void setFragmentSize(uint16_t fragment_size,
    ...
    if (fragment_size > 0) {
      ...
      if (create_fragment_list) {
        // Keep index of next fragment on the payload portion at the beginning
        // of each fragment. Last fragment will have fragment_count_ as 'next
        // fragment index'
        size_t offset = 0;
        for (uint32_t i = 1; i <= fragment_count_;
             i++, offset += fragment_size_) {
          set_next_missing_fragment(
              i - 1, i); // index to next fragment in missing list
        }
      } else {
        // List not created. This means we are going to send this change
        // fragmented, so it is already assembled, and the missing list is empty
        // (i.e. first missing points to fragment count)
  ...
  bool add_fragments(const SerializedPayload_t &incoming_data,
    ...
    if (received_fragments(fragment_starting_num - 1,
      ...
      memcpy(&serializedPayload.data[original_offset], incoming_data.data,
             incoming_length);
    }
  ...
  IPayloadPool const *payload_owner() const { return payload_owner_; }

  IPayloadPool *payload_owner() { return payload_owner_; }

  void payload_owner(IPayloadPool *owner) { payload_owner_ = owner; }
...
private:
  ...
  uint32_t get_next_missing_fragment(uint32_t fragment_index) {
    uint32_t *ptr = next_fragment_pointer(fragment_index);
    return *ptr;
  }

  void set_next_missing_fragment(uint32_t fragment_index,
                                 uint32_t next_fragment_index) {
    uint32_t *ptr = next_fragment_pointer(fragment_index);
    *ptr = next_fragment_index;
  }

  uint32_t *next_fragment_pointer(uint32_t fragment_index) {
   ...
   * Should be called BEFORE copying the received data into the serialized
   * payload.
   *
   * @param initial_fragment Index (0-based) of first received fragment.
   * @param num_of_fragments Number of received fragments. Should be strictly
   * positive.
   * @return true if the list of missing fragments was modified, false
   * otherwise.
   */
  bool received_fragments(uint32_t initial_fragment,
      ...
      if (initial_fragment <= first_missing_fragment_) {
        ...
        while (first_missing_fragment_ < last_fragment) {
          first_missing_fragment_ =
              get_next_missing_fragment(first_missing_fragment_);
        ...
        while (current_frag < initial_fragment) {
          ...
          if (next_frag >= initial_fragment) {
            ...
            // Find future value for next by repeating next = *next until next
            // >= last_fragment.
            uint32_t next_missing_fragment = next_frag;
            while (next_missing_fragment < last_fragment) {
              next_missing_fragment =
                  get_next_missing_fragment(next_missing_fragment);
              at_least_one_changed = true;
            }
