...
private:
  ...
  /*
   * Mutex to protect the m_DiscoveryServers collection. Element access is not
   * protected by this mutex, the PDP mutex needs to be used when querying or
   * modifying mutable members of the collection.
   */
...
public:
   ...
   * create the EDP readers you are not sure the selected endpoints can be used.
  ...
  /**
   * Traverses the list of discover servers translating from remote to local
   * locators if possible
   ...
   * @return True if correct.
   */
  bool addLocalWriter(RTPSWriter *w, const TopicAttributes &topicAtt,
   ...
   */
  bool addLocalReader(
      RTPSReader *R, const TopicAttributes &topicAtt,
   ...
   * @return
   */
  bool updateLocalWriter(RTPSWriter *W, const TopicAttributes &topicAtt,
                         const fastdds::dds::WriterQos &wqos);
  /**
