...
public:
   ...
   */
  bool assert_liveliness(GUID_t writer, LivelinessQosPolicyKind kind,
...
private:
  ...
  //! Minimum time among liveliness periods of automatic writers, in
  //! milliseconds
  double min_automatic_ms_;
  //! Minimum time among liveliness periods of manual by participant writers, in
  //! milliseconds
  ...
  //! Pointer to the periodic assertion timer object for automatic liveliness
  //! writers
  TimedEvent *automatic_liveliness_assertion_;
  //! Pointer to the periodic assertion timer object for manual by participant
  //! liveliness writers
  ...
  //! A boolean indicating that there is at least one reader requesting
  //! automatic liveliness
  bool automatic_readers_;

  //! A class used by writers in this participant to keep track of their
  //! liveliness
  LivelinessManager *pub_liveliness_manager_;
  //! A class used by readers in this participant to keep track of liveliness of
  //! matched writers
  ...
  /**
   * @brief A method invoked by pub_liveliness_manager_ to inform that a writer
   * changed its liveliness
   ...
   */
  void pub_liveliness_changed(const GUID_t &writer,
                              const LivelinessQosPolicyKind &kind,
                              const Duration_t &lease_duration,
                              int32_t alive_change, int32_t not_alive_change);

  /**
   * @brief A method invoked by sub_liveliness_manager_ to inform that a writer
   * changed its liveliness
   * @param writer The writer losing liveliness
   * @param kind The liveliness kind of the writer losing liveliness
   * @param lease_duration The liveliness lease duration of the writer losing
   * liveliness
   * @param alive_change The change in the alive count
   * @param not_alive_change The change in the not alive count
   */
  void sub_liveliness_changed(const GUID_t &writer,
                              const LivelinessQosPolicyKind &kind,
                              const Duration_t &lease_duration,
                              int32_t alive_change, int32_t not_alive_change);
   ...
   * @param alive_change The change requested for alive count. Should be -1, 0
   * or +1
   * @param not_alive_change The change requested for not alive count. Should be
   * -1, 0 or +1
   */
