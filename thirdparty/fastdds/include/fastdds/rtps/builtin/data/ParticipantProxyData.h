...
template <class Proxy> class ProxyHashTable;
...
public:
   ...
   * @return True on success
   */
  bool readFromCDRMessage(CDRMessage_t *msg, bool use_encapsulation,
  ...
  /**
   * Retrieves BACKUP server stamp. On deserialization hints if lease duration
   * must be enforced
   * @return GUID
   */
  ...
  last_received_message_tm() const {
...
private:
  //! Store the last timestamp it was received a RTPS message from the remote
  //! participant.
