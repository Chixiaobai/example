...
public:
  RTPS_DllAPI ReaderProxyData(
      ...
      const fastdds::rtps::ContentFilterProperty::AllocationConfiguration
          &content_filter_limits = {});
  ...
  RTPS_DllAPI bool has_locators() const {
    return !remote_locators_.unicast.empty() ||
           !remote_locators_.multicast.empty();
  }
  ...
  void set_remote_unicast_locators(const LocatorList_t &locators,
                                   const NetworkFactory &network);

  RTPS_DllAPI void add_multicast_locator(const Locator_t &locator);

  void set_multicast_locators(const LocatorList_t &locators,
                              const NetworkFactory &network);

  void set_locators(const RemoteLocatorList &locators);

  void set_remote_locators(const RemoteLocatorList &locators,
                           const NetworkFactory &network,
  ...
  RTPS_DllAPI void
  RTPSParticipantKey(const InstanceHandle_t &RTPSParticipantKey) {
  ...
  RTPS_DllAPI void
  content_filter(const fastdds::rtps::ContentFilterProperty &filter) {
    content_filter_ = filter;
  }

  RTPS_DllAPI void
  content_filter(fastdds::rtps::ContentFilterProperty &&filter) {
    content_filter_ = std::move(filter);
  }

  RTPS_DllAPI const fastdds::rtps::ContentFilterProperty &
  content_filter() const {
  ...
  RTPS_DllAPI void type_id(const TypeIdV1 &other_type_id) {
  ...
  RTPS_DllAPI bool has_type_id() const { return m_type_id != nullptr; }

  RTPS_DllAPI void type(const TypeObjectV1 &other_type) { type() = other_type; }
  ...
  RTPS_DllAPI bool has_type() const { return m_type != nullptr; }

  RTPS_DllAPI void
  type_information(const xtypes::TypeInformation &other_type_information) {
  ...
  void set_sample_identity(const SampleIdentity &sid) {
    fastdds::dds::set_proxy_property(sid, "PID_CLIENT_SERVER_KEY",
                                     m_properties);
  }
  ...
  SampleIdentity get_sample_identity() const {
    return fastdds::dds::get_proxy_property<SampleIdentity>(
        "PID_CLIENT_SERVER_KEY", m_properties);
  }

  /**
   * Get the size in bytes of the CDR serialization of this object.
   * @param include_encapsulation Whether to include the size of the
   * encapsulation info.
  ...
  /**
   * Read the information from a CDRMessage_t. The position of the message must
   * be in the beginning on the parameter list.
   * @param msg Pointer to the message.
   * @param network Reference to network factory for locator validation and
   * transformation
   * @param is_shm_transport_available Indicates whether the Reader is reachable
   * by SHM.
   * @return true on success
   */
