...
public:
  ...
  public:
    ...
    //! Bit index for matching failing due to inconsistent topic (same topic
    //! name but different characteristics)
   ...
   */
  virtual void assignRemoteEndpoints(const ParticipantProxyData &pdata,
  ...
  virtual bool areRemoteEndpointsMatched(const ParticipantProxyData *) {
    return false;
  }
   ...
   * @return True if correct.
   */
  virtual bool processLocalReaderProxyData(RTPSReader *reader,
   ...
   * @return True if correct.
   */
  virtual bool processLocalWriterProxyData(RTPSWriter *writer,
  ...
  bool newLocalReaderProxyData(
      RTPSReader *R, const TopicAttributes &att, const ReaderQos &qos,
   ...
   * @return True if correct.
   */
  bool newLocalWriterProxyData(RTPSWriter *W, const TopicAttributes &att,
                               const WriterQos &qos);
  /**
   ...
   * @return True if correctly updated
   */
  bool updatedLocalWriter(RTPSWriter *W, const TopicAttributes &att,
                          const WriterQos &qos);

  /**
   * Check the validity of a matching between a local RTPSWriter and a
   ...
   */
  bool validMatching(const WriterProxyData *wdata,
                     const ReaderProxyData *rdata);

  /**
   * Check the validity of a matching between a local RTPSReader and a
   * WriterProxyData object.
   * @param rdata Pointer to the ReaderProxyData object of the local RTPSReader.
   * @param wdata Pointer to the WriterProxyData object.
   * @return True if the two can be matched.
   */
  bool validMatching(const ReaderProxyData *rdata,
   ...
   * @param wdata Pointer to the WriterProxyData object of the local RTPSWriter.
   * @param rdata Pointer to the ReaderProxyData object.
   * @param [out] reason On return will specify the reason of failed matching
   * (if any).
   * @param [out] incompatible_qos On return will specify all the QoS values
   * that were incompatible (if any).
   * @return True if the two can be matched.
   */
  bool valid_matching(const WriterProxyData *wdata,
                      const ReaderProxyData *rdata, MatchingFailureMask &reason,
   ...
   * @return True if the two can be matched.
   */
  bool valid_matching(const ReaderProxyData *rdata,
                      const WriterProxyData *wdata, MatchingFailureMask &reason,
                      fastdds::dds::PolicyMask &incompatible_qos);
   ...
   * @param removed_by_lease Whether the writer is being unpaired due to a
   * participant drop.
   * @return True if correct.
   */
  bool unpairWriterProxy(const GUID_t &participant_guid,
                         const GUID_t &writer_guid, bool removed_by_lease);
   ...
   * @return True if correct.
   */
  bool unpairReaderProxy(const GUID_t &participant_guid,
   ...
   */
  bool
  pairing_reader_proxy_with_any_local_writer(const GUID_t &participant_guid,
                                             ReaderProxyData *rdata);

#if HAVE_SECURITY
  bool
  pairing_reader_proxy_with_local_writer(const GUID_t &local_writer,
   ...
   */
  bool
  pairing_writer_proxy_with_any_local_reader(const GUID_t &participant_guid,
                                             WriterProxyData *wdata);

#if HAVE_SECURITY
  bool
  pairing_writer_proxy_with_local_reader(const GUID_t &local_reader,
                                         const GUID_t &remote_participant_guid,
                                         WriterProxyData &wdata);
  ...
  update_subscription_matched_status(const GUID_t &reader_guid,
                                     const GUID_t &writer_guid, int change);

  const fastdds::dds::PublicationMatchedStatus &
  update_publication_matched_status(const GUID_t &reader_guid,
                                    const GUID_t &writer_guid, int change);
...
private:
   ...
   * @return True
   */
  bool pairingReader(RTPSReader *R, const GUID_t &participant_guid,
                     const ReaderProxyData &rdata);
  /**
   ...
   * @return True
   */
  bool pairingWriter(RTPSWriter *W, const GUID_t &participant_guid,
                     const WriterProxyData &wdata);

  bool checkDataRepresentationQos(const WriterProxyData *wdata,
                                  const ReaderProxyData *rdata) const;

  bool checkTypeValidation(const WriterProxyData *wdata,
                           const ReaderProxyData *rdata) const;

  bool checkTypeIdentifier(const WriterProxyData *wdata,
                           const ReaderProxyData *rdata) const;

  bool hasTypeIdentifier(const WriterProxyData *wdata,
                         const ReaderProxyData *rdata) const;

  bool checkTypeObject(const WriterProxyData *wdata,
                       const ReaderProxyData *rdata) const;

  bool hasTypeObject(const WriterProxyData *wdata,
                     const ReaderProxyData *rdata) const;
  ...
  foonathan::memory::map<GUID_t, fastdds::dds::SubscriptionMatchedStatus,
                         pool_allocator_t>
      reader_status_;
  foonathan::memory::map<GUID_t, fastdds::dds::PublicationMatchedStatus,
                         pool_allocator_t>
      writer_status_;
};
