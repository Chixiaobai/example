...
public:
  ...
  //! Pointer to the Publications Writer (only created if indicated in the
  //! DiscoveryAtributes).
  t_p_StatefulWriter publications_writer_;
  //! Pointer to the Subscriptions Writer (only created if indicated in the
  //! DiscoveryAtributes).
  t_p_StatefulWriter subscriptions_writer_;
  //! Pointer to the Publications Reader (only created if indicated in the
  //! DiscoveryAtributes).
  t_p_StatefulReader publications_reader_;
  //! Pointer to the Subscriptions Reader (only created if indicated in the
  //! DiscoveryAtributes).
   ...
   * @return true if correct.
   */
  bool processLocalReaderProxyData(RTPSReader *reader,
   ...
   * @return true if correct.
   */
  bool processLocalWriterProxyData(RTPSWriter *writer,
...
protected:
   ...
   */
  virtual void
  set_builtin_reader_history_attributes(HistoryAttributes &attributes);
   ...
   */
  virtual void
  set_builtin_writer_history_attributes(HistoryAttributes &attributes);
  ...
  /**
   * Create a cache change on a builtin writer and serialize a WriterProxyData
   * on it.
   * @param [in] data The WriterProxyData object to be serialized.
   * @param [in] writer The writer,history pair where the change should be
   * added.
   * @param [in] remove_same_instance Should previous changes with same key be
   * removed?
   * @param [out] created_change Where the pointer to the created change should
   * be returned.
   * @return false if data could not be serialized into the created change.
   */
  bool serialize_writer_proxy_data(const WriterProxyData &data,
                                   const t_p_StatefulWriter &writer,
                                   bool remove_same_instance,
                                   CacheChange_t **created_change);

  /**
   * Create a cache change on a builtin writer and serialize a ReaderProxyData
   * on it.
   ...
   * @return false if data could not be serialized into the created change.
   */
  bool serialize_reader_proxy_data(const ReaderProxyData &data,
                                   const t_p_StatefulWriter &writer,
                                   bool remove_same_instance,
                                   CacheChange_t **created_change);

  //! Process the info recorded in the persistence database
  void processPersistentData(t_p_StatefulReader &reader,
                             t_p_StatefulWriter &writer, key_list &demises);

  /**
   * Get a pointer pair of the corresponding writer builtin endpoint for the
   * entity_id
   * @param [in] entity_id The entity_id to obtain the pair from.
   * @return A pair of nullptrs if operation was unsuccessful
   */
  t_p_StatefulWriter
  get_builtin_writer_history_pair_by_entity(const EntityId_t &entity_id);

  /**
   * Get a pointer pair of the corresponding reader builtin endpoint for the
   * entity_id. If a builtin writer Entity is passed, the equivalent reader
   * entity builtin is returned.
   * @param [in] entity_id The entity_id to obtain the pair from.
   * @return A pair of nullptrs if operation was unsuccessful
   */
  t_p_StatefulReader
  get_builtin_reader_history_pair_by_entity(const EntityId_t &entity_id);
...
private:
  ...
  bool serialize_proxy_data(const ProxyData &data,
                            const t_p_StatefulWriter &writer,
                            bool remove_same_instance,
                            CacheChange_t **created_change);
