...
/**
 * Class PDPListener, specification used by the PDP to perform the History check
 * when a new message is received. This class is implemented in order to use the
 * same structure than with any other RTPSReader.
...
protected:
   ...
   * This method is called from PDPListener::onNewCacheChangeAdded() when an
   * alive DATA(p) is received. Both the reader lock and the PDP lock are held
   * when this method is called.
   *
   * @param  old_data     Pointer to the ParticipantProxyData currently stored
   * in the database. May be nullptr, for a recently discovered participant.
   ...
   * @param  lock         Lock on the PDP database. Passed so it can be released
   * before invoking callbacks.
   */
  virtual void process_alive_data(ParticipantProxyData *old_data,
