...
/**
 * Abstract class PDP that implements the basic interfaces for all Participant
 *Discovery implementations It also keeps the Participant Discovery Data and
 *provides interfaces to access it
...
public:
  ...
  virtual void
  initializeParticipantProxyData(ParticipantProxyData *participant_data);
  ...
  createParticipantProxyData(const ParticipantProxyData &p,
                             const GUID_t &writer_guid) = 0;

  /**
   * Force the sending of our local DPD to all remote RTPSParticipants and
   * multicast Locators.
   * @param new_change If true a new change (with new seqNum) is created and
  ...
  virtual void announceParticipantState(
      bool new_change, bool dispose = false,
   ...
   * @param [out] participant_guid  GUID of the ParticipantProxyData where the
   * reader was added.
   * @param [in]  initializer_func  Function to be called in order to set the
   * data of the ReaderProxyData.
   *
   * @return A pointer to the added ReaderProxyData (nullptr if it could not be
   * added).
   */
  ReaderProxyData *addReaderProxyData(
      const GUID_t &reader_guid, GUID_t &participant_guid,
      std::function<bool(ReaderProxyData *, bool, const ParticipantProxyData &)>
          initializer_func);
   ...
   * writer was added.
   * @param [in]  initializer_func  Function to be called in order to set the
   * data of the WriterProxyData.
   *
   * @return A pointer to the added WriterProxyData (nullptr if it could not be
   * added).
   */
  WriterProxyData *addWriterProxyData(
      const GUID_t &writer_guid, GUID_t &participant_guid,
      std::function<bool(WriterProxyData *, bool, const ParticipantProxyData &)>
          initializer_func);

  /**
   * This method returns whether a ReaderProxyDataObject exists among the
   * registered RTPSParticipants (including the local RTPSParticipant).
  ...
  /**
   * This method gets a copy of a ReaderProxyData object if it is found among
   * the registered RTPSParticipants (including the local RTPSParticipant).
   * @param [in]  reader  GUID_t of the reader we are looking for.
   * @param [out] rdata   Reference to the ReaderProxyData object where data is
   * to be returned.
   * @return True if found.
   */
  ...
  /**
   * This method returns whether a WriterProxyData exists among the registered
  ...
  /**
   * This method gets a copy of a WriterProxyData object if it is found among
   * the registered RTPSParticipants (including the local RTPSParticipant).
   * @param [in]  writer  GUID_t of the writer we are looking for.
   * @param [out] wdata   Reference to the WriterProxyData object where data is
   * to be returned.
   * @return True if found.
   */
  ...
  /**
   * This method returns the name of a participant if it is found among the
   ...
   */
  virtual void notifyAboveRemoteEndpoints(const ParticipantProxyData &pdata,
   ...
   */
  virtual bool
  remove_remote_participant(const GUID_t &participant_guid,
  ...
  ParticipantProxyData *getLocalParticipantProxyData() const {
    return participant_proxies_.empty() ? nullptr
                                        : participant_proxies_.front();
  }
   ...
   */
  ResourceLimitedVector<ParticipantProxyData *>::const_iterator
   ...
   * @param remote_guid GuidPrefix_t of the participant whose liveliness is
   * being asserted.
   */
   ...
   * @param guid_prefix The GUID prefix of the participant of which the proxy
   * data is retrieved
   * @return A pointer to the ParticipantProxyData. nullptr if there is no such
   * ParticipantProxyData
   */
  ParticipantProxyData *
  get_participant_proxy_data(const GuidPrefix_t &guid_prefix);
   ...
   */
  std::list<eprosima::fastdds::rtps::RemoteServerAttributes> &
  remote_server_attributes();
...
#if HAVE_SECURITY
  void add_builtin_security_attributes(ReaderAttributes &ratt,
...
protected:
   ...
   *
   * @param participant_guid GUID of the participant for which to create the
   * proxy object.
   * @param with_lease_duration indicates whether lease duration event should be
   * created.
   * @param participant_proxy_data The participant proxy data from which the
   * copy is made (if provided)
   *
   * @return pointer to the currently inserted entry, nullptr if allocation
   * limits were reached.
   */
  ParticipantProxyData *add_participant_proxy_data(
      const GUID_t &participant_guid, bool with_lease_duration,
      const ParticipantProxyData *participant_proxy_data = nullptr);

  /**
   * Checks whether two participant prefixes are equal by calculating the
   * mangled GUID and comparing it with the remote participant prefix.
   *
   ...
   */
  bool data_matches_with_prefix(const GuidPrefix_t &guid_prefix,
   ...
   */
  bool lookup_participant_key(const GUID_t &participant_guid,
  ...
  void announceParticipantState(
      RTPSWriter &writer, WriterHistory &history, bool new_change,
      bool dispose = false,
      WriteParams &wparams = WriteParams::WRITE_PARAM_DEFAULT);

  /**
   * Called after creating the builtin endpoints to update the metatraffic
   * unicast locators of BuiltinProtocols
   */
  virtual void update_builtin_locators() = 0;

  void notify_and_maybe_ignore_new_participant(ParticipantProxyData *pdata,
...
private:
  ...
  void
  check_remote_participant_liveliness(ParticipantProxyData *remote_participant);

  void check_and_notify_type_discovery(RTPSParticipantListener *listener,
  ...
  void check_and_notify_type_discovery(
      RTPSParticipantListener *listener, const string_255 &topic_name,
      const string_255 &type_name, const types::TypeIdentifier *type_id,
  ...
  /**
   * Performs all the necessary actions after removing a ParticipantProxyData
   * from the participant_proxies_ collection.
   *
  ...
  void actions_on_remote_participant_removed(
      ParticipantProxyData *pdata, const GUID_t &partGUID,
      ParticipantDiscoveryInfo::DISCOVERY_STATUS reason,
