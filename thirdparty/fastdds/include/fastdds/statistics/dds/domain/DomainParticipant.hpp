...
public:
   ...
   * @return RETCODE_UNSUPPORTED if the FASTDDS_STATISTICS CMake option has not
   * been set, RETCODE_BAD_PARAMETER if the topic name provided does not
   * correspond to any Statistics DataWriter, RETCODE_INCONSISTENT_POLICY if the
   * DataWriterQos provided is inconsistent, RETCODE_OK if the DataWriter has
   * been created or if it has been created previously, and RETCODE_ERROR
   * otherwise
   */
   ...
   * @return RETCODE_UNSUPPORTED if the FASTDDS_STATISTICS CMake option has not
   * been set, RETCODE_BAD_PARAMETER if the topic name provided does not
   * correspond to any Statistics DataWriter, RETCODE_OK if the DataWriter has
   * been correctly deleted or does not exist, and RETCODE_ERROR otherwise
   */
  RTPS_DllAPI ReturnCode_t
  disable_statistics_datawriter(const std::string &topic_name);

  /**
   * @brief This operation narrows the DDS DomainParticipant to the Statistics
   * DomainParticipant
   ...
   * nullptr otherwise.
   */
  RTPS_DllAPI static DomainParticipant *
  narrow(eprosima::fastdds::dds::DomainParticipant *domain_participant);

  /**
   * @brief This operation narrows the DDS DomainParticipant to the Statistics
   * DomainParticipant
   * @param domain_participant Constant reference to the DDS DomainParticipant
   * @return Constant reference to the Statistics DomainParticipant if
   * successful. nullptr otherwise.
   */
  RTPS_DllAPI static const DomainParticipant *
  narrow(const eprosima::fastdds::dds::DomainParticipant *domain_participant);
};
