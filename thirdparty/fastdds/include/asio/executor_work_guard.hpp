...
template <typename Executor, typename = void> class executor_work_guard;
...
public:
  ...
  executor_work_guard(executor_work_guard &&other) ASIO_NOEXCEPT
      : executor_(ASIO_MOVE_CAST(Executor)(other.executor_)),
        owns_(other.owns_) {
    if (owns_) {
      new (&work_) work_type(ASIO_MOVE_CAST(work_type)(
...
private:
  ...
  typedef typename decay<typename prefer_result<
      const executor_type &,
      execution::outstanding_work_t::tracked_t>::type>::type work_type;
