...
template <typename T, typename Vs, typename = void> struct set_value_member;
...
struct set_value_member_trait<
    ...
    typename void_type<decltype(
        declval<T>().set_value(declval<Vs>()...))>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);

  using result_type = decltype(declval<T>().set_value(declval<Vs>()...));

  ASIO_STATIC_CONSTEXPR(
      bool, is_noexcept = noexcept(declval<T>().set_value(declval<Vs>()...)));
};
...
template <typename T>
struct set_value_member_trait<
    T, void(), typename void_type<decltype(declval<T>().set_value())>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);

  using result_type = decltype(declval<T>().set_value());

  ASIO_STATIC_CONSTEXPR(bool, is_noexcept = noexcept(declval<T>().set_value()));
};
...
#define ASIO_PRIVATE_SET_VALUE_MEMBER_TRAIT_DEF(n)                             \
  ...
  struct set_value_member_trait<                                               \
      ...
      typename void_type<decltype(                                             \
          declval<T>().set_value(ASIO_VARIADIC_DECLVAL(n)))>::type> {          \
    ASIO_STATIC_CONSTEXPR(bool, is_valid = true);                              \
                                                                               \
    using result_type =                                                        \
...
struct set_value_member_trait<T, void(Vs...)>
    : conditional<
          ...
          typename conditional<
              ...
              no_set_value_member,
              traits::set_value_member<typename add_const<T>::type,
                                       void(Vs...)>>::type,
          traits::set_value_member<typename remove_reference<T>::type,
...
struct set_value_member_default : detail::set_value_member_trait<T, Vs> {};

template <typename T, typename Vs, typename>
struct set_value_member : set_value_member_default<T, Vs> {};
