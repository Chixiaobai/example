...
struct bulk_execute_member_trait<
    ...
    typename void_type<decltype(
        declval<T>().bulk_execute(declval<F>(), declval<N>()))>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);

  using result_type =
      decltype(declval<T>().bulk_execute(declval<F>(), declval<N>()));

  ASIO_STATIC_CONSTEXPR(bool, is_noexcept = noexcept(declval<T>().bulk_execute(
                                  declval<F>(), declval<N>())));
};
...
struct bulk_execute_member_trait
    : conditional<
          ...
          traits::bulk_execute_member<typename remove_reference<T>::type,
...
template <typename T, typename F, typename N, typename>
struct bulk_execute_member : bulk_execute_member_default<T, F, N> {};
