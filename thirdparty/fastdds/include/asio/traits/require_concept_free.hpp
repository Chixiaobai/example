...
struct require_concept_free_trait<
    ...
    typename void_type<decltype(
        require_concept(declval<T>(), declval<Property>()))>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);

  using result_type =
      decltype(require_concept(declval<T>(), declval<Property>()));

  ASIO_STATIC_CONSTEXPR(bool, is_noexcept = noexcept(require_concept(
                                  declval<T>(), declval<Property>())));
};
...
struct require_concept_free_trait
    : conditional<
          ...
          traits::require_concept_free<typename decay<T>::type,
