...
#if defined(ASIO_HAS_DECLTYPE) && defined(ASIO_HAS_NOEXCEPT) &&                \
...
struct set_value_free_trait<T, void(Vs...),
                            typename void_type<decltype(set_value(
                                declval<T>(), declval<Vs>()...))>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);

  using result_type = decltype(set_value(declval<T>(), declval<Vs>()...));
...
struct set_value_free_trait<
    T, void(), typename void_type<decltype(set_value(declval<T>()))>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);

  using result_type = decltype(set_value(declval<T>()));

  ASIO_STATIC_CONSTEXPR(bool, is_noexcept = noexcept(set_value(declval<T>())));
};

#define ASIO_PRIVATE_SET_VALUE_FREE_TRAIT_DEF(n)                               \
  template <typename T, ASIO_VARIADIC_TPARAMS(n)>                              \
  struct set_value_free_trait<                                                 \
      T, void(ASIO_VARIADIC_TARGS(n)),                                         \
      typename void_type<decltype(                                             \
          set_value(declval<T>(), ASIO_VARIADIC_DECLVAL(n)))>::type> {         \
    ASIO_STATIC_CONSTEXPR(bool, is_valid = true);                              \
                                                                               \
    using result_type =                                                        \
        decltype(set_value(declval<T>(), ASIO_VARIADIC_DECLVAL(n)));           \
                                                                               \
    ASIO_STATIC_CONSTEXPR(bool, is_noexcept = noexcept(set_value(              \
                                    declval<T>(), ASIO_VARIADIC_DECLVAL(n)))); \
  };                                                                           \
  /**/
...
template <typename T, typename Vs, typename = void> struct set_value_free_trait;
...
struct set_value_free_trait<T, void(Vs...)>
    : conditional<
          ...
          typename conditional<
              is_same<T, typename add_const<T>::type>::value, no_set_value_free,
              traits::set_value_free<typename add_const<T>::type,
                                     void(Vs...)>>::type,
          traits::set_value_free<typename remove_reference<T>::type,
                                 void(typename decay<Vs>::type...)>>::type {};
...
struct set_value_free_trait<T, void()>
    : conditional<
          is_same<T, typename remove_reference<T>::type>::value,
          typename conditional<is_same<T, typename add_const<T>::type>::value,
                               no_set_value_free,
                               traits::set_value_free<
                                   typename add_const<T>::type, void()>>::type,
          traits::set_value_free<typename remove_reference<T>::type,
                                 void()>>::type {};
...
#define ASIO_PRIVATE_SET_VALUE_MEMBER_TRAIT_DEF(n)                             \
  ...
  struct set_value_free_trait<T, void(ASIO_VARIADIC_TARGS(n))>                 \
      : conditional<                                                           \
            ...
            traits::set_value_free<typename remove_reference<T>::type,         \
                                   void(ASIO_VARIADIC_DECAY(n))>>::type {};    \
  /**/
...
struct set_value_free_default : detail::set_value_free_trait<T, Vs> {};

template <typename T, typename Vs, typename>
struct set_value_free : set_value_free_default<T, Vs> {};
