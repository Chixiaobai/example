...
template <typename T, typename = void> struct schedule_free;
...
struct schedule_free_trait<
    T, typename void_type<decltype(schedule(declval<T>()))>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);

  using result_type = decltype(schedule(declval<T>()));

  ASIO_STATIC_CONSTEXPR(bool, is_noexcept = noexcept(schedule(declval<T>())));
};
...
struct schedule_free_trait
    : conditional<
          ...
          typename conditional<
              is_same<T, typename add_const<T>::type>::value, no_schedule_free,
              traits::schedule_free<typename add_const<T>::type>>::type,
          traits::schedule_free<typename remove_reference<T>::type>>::type {};
