...
public:
  ...
  basic_socket_acceptor(
      ...
      typename enable_if<is_convertible<Protocol1, Protocol>::value &&
                         is_convertible<Executor1, Executor>::value>::type * =
          0)
      : impl_(std::move(other.impl_)) {}
  ...
  async_accept(
    ...
    return async_initiate<AcceptHandler, void(asio::error_code)>(
        initiate_async_accept(this), handler, &peer,
   ...
   * @endcode
   */
  template <ASIO_COMPLETION_TOKEN_FOR(void(
      asio::error_code, typename Protocol::socket::template rebind_executor<
  ...
  accept(const Executor1 &ex,
         typename enable_if<is_executor<Executor1>::value ||
                            execution::is_executor<Executor1>::value>::type * =
             0) {
    asio::error_code ec;
  ...
  async_accept(
    ...
    return async_initiate<MoveAcceptHandler,
        ...
        initiate_async_move_accept(this), handler, ex,
  ...
  async_accept(
    ...
    return async_initiate<MoveAcceptHandler,
                          void(asio::error_code, other_socket_type)>(
        initiate_async_move_accept(this), handler, context.get_executor(),
        &peer_endpoint, static_cast<other_socket_type *>(0));
  }
#endif // defined(ASIO_HAS_MOVE) || defined(GENERATING_DOCUMENTATION)
...
private:
  ...
  public:
    ...
    void operator()(ASIO_MOVE_ARG(MoveAcceptHandler) handler,
                    const Executor1 &peer_ex, endpoint_type *peer_endpoint,
                    Socket *) const {
      // If you get an error on the following line it means that your handler
