...
public:
  ...
  require(execution::blocking_t::possibly_t) const {
    return basic_executor_type(io_context_, allocator_,
                               bits_ & ~blocking_never);
  }
  ...
  require(execution::blocking_t::never_t) const {
    return basic_executor_type(io_context_, allocator_, bits_ | blocking_never);
  }
  ...
  require(execution::allocator_t<OtherAllocator> a) const {
    return basic_executor_type<OtherAllocator, Bits>(io_context_, a.value(),
                                                     bits_);
  }
  ...
  friend bool operator==(const basic_executor_type &a,
    ...
    return a.io_context_ == b.io_context_ && a.allocator_ == b.allocator_ &&
           a.bits_ == b.bits_;
  }
...
private:
  ...
  explicit basic_executor_type(io_context &i) ASIO_NOEXCEPT : io_context_(&i),
                                                              allocator_(),
                                                              bits_(0) {
    if (Bits & outstanding_work_tracked)
      io_context_->impl_.work_started();
  ...
  basic_executor_type(io_context *i, const Allocator &a,
                      unsigned int bits) ASIO_NOEXCEPT : io_context_(i),
                                                         allocator_(a),
                                                         bits_(bits) {
    if (Bits & outstanding_work_tracked)
      if (io_context_)
