...
public:
  ...
  bool owns_work() const ASIO_NOEXCEPT { return true; }
...
class handler_work_base<
    ...
    typename enable_if<!execution::is_executor<Executor>::value &&
                       (!is_same<Executor, PolymorphicExecutor>::value ||
                        !is_same<CandidateExecutor, void>::value)>::type> {
public:
  explicit handler_work_base(const Executor &ex) ASIO_NOEXCEPT
      : executor_(ex),
        owns_work_(true) {
  ...
  handler_work_base(const Executor &ex, const Executor &candidate) ASIO_NOEXCEPT
      : executor_(ex),
  ...
  bool owns_work() const ASIO_NOEXCEPT { return owns_work_; }
...
class handler_work_base<
    ...
    typename enable_if<
        is_same<Executor, typename IoContext::executor_type>::value>::type> {
public:
  explicit handler_work_base(const Executor &) {}

  bool owns_work() const ASIO_NOEXCEPT { return false; }
...
public:
  ...
  bool owns_work() const ASIO_NOEXCEPT { return !!executor_; }
