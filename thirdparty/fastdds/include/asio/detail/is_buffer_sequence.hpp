...
char (&buffer_sequence_begin_helper(
    T *t, typename enable_if<!is_same<decltype(asio::buffer_sequence_begin(*t)),
                                      void>::value>::type *))[2];

#else // defined(ASIO_HAS_DECLTYPE)

template <typename> char (&buffer_sequence_begin_helper(...))[2];
...
char (&buffer_sequence_end_helper(
    T *t, typename enable_if<!is_same<decltype(asio::buffer_sequence_end(*t)),
                                      void>::value>::type *))[2];

#else // defined(ASIO_HAS_DECLTYPE)

template <typename> char (&buffer_sequence_end_helper(...))[2];
...
char buffer_sequence_element_type_helper(
    ...
    typename enable_if<is_convertible<
        decltype(*asio::buffer_sequence_begin(*t)), Buffer>::value>::type *);
