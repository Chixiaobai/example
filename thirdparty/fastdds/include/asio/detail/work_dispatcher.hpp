...
struct is_work_dispatcher_required<
    Handler, Executor,
    typename enable_if<is_same<typename associated_executor<Handler, Executor>::
...
public:
  ...
  void operator()() {
    execution::execute(asio::prefer(executor_, execution::blocking.possibly,
                                    execution::allocator(
...
private:
  typedef typename decay<typename prefer_result<
      const Executor &, execution::outstanding_work_t::tracked_t>::type>::type
      work_executor_type;
