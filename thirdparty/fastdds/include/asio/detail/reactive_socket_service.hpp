...
public:
  ...
  size_t receive_from(implementation_type &impl,
    ...
    if (bufs_type::is_single_buffer) {
      bytes_recvd = socket_ops::sync_recvfrom1(
          impl.socket_, impl.state_, bufs_type::first(buffers).data(),
          bufs_type::first(buffers).size(), flags, sender_endpoint.data(),
          &addr_len, ec);
    } else {
      bufs_type bufs(buffers);
