...
public:
  ...
  /// Construct a buffer to represent a given memory range.
  const_buffer(const void *data, std::size_t size) ASIO_NOEXCEPT : data_(data),
                                                                   size_(size) {
  }
...
inline const mutable_buffer *buffer_sequence_begin(
    ...
    typename enable_if<is_convertible<const MutableBuffer *,
                                      const mutable_buffer *>::value>::type * =
        0) ASIO_NOEXCEPT {
...
inline const const_buffer *buffer_sequence_begin(
    const ConstBuffer &b,
    typename enable_if<
        is_convertible<const ConstBuffer *, const const_buffer *>::value>::type
        * = 0) ASIO_NOEXCEPT {
...
template <bool IsConst> struct buffer_types_base;

template <> struct buffer_types_base<false> {
  typedef mutable_buffer buffer_type;
  typedef ASIO_MUTABLE_BUFFER container_type;
};

template <> struct buffer_types_base<true> {
...
dynamic_buffer(std::basic_string<Elem, Traits, Allocator> &data) ASIO_NOEXCEPT {
...
std::size_t buffer_copy(multiple_buffers, multiple_buffers,
  ...
  while (total_bytes_copied != max_bytes_to_copy && target_iter != target_end &&
    ...
    std::size_t bytes_copied = (buffer_copy_1)(
        target_buffer,
        asio::buffer(source_buffer, max_bytes_to_copy - total_bytes_copied));
    total_bytes_copied += bytes_copied;
