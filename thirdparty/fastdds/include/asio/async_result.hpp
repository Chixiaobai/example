...
#if defined(ASIO_HAS_CONCEPTS) && defined(ASIO_HAS_VARIADIC_TEMPLATES) &&      \
...
public:
  ...
  static return_type initiate(ASIO_MOVE_ARG(Initiation) initiation,
    ...
    (initiation)(ASIO_MOVE_CAST(RawCompletionToken)(token),
...
struct async_result_has_initiate_memfn
    : integral_constant<
          bool, sizeof(async_result_initiate_memfn_helper<async_result<
                           typename decay<CompletionToken>::type, Signature>>(
                    0)) != 1> {};
...
template <typename CompletionToken, completion_signature Signature,
...
inline typename enable_if<
    ...
    ASIO_INITFN_DEDUCED_RESULT_TYPE(
        ...
        (async_result<typename decay<CompletionToken>::type, Signature>::
             initiate(declval<ASIO_MOVE_ARG(Initiation)>(),
...
async_initiate(ASIO_MOVE_ARG(Initiation) initiation,
               ...
               ASIO_MOVE_ARG(Args)... args) {
  return async_result<typename decay<CompletionToken>::type, Signature>::
      initiate(ASIO_MOVE_CAST(Initiation)(initiation),
...
#define ASIO_PRIVATE_INITIATE_DEF(n)                                           \
  ...
  async_initiate(ASIO_MOVE_ARG(Initiation) initiation,                         \
    ...
    ASIO_MOVE_CAST(Initiation)                                                 \
    (initiation)(                                                              \
        ASIO_MOVE_CAST(ASIO_HANDLER_TYPE(CompletionToken, Signature))(         \
            completion.completion_handler),                                    \
        ASIO_VARIADIC_MOVE_ARGS(n));                                           \
                                                                               \
    return completion.result.get();                                            \
...
template <typename Signature> struct initiation_archetype {
...
struct default_completion_token_impl<
    T, typename void_type<typename T::default_completion_token_type>::type> {
