...
write(SyncWriteStream &s, const ConstBufferSequence &buffers,
  ...
  return detail::write_buffer_sequence(
...
inline std::size_t write(
    ...
    typename enable_if<
        ...
        !is_dynamic_buffer_v2<typename decay<DynamicBuffer_v1>::type>::value>::
        type *) {
  return write(s, ASIO_MOVE_CAST(DynamicBuffer_v1)(buffers), transfer_all(),
               ec);
}
...
template <typename SyncWriteStream, typename DynamicBuffer_v2,
          typename CompletionCondition>
std::size_t write(
    SyncWriteStream &s, DynamicBuffer_v2 buffers,
    CompletionCondition completion_condition, asio::error_code &ec,
...
template <typename SyncWriteStream, typename DynamicBuffer_v2>
inline std::size_t write(
    SyncWriteStream &s, DynamicBuffer_v2 buffers, asio::error_code &ec,
    typename enable_if<is_dynamic_buffer_v2<DynamicBuffer_v2>::value>::type *) {
  return write(s, ASIO_MOVE_CAST(DynamicBuffer_v2)(buffers), transfer_all(),
               ec);
}
...
public:
  ...
  write_op(write_op &&other)
      : detail::base_from_completion_cond<CompletionCondition>(ASIO_MOVE_CAST(
            detail::base_from_completion_cond<CompletionCondition>)(other)),
...
template <typename AsyncWriteStream, typename ConstBufferSequence,
          typename ConstBufferIterator, typename CompletionCondition,
          typename WriteHandler>
inline void start_write_buffer_sequence_op(
    AsyncWriteStream &stream, const ConstBufferSequence &buffers,
...
struct associated_allocator<
  ...
  static type get(const detail::write_op<AsyncWriteStream, ConstBufferSequence,
                                         ConstBufferIterator,
...
inline ASIO_INITFN_AUTO_RESULT_TYPE(WriteHandler,
    ...
    async_write(
        ...
        ASIO_MOVE_ARG(WriteHandler) handler,
        typename enable_if<is_dynamic_buffer_v1<
                               typename decay<DynamicBuffer_v1>::type>::value &&
                           !is_dynamic_buffer_v2<typename decay<
                               DynamicBuffer_v1>::type>::value>::type *) {
  return async_write(s, ASIO_MOVE_CAST(DynamicBuffer_v1)(buffers),
...
inline ASIO_INITFN_AUTO_RESULT_TYPE(WriteHandler,
    ...
    async_write(
        ...
        typename enable_if<is_dynamic_buffer_v2<DynamicBuffer_v2>::value>::type
            *) {
  return async_write(s, ASIO_MOVE_CAST(DynamicBuffer_v2)(buffers),
                     transfer_all(), ASIO_MOVE_CAST(WriteHandler)(handler));
}
