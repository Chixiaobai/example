...
inline Iterator
call_connect_condition(ConnectCondition &connect_condition,
...
Iterator
connect(basic_socket<Protocol, Executor> &s, Iterator begin,
...
public:
  ...
  template <typename IteratorConnectHandler, typename Iterator,
            typename ConnectCondition>
  void operator()(ASIO_MOVE_ARG(IteratorConnectHandler) handler, Iterator begin,
                  Iterator end,
                  const ConnectCondition &connect_condition) const {
    // If you get an error on the following line it means that your
    // handler does not meet the documented type requirements for an
...
struct associated_allocator<
    ...
    Allocator> {
  typedef
      typename associated_allocator<RangeConnectHandler, Allocator>::type type;

  static type
  get(const detail::range_connect_op<Protocol, Executor, EndpointSequence,
                                     ConnectCondition, RangeConnectHandler> &h,
      const Allocator &a = Allocator()) ASIO_NOEXCEPT {
    return associated_allocator<RangeConnectHandler, Allocator>::get(h.handler_,
                                                                     a);
  }
};
...
struct associated_executor<
    detail::range_connect_op<Protocol, Executor, EndpointSequence,
                             ConnectCondition, RangeConnectHandler>,
    Executor1>
    : detail::associated_executor_forwarding_base<RangeConnectHandler,
                                                  Executor1> {
  typedef
      typename associated_executor<RangeConnectHandler, Executor1>::type type;

  static type
  get(const detail::range_connect_op<Protocol, Executor, EndpointSequence,
                                     ConnectCondition, RangeConnectHandler> &h,
      const Executor1 &ex = Executor1()) ASIO_NOEXCEPT {
    return associated_executor<RangeConnectHandler, Executor1>::get(h.handler_,
                                                                    ex);
  }
};
...
struct associated_allocator<
    detail::iterator_connect_op<Protocol, Executor, Iterator, ConnectCondition,
                                IteratorConnectHandler>,
    Allocator> {
  typedef typename associated_allocator<IteratorConnectHandler, Allocator>::type
      type;

  static type get(const detail::iterator_connect_op<Protocol, Executor,
...
struct associated_executor<
    detail::iterator_connect_op<Protocol, Executor, Iterator, ConnectCondition,
                                IteratorConnectHandler>,
    Executor1>
    : detail::associated_executor_forwarding_base<IteratorConnectHandler,
                                                  Executor1> {
  typedef typename associated_executor<IteratorConnectHandler, Executor1>::type
      type;
  ...
  static type get(const detail::iterator_connect_op<Protocol, Executor,
                                                    ...
                                                    IteratorConnectHandler> &h,
                  const Executor1 &ex = Executor1()) ASIO_NOEXCEPT {
    return associated_executor<IteratorConnectHandler, Executor1>::get(
        h.handler_, ex);
  }
};
...
inline ASIO_INITFN_AUTO_RESULT_TYPE(RangeConnectHandler,
  ...
  return async_initiate<RangeConnectHandler,
      ...
      detail::initiate_async_range_connect<Protocol, Executor>(s), handler,
...
inline ASIO_INITFN_AUTO_RESULT_TYPE(IteratorConnectHandler,
  ...
  return async_initiate<IteratorConnectHandler,
                        void(asio::error_code, Iterator)>(
      detail::initiate_async_iterator_connect<Protocol, Executor>(s), handler,
...
inline ASIO_INITFN_AUTO_RESULT_TYPE(IteratorConnectHandler,
    ...
    async_connect(basic_socket<Protocol, Executor> &s, Iterator begin,
                  Iterator end, ASIO_MOVE_ARG(IteratorConnectHandler) handler) {
  return async_initiate<IteratorConnectHandler,
                        void(asio::error_code, Iterator)>(
      detail::initiate_async_iterator_connect<Protocol, Executor>(s), handler,
