...
#define ASIO_PRIVATE_COMPOSED_WORK_DEF(n)                                      \
  ...
  struct composed_work<void(Head, ASIO_VARIADIC_TARGS(n))> {                   \
    typedef composed_io_executors<void(Head, ASIO_VARIADIC_TARGS(n))>          \
        executors_type;                                                        \
                                                                               \
    explicit composed_work(const executors_type &ex) ASIO_NOEXCEPT             \
        : head_(ex.head_),                                                     \
...
public:
  ...
  composed_op(ASIO_MOVE_ARG(I) impl, ASIO_MOVE_ARG(W) work,
              ASIO_MOVE_ARG(H) handler)
      : impl_(ASIO_MOVE_CAST(I)(impl)), work_(ASIO_MOVE_CAST(W)(work)),
        handler_(ASIO_MOVE_CAST(H)(handler)), invocations_(0) {}
  ...
  typedef typename associated_allocator<Handler, std::allocator<void>>::type
      allocator_type;
  ...
  template <typename... T> void operator()(ASIO_MOVE_ARG(T)... t) {
    if (invocations_ < ~0u)
      ++invocations_;
...
template <typename Function, typename Impl, typename Work, typename Handler,
          typename Signature>
...
public:
  ...
  void operator()(ASIO_MOVE_ARG(Handler) handler,
    ...
    composed_op<typename decay<Impl>::type, composed_work<Executors>,
        ...
        ASIO_MOVE_CAST(Impl)(impl), composed_work<Executors>(executors_),
...
make_initiate_composed_op(ASIO_MOVE_ARG(composed_io_executors<Executors>)
                              executors) {
  return initiate_composed_op<Signature, Executors>(
      0, ASIO_MOVE_CAST(composed_io_executors<Executors>)(executors));
}

template <typename IoObject>
inline typename IoObject::executor_type get_composed_io_executor(
    IoObject &io_object,
    typename enable_if<!is_executor<IoObject>::value &&
                       !execution::is_executor<IoObject>::value>::type * = 0) {
  ...
  return async_initiate<CompletionToken, Signature>(
      detail::make_initiate_composed_op<Signature>(
          detail::make_composed_io_executors(
              detail::get_composed_io_executor(ASIO_MOVE_CAST(
