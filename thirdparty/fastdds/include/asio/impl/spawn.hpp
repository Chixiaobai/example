...
template <typename Handler, typename T> class coro_handler {
public:
  coro_handler(basic_yield_context<Handler> ctx)
      : coro_(ctx.coro_.lock()), ca_(ctx.ca_), handler_(ctx.handler_),
        ready_(0), ec_(ctx.ec_), value_(0) {}
...
template <typename Handler> class coro_handler<Handler, void> {
public:
  coro_handler(basic_yield_context<Handler> ctx)
      : coro_(ctx.coro_.lock()), ca_(ctx.ca_), handler_(ctx.handler_),
        ready_(0), ec_(ctx.ec_) {}
...
template <typename Handler, typename T> class coro_async_result {
public:
...
template <typename Handler> class coro_async_result<Handler, void> {
public:
...
public:
  explicit async_result(
      typename detail::coro_async_result<Handler, void>::completion_handler_type
          &h)
...
struct spawn_data : private noncopyable {
  ...
  spawn_data(ASIO_MOVE_ARG(Hand) handler, bool call_handler,
             ASIO_MOVE_ARG(Func) function)
      : handler_(ASIO_MOVE_CAST(Hand)(handler)), call_handler_(call_handler),
...
template <typename Handler, typename Function> struct coro_entry_point {
...
template <typename Handler, typename Function> struct spawn_helper {
...
template <typename Handler, typename Function>
void spawn(
    ASIO_MOVE_ARG(Handler) handler, ASIO_MOVE_ARG(Function) function,
    const boost::coroutines::attributes &attributes,
    typename enable_if<
...
inline void
spawn(const Executor &ex, ASIO_MOVE_ARG(Function) function,
      const boost::coroutines::attributes &attributes,
      typename enable_if<is_executor<Executor>::value ||
                         execution::is_executor<Executor>::value>::type *) {
  asio::spawn(asio::strand<Executor>(ex), ASIO_MOVE_CAST(Function)(function),
              attributes);
}
...
inline void
spawn(ExecutionContext &ctx, ASIO_MOVE_ARG(Function) function,
      const boost::coroutines::attributes &attributes,
      typename enable_if<is_convertible<ExecutionContext &,
                                        execution_context &>::value>::type *) {
  asio::spawn(ctx.get_executor(), ASIO_MOVE_CAST(Function)(function),
              attributes);
}
