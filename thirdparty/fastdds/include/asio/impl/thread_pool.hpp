...
inline thread_pool::executor_type thread_pool::executor() ASIO_NOEXCEPT {
  return executor_type(*this);
}

inline thread_pool::scheduler_type thread_pool::scheduler() ASIO_NOEXCEPT {
...
thread_pool::basic_executor_type<Allocator, Bits> &
thread_pool::basic_executor_type<Allocator, Bits>::
operator=(basic_executor_type &&other) ASIO_NOEXCEPT {
  if (this != &other) {
    pool_ = other.pool_;
...
void thread_pool::basic_executor_type<Allocator, Bits>::do_execute(
    ASIO_MOVE_ARG(Function) f, false_type) const {
...
#if defined(ASIO_HAS_STD_EXCEPTION_PTR) && !defined(ASIO_NO_EXCEPTIONS)
    try {
...
template <typename Function>
void thread_pool::basic_executor_type<Allocator, Bits>::do_execute(
    ASIO_MOVE_ARG(Function) f, true_type) const {
...
void thread_pool::basic_executor_type<Allocator, Bits>::do_bulk_execute(
  ...
  for (std::size_t i = 0; i < n; ++i) {
    ...
    if ((bits_ & relationship_continuation) != 0) {
      ASIO_HANDLER_CREATION((*pool_, *p.p, "thread_pool", pool_, 0,
                             "bulk_execute(blk=never,rel=cont)"));
    } else {
      ASIO_HANDLER_CREATION((*pool_, *p.p, "thread_pool", pool_, 0,
                             "bulk)execute(blk=never,rel=fork)"));
    }

    p.v = p.p = 0;
  }

  pool_->scheduler_.post_immediate_completions(
      n, ops, (bits_ & relationship_continuation) != 0);
}
...
void thread_pool::basic_executor_type<Allocator, Bits>::do_bulk_execute(
  ...
  thread_pool_always_blocking_function_adapter<Function> adapter = {
      detail::addressof(f2.value), n};
...
template <typename Allocator, unsigned int Bits>
inline thread_pool &thread_pool::basic_executor_type<Allocator, Bits>::context()
    const ASIO_NOEXCEPT {
  return *pool_;
}
...
template <typename Allocator, unsigned int Bits>
inline void
thread_pool::basic_executor_type<Allocator, Bits>::on_work_finished() const
    ASIO_NOEXCEPT {
