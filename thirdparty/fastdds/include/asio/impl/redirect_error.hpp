...
template <typename R> struct redirect_error_signature<R(asio::error_code)> {
  typedef R type();
};
...
struct async_result<redirect_error_t<CompletionToken>, Signature> {
  typedef typename async_result<
      CompletionToken,
      typename detail::redirect_error_signature<Signature>::type>::return_type
      return_type;
  ...
  template <typename Initiation> struct init_wrapper {
    ...
    void operator()(ASIO_MOVE_ARG(Handler) handler,
      ...
      (initiation_)(
          detail::redirect_error_handler<typename decay<Handler>::type>(
...
struct associated_allocator<detail::redirect_error_handler<Handler>,
                            Allocator> {
