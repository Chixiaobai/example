...
public:
   ...
   */
  ASIO_CONSTEXPR
  as_single_t(default_constructor_tag = default_constructor_tag(),
              CompletionToken token = CompletionToken())
      : token_(ASIO_MOVE_CAST(CompletionToken)(token)) {}

  /// Constructor.
  template <typename T>
  ASIO_CONSTEXPR explicit as_single_t(ASIO_MOVE_ARG(T) completion_token)
  ...
  struct executor_with_default : InnerExecutor {
    ...
    executor_with_default(
        ...
        typename enable_if<
            is_convertible<OtherExecutor, InnerExecutor>::value>::type * = 0)
        ASIO_NOEXCEPT : InnerExecutor(ex) {}
  };
