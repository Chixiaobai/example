...
public:
  ...
  template <typename Arg> void operator()(ASIO_MOVE_ARG(Arg) arg) {
    handler_(ASIO_MOVE_CAST(Arg)(arg));
  }

  template <typename... Args> void operator()(ASIO_MOVE_ARG(Args)... args) {
...
struct async_result<experimental::as_single_t<CompletionToken>, Signature> {
  typedef
      typename async_result<CompletionToken,
                            typename experimental::detail::as_single_signature<
                                Signature>::type>::return_type return_type;
    ...
    void operator()(ASIO_MOVE_ARG(Handler) handler,
      ...
      (initiation_)(
          experimental::detail::as_single_handler<
              typename decay<Handler>::type>(ASIO_MOVE_CAST(Handler)(handler)),
...
struct associated_executor<experimental::detail::as_single_handler<Handler>,
                           Executor>
    : detail::associated_executor_forwarding_base<Handler, Executor> {
  typedef typename associated_executor<Handler, Executor>::type type;

  static type get(const experimental::detail::as_single_handler<Handler> &h,
...
struct associated_allocator<experimental::detail::as_single_handler<Handler>,
                            Allocator> {
