...
template <typename Executor, typename Receiver> struct as_operation {
  ...
  explicit as_operation(ASIO_MOVE_ARG(E) e, ASIO_MOVE_ARG(R) r)
      : ex_(ASIO_MOVE_CAST(E)(e)), receiver_(ASIO_MOVE_CAST(R)(r))
#if !defined(ASIO_HAS_MOVE)
        ,
  ...
  void start() ASIO_NOEXCEPT {
    ...
    try {
      ...
      execution::execute(
          ...
          as_invocable<typename remove_cvref<Receiver>::type, Executor>(
              receiver_
#if !defined(ASIO_HAS_MOVE)
              ,
              ref_count_
...
struct start_member<asio::execution::detail::as_operation<Executor, Receiver>> {
