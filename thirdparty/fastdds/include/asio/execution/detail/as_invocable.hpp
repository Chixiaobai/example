...
template <typename Receiver, typename> struct as_invocable {
  Receiver *receiver_;
  ...
  void operator()() ASIO_LVALUE_REF_QUAL ASIO_NOEXCEPT {
    ...
    try {
      ...
      execution::set_error(
          ASIO_MOVE_CAST(Receiver)(*receiver_),
...
template <typename Receiver, typename> struct as_invocable {
  ...
  explicit as_invocable(
      Receiver &r,
      const asio::detail::shared_ptr<asio::detail::atomic_count> &c)
      ASIO_NOEXCEPT : receiver_(asio::detail::addressof(r)),
                      ref_count_(c) {}
...
#if defined(ASIO_HAS_STD_EXCEPTION_PTR)
      execution::set_error(
          *receiver_, std::make_exception_ptr(receiver_invocation_error()));
      ++(*ref_count_);
#else  // defined(ASIO_HAS_STD_EXCEPTION_PTR)
      std::terminate();
#endif // defined(ASIO_HAS_STD_EXCEPTION_PTR)
...
template <typename T> struct is_as_invocable : false_type {};
