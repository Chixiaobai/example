...
template <typename Sender, typename Receiver> struct submit_receiver;

template <typename Sender, typename Receiver> struct submit_receiver_wrapper {
  ...
  set_value(ASIO_MOVE_ARG(Args)... args) ASIO_RVALUE_REF_QUAL
    ...
    execution::set_value(
        ASIO_MOVE_OR_LVALUE(typename remove_cvref<Receiver>::type)(p_->r_),
...
#define ASIO_PRIVATE_SUBMIT_RECEIVER_SET_VALUE_DEF(n)                          \
  ...
  typename enable_if<                                                          \
      is_receiver_of<Receiver, ASIO_VARIADIC_TARGS(n)>::value>::type           \
  set_value(ASIO_VARIADIC_MOVE_PARAMS(n)) ASIO_RVALUE_REF_QUAL                 \
  ASIO_NOEXCEPT_IF(                                                            \
      (is_nothrow_receiver_of<Receiver, ASIO_VARIADIC_TARGS(n)>::value)) {     \
    execution::set_value(                                                      \
        ASIO_MOVE_OR_LVALUE(typename remove_cvref<Receiver>::type)(p_->r_),    \
...
#undef ASIO_PRIVATE_SUBMIT_RECEIVER_SET_VALUE_DEF
  ...
  template <typename E>
  void set_error(ASIO_MOVE_ARG(E) e) ASIO_RVALUE_REF_QUAL ASIO_NOEXCEPT {
    execution::set_error(
        ASIO_MOVE_OR_LVALUE(typename remove_cvref<Receiver>::type)(p_->r_),
        ASIO_MOVE_CAST(E)(e));
    delete p_;
...
template <typename Sender, typename Receiver> struct submit_receiver {
  ...
  typename connect_result<
      Sender, submit_receiver_wrapper<Sender, Receiver>>::type state_;
#else  // defined(ASIO_HAS_MOVE)
  typename connect_result<
      Sender, const submit_receiver_wrapper<Sender, Receiver> &>::type state_;
  ...
  explicit submit_receiver(ASIO_MOVE_ARG(S) s, ASIO_MOVE_ARG(R) r)
      : r_(ASIO_MOVE_CAST(R)(r)),
        state_(execution::connect(
            ASIO_MOVE_CAST(S)(s),
            submit_receiver_wrapper<Sender, Receiver>(this))) {}
#else  // defined(ASIO_HAS_MOVE)
  explicit submit_receiver(Sender s, Receiver r)
      : r_(r), state_(execution::connect(
                   s, submit_receiver_wrapper<Sender, Receiver>(this))) {}
#endif // defined(ASIO_HAS_MOVE)
};
...
struct set_value_member<
  ...
  ASIO_STATIC_CONSTEXPR(
      bool,
      is_noexcept = asio::execution::is_nothrow_receiver_of<Receiver>::value);
  typedef void result_type;
};

#define ASIO_PRIVATE_SUBMIT_RECEIVER_TRAIT_DEF(n)                              \
  template <typename Sender, typename Receiver, ASIO_VARIADIC_TPARAMS(n)>      \
  struct set_value_member<                                                     \
      asio::execution::detail::submit_receiver_wrapper<Sender, Receiver>,      \
      void(ASIO_VARIADIC_TARGS(n))> {                                          \
    ASIO_STATIC_CONSTEXPR(bool, is_valid = true);                              \
    ASIO_STATIC_CONSTEXPR(                                                     \
        bool, is_noexcept = (asio::execution::is_nothrow_receiver_of<          \
...
struct set_error_member<
    asio::execution::detail::submit_receiver_wrapper<Sender, Receiver>, E> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
