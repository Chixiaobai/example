...
template <int I = 0> struct blocking_adaptation_t {
  ...
  ASIO_CONSTEXPR blocking_adaptation_t() : value_(-1) {}

  ASIO_CONSTEXPR blocking_adaptation_t(disallowed_t) : value_(0) {}

  ASIO_CONSTEXPR blocking_adaptation_t(allowed_t) : value_(1) {}
  ...
  static_query() ASIO_NOEXCEPT_IF((traits::query_static_constexpr_member<
                                   T, blocking_adaptation_t>::is_noexcept)) {
  ...
  static ASIO_CONSTEXPR
      ...
      static_query(
          typename enable_if<
              ...
              traits::static_query<T, disallowed_t>::is_valid>::type * = 0)
          ASIO_NOEXCEPT {
    return traits::static_query<T, disallowed_t>::value();
  }

  template <typename T>
  static ASIO_CONSTEXPR typename traits::static_query<T, allowed_t>::result_type
  static_query(typename enable_if<
                   !traits::query_static_constexpr_member<
                       T, blocking_adaptation_t>::is_valid &&
                   !traits::query_member<T, blocking_adaptation_t>::is_valid &&
                   !traits::static_query<T, disallowed_t>::is_valid &&
                   traits::static_query<T, allowed_t>::is_valid>::type * = 0)
      ASIO_NOEXCEPT {
  ...
  static ASIO_CONSTEXPR const
      T static_query_v = blocking_adaptation_t::static_query<E>();
#endif                  // defined(ASIO_HAS_DEDUCED_STATIC_QUERY_TRAIT)
                        //   && defined(ASIO_HAS_SFINAE_VARIABLE_TEMPLATES)

  friend ASIO_CONSTEXPR bool operator==(const blocking_adaptation_t &a,
  ...
  struct convertible_from_blocking_adaptation_t {
    ASIO_CONSTEXPR
    convertible_from_blocking_adaptation_t(blocking_adaptation_t) {}
  };

  template <typename Executor>
  friend ASIO_CONSTEXPR blocking_adaptation_t query(
      const Executor &ex, convertible_from_blocking_adaptation_t,
      typename enable_if<can_query<const Executor &, disallowed_t>::value>::type
          * = 0)
  ...
  query(const Executor &ex, convertible_from_blocking_adaptation_t,
        typename enable_if<!can_query<const Executor &, disallowed_t>::value &&
                           can_query<const Executor &, allowed_t>::value>::type
            * = 0)
#if !defined(__clang__) // Clang crashes if noexcept is used here.
#if defined(ASIO_MSVC)  // Visual C++ wants the type to be qualified.
      ASIO_NOEXCEPT_IF(
...
#else                   // defined(ASIO_MSVC)
      ASIO_NOEXCEPT_IF((is_nothrow_query<const Executor &, allowed_t>::value))
#endif                  // defined(ASIO_MSVC)
#endif                  // !defined(__clang__)
  {
...
template <int I = 0> struct disallowed_t {
  ...
  static ASIO_CONSTEXPR disallowed_t static_query(
      typename enable_if<
          ...
          !can_query<T, allowed_t<I>>::value>::type * = 0) ASIO_NOEXCEPT {
    return disallowed_t();
  }

  template <typename E, typename T = decltype(disallowed_t::static_query<E>())>
  static ASIO_CONSTEXPR const
      T static_query_v = disallowed_t::static_query<E>();
#endif // defined(ASIO_HAS_DEDUCED_STATIC_QUERY_TRAIT)
       //   && defined(ASIO_HAS_SFINAE_VARIABLE_TEMPLATES)
  ...
  friend ASIO_CONSTEXPR bool operator==(const disallowed_t &,
                                        const disallowed_t &) {
    return true;
  }
...
public:
  ...
  query(blocking_adaptation_t<I>) ASIO_NOEXCEPT {
    return allowed_t<I>();
  }

  template <int I>
  static ASIO_CONSTEXPR allowed_t<I> query(allowed_t<I>) ASIO_NOEXCEPT {
    return allowed_t<I>();
  }

  template <int I>
  static ASIO_CONSTEXPR allowed_t<I> query(disallowed_t<I>) ASIO_NOEXCEPT {
    return allowed_t<I>();
  }
  ...
  template <int I> Executor require(disallowed_t<I>) const ASIO_NOEXCEPT {
    return executor_;
  }
...
template <int I = 0> struct allowed_t {
  ...
  static ASIO_CONSTEXPR const T static_query_v = allowed_t::static_query<E>();
#endif // defined(ASIO_HAS_DEDUCED_STATIC_QUERY_TRAIT)
       //   && defined(ASIO_HAS_SFINAE_VARIABLE_TEMPLATES)

  static ASIO_CONSTEXPR blocking_adaptation_t<I> value() { return allowed_t(); }

  friend ASIO_CONSTEXPR bool operator==(const allowed_t &, const allowed_t &) {
    return true;
  }
...
public:
  ...
  blocking_execute_state(ASIO_MOVE_ARG(F) f)
      : func_(ASIO_MOVE_CAST(F)(f)), is_complete_(false) {}
...
void blocking_execute(ASIO_MOVE_ARG(Executor) ex,
...
namespace {
static const blocking_adaptation_t &blocking_adaptation =
...
struct query_free_default<
    ...
    typename enable_if<can_query<
        T, execution::blocking_adaptation_t::disallowed_t>::value>::type> {
...
struct query_free_default<
    ...
    typename enable_if<
        ...
        can_query<T,
                  execution::blocking_adaptation_t::allowed_t>::value>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  ASIO_STATIC_CONSTEXPR(
      bool,
...
struct static_query<T, execution::blocking_adaptation_t,
                    typename enable_if<traits::query_static_constexpr_member<
                        T, execution::blocking_adaptation_t>::is_valid>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  ASIO_STATIC_CONSTEXPR(bool, is_noexcept = true);

  typedef typename traits::query_static_constexpr_member<
      T, execution::blocking_adaptation_t>::result_type result_type;

  static ASIO_CONSTEXPR result_type value() {
    return traits::query_static_constexpr_member<
        T, execution::blocking_adaptation_t>::value();
  }
};
...
struct static_query<
    ...
    typename enable_if<
        !traits::query_static_constexpr_member<
            T, execution::blocking_adaptation_t>::is_valid &&
        !traits::query_member<T, execution::blocking_adaptation_t>::is_valid &&
        traits::static_query<T, execution::blocking_adaptation_t::
                                    disallowed_t>::is_valid>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  ASIO_STATIC_CONSTEXPR(bool, is_noexcept = true);
...
struct static_query<
    ...
    typename enable_if<
        ...
        traits::static_query<
            T, execution::blocking_adaptation_t::allowed_t>::is_valid>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  ASIO_STATIC_CONSTEXPR(bool, is_noexcept = true);
...
struct static_query<
    ...
    typename enable_if<traits::query_static_constexpr_member<
        T, execution::blocking_adaptation_t::disallowed_t>::is_valid>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  ASIO_STATIC_CONSTEXPR(bool, is_noexcept = true);
...
struct static_require<
  ...
  ASIO_STATIC_CONSTEXPR(
      bool,
      is_valid =
          (is_same<typename static_query<T, execution::blocking_adaptation_t::
                                                disallowed_t>::result_type,
...
struct require_free_default<
    ...
    typename enable_if<is_same<T, typename decay<T>::type>::value &&
                       execution::is_executor<T>::value>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
