...
template <typename ProtoAllocator> struct allocator_t {
  ...
  static ASIO_CONSTEXPR const T static_query_v = allocator_t::static_query<E>();
#endif // defined(ASIO_HAS_DEDUCED_STATIC_QUERY_TRAIT)
       //   && defined(ASIO_HAS_SFINAE_VARIABLE_TEMPLATES)

  ASIO_CONSTEXPR ProtoAllocator value() const { return a_; }

private:
  friend struct allocator_t<void>;

  explicit ASIO_CONSTEXPR allocator_t(const ProtoAllocator &a) : a_(a) {}
...
template <typename T> struct allocator_instance {
  static allocator_t<T> instance;
};

template <typename T> allocator_t<T> allocator_instance<T>::instance;
...
struct static_query<
    ...
    typename enable_if<traits::query_static_constexpr_member<
        T, execution::allocator_t<ProtoAllocator>>::is_valid>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  ASIO_STATIC_CONSTEXPR(bool, is_noexcept = true);

  typedef typename traits::query_static_constexpr_member<
      T, execution::allocator_t<ProtoAllocator>>::result_type result_type;

  static ASIO_CONSTEXPR result_type value() {
    return traits::query_static_constexpr_member<
        T, execution::allocator_t<ProtoAllocator>>::value();
  }
};
