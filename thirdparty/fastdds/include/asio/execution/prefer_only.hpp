...
struct prefer_only_is_preferable<
    InnerProperty, typename enable_if<InnerProperty::is_preferable>::type> {
...
struct prefer_only_polymorphic_query_result_type<
    InnerProperty,
    typename void_type<
        typename InnerProperty::polymorphic_query_result_type>::type> {
...
struct prefer_only_property<
    InnerProperty, typename void_type<decltype(
                       asio::declval<const InnerProperty>().value())>::type> {
  InnerProperty property;

  prefer_only_property(const InnerProperty &p) : property(p) {}

  ASIO_CONSTEXPR auto value() const
      ASIO_NOEXCEPT_IF((noexcept(asio::declval<const InnerProperty>().value())))
...
struct prefer_only
  ...
  static ASIO_CONSTEXPR const T static_query_v = prefer_only::static_query<E>();
#endif // defined(ASIO_HAS_DEDUCED_STATIC_QUERY_TRAIT)
       //   && defined(ASIO_HAS_SFINAE_VARIABLE_TEMPLATES)

  template <typename Executor, typename Property>
  friend ASIO_CONSTEXPR typename prefer_result<const Executor &,
                                               const InnerProperty &>::type
  prefer(const Executor &ex, const prefer_only<Property> &p,
         typename enable_if<
             is_same<Property, InnerProperty>::value &&
             can_prefer<const Executor &, const InnerProperty &>::value>::type
             * = 0)
  ...
  friend ASIO_CONSTEXPR
      ...
      query(const Executor &ex, const prefer_only<Property> &p,
            typename enable_if<
                is_same<Property, InnerProperty>::value &&
                can_query<const Executor &, const InnerProperty &>::value>::type
                * = 0)
#if !defined(ASIO_MSVC) &&                                                     \
    !defined(__clang__) // Clang crashes if noexcept is used here.
          ASIO_NOEXCEPT_IF((
...
struct prefer_free_default<
    ...
    typename enable_if<
        can_prefer<const T &, const InnerProperty &>::value>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  ASIO_STATIC_CONSTEXPR(
      bool, is_noexcept =
                (is_nothrow_prefer<const T &, const InnerProperty &>::value));

  typedef typename prefer_result<const T &, const InnerProperty &>::type
      result_type;
};
...
struct query_free<T, execution::prefer_only<InnerProperty>,
                  typename enable_if<can_query<
                      const T &, const InnerProperty &>::value>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  ASIO_STATIC_CONSTEXPR(
      bool, is_noexcept =
  ...
  typedef
      typename query_result<const T &, const InnerProperty &>::type result_type;
};
