...
struct can_connect : integral_constant<bool, automatically_determined> {};

/// A type trait to determine the result of a @c connect expression.
template <typename S, typename R> struct connect_result {
...
struct call_traits<
    ...
    typename enable_if<(
        ...
        is_sender<typename remove_cvref<S>::type>::value)>::type>
    : connect_member<S, R> {
...
struct call_traits<
    S, void(R),
    typename enable_if<(
        !connect_member<S, R>::is_valid && connect_free<S, R>::is_valid &&
...
struct call_traits<
    ...
    typename enable_if<(
        ...
        is_receiver<R>::value &&
        conditional<
            !is_as_receiver<typename remove_cvref<R>::type>::value,
            is_executor_of<typename remove_cvref<S>::type,
                           as_invocable<typename remove_cvref<R>::type, S>>,
            false_type>::type::value)>::type> {
...
struct impl {
  ...
  ASIO_CONSTEXPR
      ...
      operator()(S &&s, R &&r) const
      ASIO_NOEXCEPT_IF((call_traits<S, void(R)>::is_noexcept)) {
    return typename call_traits<S, void(R)>::result_type(ASIO_MOVE_CAST(S)(s),
                                                         ASIO_MOVE_CAST(R)(r));
  }
...
static ASIO_CONSTEXPR const asio_execution_connect_fn::impl &connect =
    asio_execution_connect_fn::static_instance<>::instance;
...
struct can_connect
    : integral_constant<
          bool, asio_execution_connect_fn::call_traits<S, void(R)>::overload !=
...
struct is_nothrow_connect
    : integral_constant<bool, asio_execution_connect_fn::call_traits<
...
template <typename S, typename R> struct connect_result {
  typedef
      typename asio_execution_connect_fn::call_traits<S, void(R)>::result_type
          type;
};
