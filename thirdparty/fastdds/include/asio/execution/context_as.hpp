...
template <typename T> struct context_as_t {
  ...
  static ASIO_CONSTEXPR const
      U static_query_v = context_as_t::static_query<E>();
#endif                  // defined(ASIO_HAS_DEDUCED_STATIC_QUERY_TRAIT)
                        //   && defined(ASIO_HAS_SFINAE_VARIABLE_TEMPLATES)

  template <typename Executor, typename U>
  friend ASIO_CONSTEXPR U
  query(const Executor &ex, const context_as_t<U> &,
        typename enable_if<
            is_same<T, U>::value &&
            can_query<const Executor &, const context_t &>::value>::type * = 0)
...
#if (defined(ASIO_HAS_VARIABLE_TEMPLATES) && defined(ASIO_HAS_CONSTEXPR)) ||   \
...
struct static_query<
    ...
    typename enable_if<static_query<T, execution::context_t>::is_valid>::type>
...
struct query_free<T, execution::context_as_t<U>,
                  typename enable_if<can_query<
                      const T &, const execution::context_t &>::value>::type> {
