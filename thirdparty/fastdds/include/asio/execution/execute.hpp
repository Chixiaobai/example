...
struct can_execute : integral_constant<bool, automatically_determined> {};
...
template <typename T, typename R> struct is_sender_to;
...
template <typename T, typename F, typename = void> struct call_traits {
  ASIO_STATIC_CONSTEXPR(overload_type, overload = ill_formed);
};

template <typename T, typename F>
struct call_traits<T, void(F),
                   typename enable_if<(execute_member<T, F>::is_valid)>::type>
    : execute_member<T, F> {
...
template <typename T, typename F>
struct call_traits<T, void(F),
                   typename enable_if<(!execute_member<T, F>::is_valid &&
                                       execute_free<T, F>::is_valid)>::type>
    : execute_free<T, F> {
...
struct call_traits<
    T, void(F),
    typename enable_if<(
        !execute_member<T, F>::is_valid && !execute_free<T, F>::is_valid &&
        conditional<true, true_type,
                    typename result_of<typename decay<F>::type &()>::type>::
            type::value &&
        conditional<!is_as_invocable<typename decay<F>::type>::value,
                    is_sender_to<T, as_receiver<typename decay<F>::type, T>>,
...
struct impl {
  template <typename T, typename F>
  ASIO_CONSTEXPR
      typename enable_if<call_traits<T, void(F)>::overload == call_member,
                         typename call_traits<T, void(F)>::result_type>::type
      operator()(ASIO_MOVE_ARG(T) t, ASIO_MOVE_ARG(F) f) const
      ASIO_NOEXCEPT_IF((call_traits<T, void(F)>::is_noexcept)) {
    return ASIO_MOVE_CAST(T)(t).execute(ASIO_MOVE_CAST(F)(f));
  }

  template <typename T, typename F>
  ASIO_CONSTEXPR
      typename enable_if<call_traits<T, void(F)>::overload == call_free,
                         typename call_traits<T, void(F)>::result_type>::type
      operator()(ASIO_MOVE_ARG(T) t, ASIO_MOVE_ARG(F) f) const
      ASIO_NOEXCEPT_IF((call_traits<T, void(F)>::is_noexcept)) {
  ...
  template <typename T, typename F>
  ASIO_CONSTEXPR
      typename enable_if<call_traits<T, void(F)>::overload == adapter,
                         typename call_traits<T, void(F)>::result_type>::type
      operator()(ASIO_MOVE_ARG(T) t, ASIO_MOVE_ARG(F) f) const
      ASIO_NOEXCEPT_IF((call_traits<T, void(F)>::is_noexcept)) {
    return asio::execution::detail::submit_helper(
        ASIO_MOVE_CAST(T)(t),
        as_receiver<typename decay<F>::type, T>(ASIO_MOVE_CAST(F)(f), 0));
  }
};
...
static ASIO_CONSTEXPR const asio_execution_execute_fn::impl &execute =
    asio_execution_execute_fn::static_instance<>::instance;
...
struct can_execute
    : integral_constant<
          bool, asio_execution_execute_fn::call_traits<T, void(F)>::overload !=
