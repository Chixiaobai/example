...
struct is_executor_of_impl_base
    : integral_constant<
          bool, conditional<true, true_type,
...
struct is_executor_of_impl
    : conditional<can_execute<typename add_const<T>::type, F>::value,
                  is_executor_of_impl_base<T, F>, false_type>::type {};
...
template <typename T>
struct executor_shape<T, typename void_type<typename T::shape_type>::type> {
  typedef typename T::shape_type type;
};

template <typename T, typename Default, typename = void> struct executor_index {
  typedef Default type;
};

template <typename T, typename Default>
struct executor_index<T, Default,
                      typename void_type<typename T::index_type>::type> {
...
template <typename T, typename F>
ASIO_CONSTEXPR const bool is_executor_of_v = is_executor_of<T, F>::value;
...
#define ASIO_EXECUTION_EXECUTOR_OF(f) ::asio::execution::executor_of<f>
...
template <typename T> using executor_shape_t = typename executor_shape<T>::type;
...
template <typename T> using executor_index_t = typename executor_index<T>::type;
