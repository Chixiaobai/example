 ...
 */
template <typename S>
struct can_schedule : integral_constant<bool, automatically_determined> {};
...
template <typename S, typename = void> struct call_traits {
...
template <typename S>
struct call_traits<S, typename enable_if<(schedule_member<S>::is_valid)>::type>
    : schedule_member<S> {
  ASIO_STATIC_CONSTEXPR(overload_type, overload = call_member);
};

template <typename S>
struct call_traits<S, typename enable_if<(!schedule_member<S>::is_valid &&
                                          schedule_free<S>::is_valid)>::type>
    : schedule_free<S> {
...
struct call_traits<
    S, typename enable_if<(
           !schedule_member<S>::is_valid && !schedule_free<S>::is_valid &&
           is_executor<typename decay<S>::type>::value)>::type> {
...
struct impl {
  template <typename S>
  ASIO_CONSTEXPR typename enable_if<call_traits<S>::overload == identity,
                                    typename call_traits<S>::result_type>::type
  operator()(ASIO_MOVE_ARG(S) s) const
      ASIO_NOEXCEPT_IF((call_traits<S>::is_noexcept)) {
    return ASIO_MOVE_CAST(S)(s);
  }

#if defined(ASIO_HAS_MOVE)
  template <typename S>
  ASIO_CONSTEXPR typename enable_if<call_traits<S>::overload == call_member,
                                    typename call_traits<S>::result_type>::type
  operator()(S &&s) const ASIO_NOEXCEPT_IF((call_traits<S>::is_noexcept)) {
    return ASIO_MOVE_CAST(S)(s).schedule();
  }

  template <typename S>
  ASIO_CONSTEXPR typename enable_if<call_traits<S>::overload == call_free,
                                    typename call_traits<S>::result_type>::type
  operator()(S &&s) const ASIO_NOEXCEPT_IF((call_traits<S>::is_noexcept)) {
  ...
  template <typename S>
  ASIO_CONSTEXPR
      typename enable_if<call_traits<S &>::overload == call_member,
                         typename call_traits<S &>::result_type>::type
      operator()(S &s) const ASIO_NOEXCEPT_IF((call_traits<S &>::is_noexcept)) {
    return s.schedule();
  }
  ...
  template <typename S>
  ASIO_CONSTEXPR
      typename enable_if<call_traits<const S &>::overload == call_member,
                         typename call_traits<const S &>::result_type>::type
      operator()(const S &s) const
      ASIO_NOEXCEPT_IF((call_traits<const S &>::is_noexcept)) {
    return s.schedule();
  }

  template <typename S>
  ASIO_CONSTEXPR
      typename enable_if<call_traits<S &>::overload == call_free,
                         typename call_traits<S &>::result_type>::type
      operator()(S &s) const ASIO_NOEXCEPT_IF((call_traits<S &>::is_noexcept)) {
    return schedule(s);
  }
  ...
  template <typename S>
  ASIO_CONSTEXPR
      typename enable_if<call_traits<const S &>::overload == call_free,
                         typename call_traits<const S &>::result_type>::type
      operator()(const S &s) const
      ASIO_NOEXCEPT_IF((call_traits<const S &>::is_noexcept)) {
    return schedule(s);
...
static ASIO_CONSTEXPR const asio_execution_schedule_fn::impl &schedule =
    asio_execution_schedule_fn::static_instance<>::instance;
...
template <typename S> constexpr bool can_schedule_v = can_schedule<S>::value;
...
struct is_nothrow_schedule
    : integral_constant<
          bool, asio_execution_schedule_fn::call_traits<S>::is_noexcept> {};
