...
template <int I = 0> struct outstanding_work_t {
  ...
  ASIO_CONSTEXPR outstanding_work_t() : value_(-1) {}

  ASIO_CONSTEXPR outstanding_work_t(untracked_t) : value_(0) {}

  ASIO_CONSTEXPR outstanding_work_t(tracked_t) : value_(1) {}
  ...
  static_query() ASIO_NOEXCEPT_IF((traits::query_static_constexpr_member<
                                   T, outstanding_work_t>::is_noexcept)) {
    return traits::query_static_constexpr_member<T,
                                                 outstanding_work_t>::value();
  }
  ...
  static ASIO_CONSTEXPR
      ...
      static_query(typename enable_if<
                       ...
                       traits::static_query<T, untracked_t>::is_valid>::type * =
                       0) ASIO_NOEXCEPT {
    return traits::static_query<T, untracked_t>::value();
  }

  template <typename T>
  static ASIO_CONSTEXPR typename traits::static_query<T, tracked_t>::result_type
  static_query(typename enable_if<
                   !traits::query_static_constexpr_member<
                       T, outstanding_work_t>::is_valid &&
                   !traits::query_member<T, outstanding_work_t>::is_valid &&
                   !traits::static_query<T, untracked_t>::is_valid &&
                   traits::static_query<T, tracked_t>::is_valid>::type * = 0)
      ASIO_NOEXCEPT {
  ...
  static ASIO_CONSTEXPR const
      T static_query_v = outstanding_work_t::static_query<E>();
#endif                  // defined(ASIO_HAS_DEDUCED_STATIC_QUERY_TRAIT)
                        //   && defined(ASIO_HAS_SFINAE_VARIABLE_TEMPLATES)

  friend ASIO_CONSTEXPR bool operator==(const outstanding_work_t &a,
  ...
  friend ASIO_CONSTEXPR outstanding_work_t query(
      ...
      typename enable_if<can_query<const Executor &, untracked_t>::value>::type
          * = 0)
      ...
      ASIO_NOEXCEPT_IF((is_nothrow_query<const Executor &, untracked_t>::value))
  ...
  template <typename Executor>
  friend ASIO_CONSTEXPR outstanding_work_t
  query(const Executor &ex, convertible_from_outstanding_work_t,
        typename enable_if<!can_query<const Executor &, untracked_t>::value &&
                           can_query<const Executor &, tracked_t>::value>::type
            * = 0)
#if !defined(__clang__) // Clang crashes if noexcept is used here.
#if defined(ASIO_MSVC)  // Visual C++ wants the type to be qualified.
      ASIO_NOEXCEPT_IF(
...
#else                   // defined(ASIO_MSVC)
      ASIO_NOEXCEPT_IF((is_nothrow_query<const Executor &, tracked_t>::value))
#endif                  // defined(ASIO_MSVC)
#endif                  // !defined(__clang__)
  {
...
#if !defined(ASIO_HAS_CONSTEXPR)
template <int I> const outstanding_work_t<I> outstanding_work_t<I>::instance;
#endif
...
template <int I = 0> struct untracked_t {
  ...
  static ASIO_CONSTEXPR untracked_t static_query(
      typename enable_if<
          ...
          !can_query<T, tracked_t<I>>::value>::type * = 0) ASIO_NOEXCEPT {
    return untracked_t();
  }

  template <typename E, typename T = decltype(untracked_t::static_query<E>())>
  static ASIO_CONSTEXPR const T static_query_v = untracked_t::static_query<E>();
#endif // defined(ASIO_HAS_DEDUCED_STATIC_QUERY_TRAIT)
       //   && defined(ASIO_HAS_SFINAE_VARIABLE_TEMPLATES)

  static ASIO_CONSTEXPR outstanding_work_t<I> value() { return untracked_t(); }

  friend ASIO_CONSTEXPR bool operator==(const untracked_t &,
                                        const untracked_t &) {
    return true;
  }
...
template <int I = 0> struct tracked_t {
  ...
  static ASIO_CONSTEXPR const T static_query_v = tracked_t::static_query<E>();
#endif // defined(ASIO_HAS_DEDUCED_STATIC_QUERY_TRAIT)
       //   && defined(ASIO_HAS_SFINAE_VARIABLE_TEMPLATES)

  static ASIO_CONSTEXPR outstanding_work_t<I> value() { return tracked_t(); }

  friend ASIO_CONSTEXPR bool operator==(const tracked_t &, const tracked_t &) {
    return true;
  }
...
namespace {
static const outstanding_work_t &outstanding_work =
    outstanding_work_t::instance;
}
#endif
...
struct query_free_default<
    ...
    typename enable_if<can_query<
        T, execution::outstanding_work_t::untracked_t>::value>::type> {
...
struct query_free_default<
    ...
    typename enable_if<
        ...
        can_query<T, execution::outstanding_work_t::tracked_t>::value>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  ASIO_STATIC_CONSTEXPR(
      bool,
      is_noexcept = (is_nothrow_query<
...
struct static_query<T, execution::outstanding_work_t,
                    typename enable_if<traits::query_static_constexpr_member<
                        T, execution::outstanding_work_t>::is_valid>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  ASIO_STATIC_CONSTEXPR(bool, is_noexcept = true);

  typedef typename traits::query_static_constexpr_member<
      T, execution::outstanding_work_t>::result_type result_type;

  static ASIO_CONSTEXPR result_type value() {
    return traits::query_static_constexpr_member<
        T, execution::outstanding_work_t>::value();
  }
};
...
struct static_query<
    ...
    typename enable_if<
        !traits::query_static_constexpr_member<
            T, execution::outstanding_work_t>::is_valid &&
        !traits::query_member<T, execution::outstanding_work_t>::is_valid &&
        traits::static_query<
            T, execution::outstanding_work_t::untracked_t>::is_valid>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  ASIO_STATIC_CONSTEXPR(bool, is_noexcept = true);
...
struct static_query<
    ...
    typename enable_if<
        ...
        traits::static_query<
            T, execution::outstanding_work_t::tracked_t>::is_valid>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  ASIO_STATIC_CONSTEXPR(bool, is_noexcept = true);
...
struct static_require<
  ...
  ASIO_STATIC_CONSTEXPR(
      bool,
      is_valid =
          (is_same<typename static_query<T, execution::outstanding_work_t::
                                                untracked_t>::result_type,
  ...
  ASIO_STATIC_CONSTEXPR(
      ...
      is_valid = (is_same<
                  typename static_query<
                      T, execution::outstanding_work_t::tracked_t>::result_type,
