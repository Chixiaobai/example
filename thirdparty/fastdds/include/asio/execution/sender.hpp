...
#include "asio/detail/push_options.hpp"
        ...
        defined(ASIO_HAS_VARIADIC_TEMPLATES) && defined(ASIO_HAS_DECLTYPE) &&  \
        !defined(ASIO_MSVC) ||                                                 \
    (_MSC_VER >= 1910)
...
struct sender_traits_base<
    S, typename enable_if<
           is_base_of<sender_base_ns::sender_base, S>::value>::type> {};
...
template <template <template <typename...> class Tuple,
                    template <typename...> class Variant> class>
struct has_value_types {
  typedef void type;
};

template <template <template <typename...> class Variant> class>
...
struct has_sender_types<S,
                        ...
                        typename conditional<S::sends_done, void, void>::type>
    : true_type {};

template <typename S>
struct sender_traits_base<
    S, typename enable_if<has_sender_types<S>::value>::type> {
...
struct sender_traits_base<
    S,
    typename enable_if<!has_sender_types<S>::value &&
                       detail::is_executor_of_impl<
                           S, as_invocable<void_receiver, S>>::value>::type> {
#if defined(ASIO_HAS_DEDUCED_EXECUTION_IS_TYPED_SENDER_TRAIT) &&               \
...
struct has_sender_traits<
    S, typename enable_if<
           is_same<typename asio::execution::sender_traits<
...
struct is_sender :
    ...
    conditional<
        ...
        is_move_constructible<typename remove_cvref<T>::type>, false_type>::type
#endif // defined(GENERATING_DOCUMENTATION)
{
};
...
template <typename T> ASIO_CONCEPT sender = is_sender<T>::value;
...
struct is_sender_to :
    ...
    integral_constant<bool, automatically_determined>
#else  // defined(GENERATING_DOCUMENTATION)
    integral_constant<bool, is_sender<T>::value && is_receiver<R>::value &&
...
template <typename T, typename R>
ASIO_CONSTEXPR const bool is_sender_to_v = is_sender_to<T, R>::value;
...
#define ASIO_EXECUTION_SENDER_TO(r) ::asio::execution::sender_to<r>
...
struct is_typed_sender :
    ...
    integral_constant<bool, automatically_determined>
#else  // defined(GENERATING_DOCUMENTATION)
    integral_constant<bool, is_sender<T>::value &&
                                detail::has_sender_types<sender_traits<
...
template <typename T> ASIO_CONCEPT typed_sender = is_typed_sender<T>::value;

#define ASIO_EXECUTION_TYPED_SENDER ::asio::execution::typed_sender
