...
struct is_scheduler_base
    : integral_constant<
          bool, is_copy_constructible<typename remove_cvref<T>::type>::value &&
                    traits::equality_comparable<
                        typename remove_cvref<T>::type>::is_valid> {};
...
struct is_scheduler :
    ...
    conditional<can_schedule<T>::value, detail::is_scheduler_base<T>,
                false_type>::type
...
template <typename T> ASIO_CONCEPT scheduler = is_scheduler<T>::value;
