 ...
 */
template <typename R>
struct can_start : integral_constant<bool, automatically_determined> {};
...
template <typename R>
struct call_traits<R, typename enable_if<(start_member<R>::is_valid)>::type>
    : start_member<R> {
  ASIO_STATIC_CONSTEXPR(overload_type, overload = call_member);
};

template <typename R>
struct call_traits<R, typename enable_if<(!start_member<R>::is_valid &&
                                          start_free<R>::is_valid)>::type>
    : start_free<R> {
...
static ASIO_CONSTEXPR const asio_execution_start_fn::impl &start =
    asio_execution_start_fn::static_instance<>::instance;
...
template <typename R> constexpr bool can_start_v = can_start<R>::value;
