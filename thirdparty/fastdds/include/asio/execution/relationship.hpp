...
template <int I = 0> struct relationship_t {
  ...
  ASIO_CONSTEXPR relationship_t() : value_(-1) {}

  ASIO_CONSTEXPR relationship_t(fork_t) : value_(0) {}

  ASIO_CONSTEXPR relationship_t(continuation_t) : value_(1) {}
  ...
  static_query() ASIO_NOEXCEPT_IF(
      (traits::query_static_constexpr_member<T, relationship_t>::is_noexcept)) {
    return traits::query_static_constexpr_member<T, relationship_t>::value();
  }

  template <typename T>
  static ASIO_CONSTEXPR typename traits::static_query<T, fork_t>::result_type
  static_query(
      typename enable_if<
          !traits::query_static_constexpr_member<T, relationship_t>::is_valid &&
          !traits::query_member<T, relationship_t>::is_valid &&
          traits::static_query<T, fork_t>::is_valid>::type * = 0)
      ASIO_NOEXCEPT {
    return traits::static_query<T, fork_t>::value();
  }

  template <typename T>
  static ASIO_CONSTEXPR typename traits::static_query<
      T, continuation_t>::result_type
  static_query(
      typename enable_if<
          !traits::query_static_constexpr_member<T, relationship_t>::is_valid &&
          !traits::query_member<T, relationship_t>::is_valid &&
          !traits::static_query<T, fork_t>::is_valid &&
          traits::static_query<T, continuation_t>::is_valid>::type * = 0)
      ASIO_NOEXCEPT {
  ...
  static ASIO_CONSTEXPR const
      T static_query_v = relationship_t::static_query<E>();
#endif                  // defined(ASIO_HAS_DEDUCED_STATIC_QUERY_TRAIT)
                        //   && defined(ASIO_HAS_SFINAE_VARIABLE_TEMPLATES)

  friend ASIO_CONSTEXPR bool operator==(const relationship_t &a,
  ...
  query(const Executor &ex, convertible_from_relationship_t,
        typename enable_if<can_query<const Executor &, fork_t>::value>::type * =
            0)
      ...
      ASIO_NOEXCEPT_IF((is_nothrow_query<const Executor &, fork_t>::value))
  ...
  query(const Executor &ex, convertible_from_relationship_t,
        typename enable_if<
            ...
            can_query<const Executor &, continuation_t>::value>::type * = 0)
#if !defined(__clang__) // Clang crashes if noexcept is used here.
#if defined(ASIO_MSVC)  // Visual C++ wants the type to be qualified.
      ASIO_NOEXCEPT_IF(
...
#if !defined(ASIO_HAS_CONSTEXPR)
template <int I> const relationship_t<I> relationship_t<I>::instance;
#endif
...
template <int I = 0> struct fork_t {
  ...
  static ASIO_CONSTEXPR fork_t static_query(
      typename enable_if<
          ...
          !can_query<T, continuation_t<I>>::value>::type * = 0) ASIO_NOEXCEPT {
    return fork_t();
  }

  template <typename E, typename T = decltype(fork_t::static_query<E>())>
  static ASIO_CONSTEXPR const T static_query_v = fork_t::static_query<E>();
#endif // defined(ASIO_HAS_DEDUCED_STATIC_QUERY_TRAIT)
       //   && defined(ASIO_HAS_SFINAE_VARIABLE_TEMPLATES)

  static ASIO_CONSTEXPR relationship_t<I> value() { return fork_t(); }

  friend ASIO_CONSTEXPR bool operator==(const fork_t &, const fork_t &) {
    return true;
  }
...
template <int I = 0> struct continuation_t {
  ...
  static ASIO_CONSTEXPR const
      T static_query_v = continuation_t::static_query<E>();
#endif // defined(ASIO_HAS_DEDUCED_STATIC_QUERY_TRAIT)
       //   && defined(ASIO_HAS_SFINAE_VARIABLE_TEMPLATES)

  static ASIO_CONSTEXPR relationship_t<I> value() { return continuation_t(); }

  friend ASIO_CONSTEXPR bool operator==(const continuation_t &,
...
struct query_free_default<
    ...
    typename enable_if<
        can_query<T, execution::relationship_t::fork_t>::value>::type> {
...
struct query_free_default<
    ...
    typename enable_if<
        ...
        can_query<T, execution::relationship_t::continuation_t>::value>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  ASIO_STATIC_CONSTEXPR(
      bool,
...
struct static_query<T, execution::relationship_t,
                    typename enable_if<traits::query_static_constexpr_member<
                        T, execution::relationship_t>::is_valid>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  ASIO_STATIC_CONSTEXPR(bool, is_noexcept = true);

  typedef typename traits::query_static_constexpr_member<
      T, execution::relationship_t>::result_type result_type;

  static ASIO_CONSTEXPR result_type value() {
    return traits::query_static_constexpr_member<
        T, execution::relationship_t>::value();
  }
};
...
struct static_query<
    ...
    typename enable_if<
        !traits::query_static_constexpr_member<
            T, execution::relationship_t>::is_valid &&
        !traits::query_member<T, execution::relationship_t>::is_valid &&
...
struct static_query<
    ...
    typename enable_if<
        ...
        !traits::static_query<T, execution::relationship_t::fork_t>::is_valid &&
        traits::static_query<
            T, execution::relationship_t::continuation_t>::is_valid>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  ASIO_STATIC_CONSTEXPR(bool, is_noexcept = true);
...
struct static_query<
    ...
    typename enable_if<traits::query_static_constexpr_member<
        T, execution::relationship_t::fork_t>::is_valid>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  ASIO_STATIC_CONSTEXPR(bool, is_noexcept = true);
  ...
  static ASIO_CONSTEXPR result_type value() {
    return traits::query_static_constexpr_member<
        T, execution::relationship_t::fork_t>::value();
  }
};
...
struct static_query<
    ...
    typename enable_if<
        !traits::query_static_constexpr_member<
            T, execution::relationship_t::fork_t>::is_valid &&
        !traits::query_member<T, execution::relationship_t::fork_t>::is_valid &&
        !traits::query_free<T, execution::relationship_t::fork_t>::is_valid &&
        !can_query<T,
                   execution::relationship_t::continuation_t>::value>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  ASIO_STATIC_CONSTEXPR(bool, is_noexcept = true);
...
struct static_require<
    T, execution::relationship_t::fork_t,
    typename enable_if<
        static_query<T, execution::relationship_t::fork_t>::is_valid>::type> {
  ASIO_STATIC_CONSTEXPR(
      bool, is_valid =
                (is_same<typename static_query<
                             T, execution::relationship_t::fork_t>::result_type,
...
struct static_require<
  ...
  ASIO_STATIC_CONSTEXPR(
      bool,
      is_valid =
          (is_same<typename static_query<T, execution::relationship_t::
                                                continuation_t>::result_type,
