...
public:
  ...
  /// Assignment operator.
  basic_executor_type &
  operator=(const basic_executor_type &other) ASIO_NOEXCEPT;

#if defined(ASIO_HAS_MOVE) || defined(GENERATING_DOCUMENTATION)
  /// Move assignment operator.
  basic_executor_type &operator=(basic_executor_type &&other) ASIO_NOEXCEPT;
#endif // defined(ASIO_HAS_MOVE) || defined(GENERATING_DOCUMENTATION)
   ...
   */
  ASIO_CONSTEXPR
  basic_executor_type<Allocator, ASIO_UNSPECIFIED(Bits & ~blocking_mask)>
   ...
   *   ... @endcode
   */
  static ASIO_CONSTEXPR execution::bulk_guarantee_t
  query(execution::bulk_guarantee_t) ASIO_NOEXCEPT {
   ...
   *   ... @endcode
   */
  static ASIO_CONSTEXPR execution::mapping_t
  query(execution::mapping_t) ASIO_NOEXCEPT {
   ...
   *   ... @endcode
   */
  ASIO_CONSTEXPR execution::blocking_t
  query(execution::blocking_t) const ASIO_NOEXCEPT {
   ...
   *   ... @endcode
   */
  ASIO_CONSTEXPR execution::relationship_t
  query(execution::relationship_t) const ASIO_NOEXCEPT {
   ...
   */
  template <typename OtherAllocator>
  ASIO_CONSTEXPR Allocator
  query(execution::allocator_t<OtherAllocator>) const ASIO_NOEXCEPT {
    return allocator_;
  }
   ...
   */
  ASIO_CONSTEXPR Allocator
  query(execution::allocator_t<void>) const ASIO_NOEXCEPT {
    return allocator_;
  }
   ...
   */
  template <typename Function> void execute(ASIO_MOVE_ARG(Function) f) const {
  ...
  void bulk_execute(ASIO_MOVE_ARG(Function) f, std::size_t n) const {
    this->do_bulk_execute(
        ASIO_MOVE_CAST(Function)(f), n,
        integral_constant<bool, (Bits & blocking_always) != 0>());
  }
   ...
   */
  sender_type schedule() const ASIO_NOEXCEPT { return *this; }
...
private:
  ...
  explicit basic_executor_type(thread_pool &p) ASIO_NOEXCEPT : pool_(&p),
                                                               allocator_(),
                                                               bits_(0) {
    if (Bits & outstanding_work_tracked)
      pool_->scheduler_.work_started();
  ...
  basic_executor_type(thread_pool *p, const Allocator &a,
                      unsigned int bits) ASIO_NOEXCEPT : pool_(p),
                                                         allocator_(a),
                                                         bits_(bits) {
    if (Bits & outstanding_work_tracked)
      if (pool_)
  ...
  template <typename Function>
  void do_bulk_execute(ASIO_MOVE_ARG(Function) f, std::size_t n,
                       false_type) const;
...
struct connect_member<
    const asio::thread_pool::basic_executor_type<Allocator, Bits>, Receiver> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  ASIO_STATIC_CONSTEXPR(bool, is_noexcept = false);
  typedef asio::execution::detail::as_operation<
      asio::thread_pool::basic_executor_type<Allocator, Bits>, Receiver>
      result_type;
};
...
struct query_static_constexpr_member<
    ...
    typename asio::enable_if<asio::is_convertible<
        Property, asio::execution::bulk_guarantee_t>::value>::type> {
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  ASIO_STATIC_CONSTEXPR(bool, is_noexcept = true);
