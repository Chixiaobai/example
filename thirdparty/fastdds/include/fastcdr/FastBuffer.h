...
public:
   ...
   * This operator makes the iterator point to the same position but in another
   * raw buffer. The new raw buffer is the same than the source iterator's.
   * @param iterator The source iterator. The iterator will use the source
   * iterator's raw buffer after this operation.
   */
  inline void operator<<(const _FastBuffer_iterator &iterator) {
     ...
     */
    inline void operator+=(size_t numBytes) { m_currentPosition += numBytes; }
     ...
     */
    inline char *operator&() { return m_currentPosition; }
...
public:
     ...
     */
    inline iterator begin() { return (iterator(m_buffer, 0)); }

    /*!
     * @brief This function returns a iterator that points to the end of the stream.
     * @return The new iterator.
     */
    inline iterator end() { return (iterator(m_buffer, m_bufferSize)); }
    ...
    /*!
     * @brief This function resizes the raw buffer. It will call the user's
     * defined function for this purpose.
