...
/**
 * Class Subscriber, contains the public API that allows the user to control the
 * reception of messages. This class should not be instantiated directly.
...
public:
   ...
   * @param timeout Maximum time the function will be blocked if any sample is
   * received.
   ...
   * @param info Pointer to a SampleInfo_t structure that informs you about your
   * sample.
   * @return True if a sample was read.
   * @note This method is blocked for a period of time.
   * ReliabilityQosPolicy.max_blocking_time on SubscriberAttributes defines this
   * period of time.
   */
  bool readNextData(void *sample, SampleInfo_t *info);

  /**
   * @brief Takes next sample from the Subscriber. The sample is removed from
   * the subscriber.
   * @param sample Pointer to the object where you want the sample stored.
   * @param info Pointer to a SampleInfo_t structure that informs you about your
   * sample.
   ...
   * @param att Reference to a SubscriberAttributes object to update the
   * parameters;
   ...
   * It occurs when the Subscriber received all samples sent by Publishers. In
   * other words, its WriterProxies are up to date.
   ...
   * @return Unread count
   */
  inline uint64_t getUnreadCount() const { return get_unread_count(); }
   ...
   *
   * @param [out] locators  LocatorList_t where the list of locators will be
   * stored.
   */
