...
/**
 * Class SubscriberListener, it should be used by the end user to implement
...
public:
  ...
  /**
   * Virtual function to be implemented by the user containing the actions to be
   * performed when a new  Data Message is received.
   * @param sub Subscriber
   */
  virtual void onNewDataMessage(Subscriber *sub) { (void)sub; }

  /**
   * Virtual method to be called when the subscriber is matched with a new
   * Writer (or unmatched); i.e., when a writer publishing in the same topic is
   * discovered.
   * @param sub Subscriber
   ...
   */
  virtual void
  on_requested_deadline_missed(Subscriber *sub,
  ...
  /**
   * @brief Method called when the liveliness status associated to a subscriber
   * changes
