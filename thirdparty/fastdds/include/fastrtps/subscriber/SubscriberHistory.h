...
/**
 * Class SubscriberHistory, container of the different CacheChanges of a
 * subscriber
...
public:
  ...
  SubscriberHistory(const TopicAttributes &topic_att,
                    ...
                    const fastrtps::ReaderQos &qos, uint32_t payloadMax,
   ...
   * @param release defaults to true and hints if the CacheChange_t should
   * return to the pool
   ...
   *
   * @param [in]  writer_guid                    GUID of the writer where the
   * change came from.
   * @param [in]  total_payload_size             Total payload size of the
   * incoming change.
   * @param [in]  unknown_missing_changes_up_to  The number of changes from the
   * same writer with a lower sequence number that could potentially be received
   * in the future.
   * @param [out] will_never_be_accepted         When the method returns @c
   * false, this parameter will inform whether the change could be accepted in
   * the future or not.
   *
   * @pre change should not be present in the history
   *
   * @return Whether a call to received_change will succeed when called with the
   * same arguments.
   */
  bool can_change_be_added_nts(const rtps::GUID_t &writer_guid,
                               uint32_t total_payload_size,
                               size_t unknown_missing_changes_up_to,
                               bool &will_never_be_accepted) const override;

  /**
   * Called when a change is received by the Subscriber. Will add the change to
   * the history.
   * @pre Change should not be already present in the history.
   * @param[in] change The received change
   * @param unknown_missing_changes_up_to Number of missing changes before this
   * one
   * @return
   */
   ...
   * @param data Pointer to the object where you want to read or take the
   * information.
   ...
   * @param [out] info Pointer to a SampleInfo_t structure to store first
   * untaken sample information.
   * @return true if sample info was returned. false if there is no sample to
   * take.
   */
   ...
   * @param [in,out] it     Iterator pointing to change on input. Will point to
   * next valid change on output.
   * @return True if removed.
   */
  ...
  /**
   * @brief A method to get the next instance handle that will miss the deadline
   * and the time when the deadline will occur
   * @param handle The handle to the instance
   * @param next_deadline_us The time point when the instance will miss the
   * deadline
   * @return True if the deadline was retrieved successfully
   */
  bool
  get_next_deadline(rtps::InstanceHandle_t &handle,
...
private:
  ...
  /**
   * @brief Method that finds a key in m_keyedChanges or tries to add it if not
   * found
   ...
   */
  bool find_key(rtps::CacheChange_t *a_change,
   ...
   * @return True if it was found or could be added to the map
   */
  bool find_key_for_change(rtps::CacheChange_t *a_change,
                           t_m_Inst_Caches::iterator &map_it);
   ...
   */
  ///@{
  bool received_change_keep_all_no_key(rtps::CacheChange_t *change,
                                       size_t unknown_missing_changes_up_to);

  bool received_change_keep_last_no_key(rtps::CacheChange_t *change,
                                        size_t unknown_missing_changes_up_to);

  bool received_change_keep_all_with_key(rtps::CacheChange_t *change,
                                         size_t unknown_missing_changes_up_to);

  bool received_change_keep_last_with_key(rtps::CacheChange_t *change,
                                          size_t unknown_missing_changes_up_to);

  bool completed_change_keep_all_with_key(rtps::CacheChange_t *change);

  bool completed_change_keep_last_with_key(rtps::CacheChange_t *change);
  ///@}

  bool add_received_change(rtps::CacheChange_t *a_change);

  bool add_received_change_with_key(
      rtps::CacheChange_t *a_change,
      std::vector<rtps::CacheChange_t *> &instance_changes);

  bool deserialize_change(rtps::CacheChange_t *change,
                          uint32_t ownership_strength, void *data,
                          SampleInfo_t *info);
};
