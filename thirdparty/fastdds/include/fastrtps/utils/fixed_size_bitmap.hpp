...
template <class T> struct DiffFunction {
  constexpr auto operator()(T a, T b) const -> decltype(a - b) { return a - b; }
};
...
public:
  ...
  BitmapRange(T base, uint32_t max_bits) noexcept
      : base_(base), range_max_(base_ + (std::min)(max_bits, NBITS - 1)),
        bitmap_(), num_bits_(0u) {}

  // We don't need to define copy/move constructors/assignment operators as the
  // default ones would be enough
   ...
   * This method resets the range and sets a new value for its base, as long as
   * a maximum number of bits.
   *
   * @param base      New base value to set.
   ...
   * This method implements a sliding window mechanism for changing the base of
   * the range.
   *
   * @param base   New base value to set.
   */
   ...
   *
   * @return the highest value set in the range. If the range is empty, the
   ...
   *
   * @return the lowest value set in the range. If the range is empty, the
   * result is undetermined.
   */
  T min() const noexcept {
    ...
    for (uint32_t i = 0; i < n_longs; i++) {
      ...
      if (bits) {
        ...
        // Most modern CPUs have an instruction to count the leading zeroes of a
        // word. The number of leading zeroes will give us the index we need.
#if _MSC_VER
        unsigned long bit;
   ...
   *
   * @return true if the item has been added (i.e. is in the allowed range),
   * false otherwise.
   */
  ...
  void add_range(const T &from, const T &to) {
    ...
    uint32_t bits_in_mask =
        32u - offset; // Take note of number of set bits in mask
    ...
    // This condition will be true if the last bits of the mask should not be
    // used
    if (n_bits > 0) {
   ...
   * This method is designed to be used when performing serialization of a
   * bitmap range.
   *
   * @param num_bits         Upon return, it will contain the number of
   * significant bits in the bitmap.
   * @param bitmap           Upon return, it will contain the current value of
   * the bitmap.
   * @param num_longs_used   Upon return, it will contain the number of valid
   * elements on the returned bitmap.
   */
  void bitmap_get(uint32_t &num_bits, bitmap_type &bitmap,
   ...
   * This method is designed to be used when performing deserialization of a
   * bitmap range.
   *
   * @param num_bits   Number of significant bits in the input bitmap.
   * @param bitmap     Points to the beginning of a uint32_t array holding the
   * input bitmap.
   */
   ...
   */
  template <class UnaryFunc> void for_each(UnaryFunc f) const {
    T item = base_;
  ...
  void shift_map_left(uint32_t n_bits) {
    if (n_bits >= num_bits_) {
      ...
      if (n_bits == 0) {
        ...
        // Example. Shifting 44 bits. Should shift one complete word and 12
        // bits. Need to iterate forward and take 12 bits from next word
        // (shifting it 20 bits). aaaaaaaa bbbbbbbb cccccccc dddddddd bbbbbccc
        // bbbbbbbb cccccccc dddddddd bbbbbccc cccccddd ddddd000 dddddddd
        ...
        for (size_t i = 0, n = n_items; n < last_index; i++, n++) {
          bitmap_[i] =
              (bitmap_[n] << n_bits) | (bitmap_[n + 1] >> overflow_bits);
        }
  ...
  void shift_map_right(uint32_t n_bits) {
    if (n_bits >= NBITS) {
      ...
      if (n_bits == 0) {
        ...
        // Example. Shifting 44 bits. Should shift one complete word and 12
        // bits. Need to iterate backwards and take 12 bits from previous word
        // (shifting it 20 bits). aaaaaaaa bbbbbbbb cccccccc dddddddd aaaaaaaa
        // bbbbbbbb cccccccc bbbccccc aaaaaaaa bbbbbbbb aaabbbbb bbbccccc
