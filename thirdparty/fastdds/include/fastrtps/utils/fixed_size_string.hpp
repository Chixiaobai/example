...
/**
 * Template class for non-alloc strings. Will be truncated when assigned from a
 * longer string.
 * @tparam MAX_CHARS Maximum number of characters is specified as the template
 * parameter. Space for an additional null terminator will be reserved.
 * @ingroup UTILITIES_MODULE
 */
template <size_t MAX_CHARS> struct fixed_string {
public:
  ...
  fixed_string &assign(const char *c_array, size_t n_chars) noexcept {
    string_len =
        (nullptr == c_array) ? 0 : (MAX_CHARS < n_chars) ? MAX_CHARS : n_chars;
    if (0 < string_len) {
...
private:
  void set(const char *c_string) noexcept {
    ...
    string_len =
        (result == nullptr) ? MAX_CHARS : (size_t)(result - string_data) - 1u;
  }

  char string_data[MAX_CHARS +
                   1]; ///< Holds string data, including ending null character.
