/*
   ...
   The original implementation has been modified to support the POSIX
   priorities:
    ...
    The C++ Standard has not yet (C++20) imposed any requirements on
   shared_mutex implementation thus each platform made its own choices: Windows
   & Boost defaults to PTHREAD_RWLOCK_PREFER_WRITER_NONRECURSIVE_NP. Linux & Mac
   defaults to PTHREAD_RWLOCK_PREFER_READER_NP.
 */
...
public:
  ...
  ~shared_mutex_base() { std::lock_guard<mutex_t> _(mut_); }
...
template <shared_mutex_type> class shared_mutex;

// original Hinnant implementation prioritizing writers

template <>
class shared_mutex<
    shared_mutex_type::PTHREAD_RWLOCK_PREFER_WRITER_NONRECURSIVE_NP>
...
template <class sm> class debug_wrapper : public sm {
  std::mutex wm_;
...
public:
  ~debug_wrapper() { std::lock_guard<std::mutex> _(wm_); }
...
// Detect if the shared_lock feature is available
    ...
    __has_include(                                                             \
        <version>) && !defined(__cpp_lib_shared_mutex) || /* deprecated        \
                                                             procedure if the  \
                                                             good one is not   \
                                                             available*/ \
...
template <class Mutex> void shared_lock<Mutex>::lock() {
  if (m_ == nullptr) {
...
template <class Mutex> bool shared_lock<Mutex>::try_lock() {
  if (m_ == nullptr) {
    throw std::system_error(std::error_code(EPERM, std::system_category()),
...
template <class Mutex> void shared_lock<Mutex>::unlock() {
  if (!owns_) {
    throw std::system_error(std::error_code(EPERM, std::system_category()),
...
#else
#warning                                                                       \
    "USE_THIRDPARTY_SHARED_MUTEX not defined. By default use framework version."
...
#if defined(__has_include) &&                                                  \
    __has_include(                                                             \
        <version>) && !defined(__cpp_lib_shared_mutex) || /* allow users to    \
                                                             ignore            \
                                                             shared_mutex      \
                                                             framework         \
                                                             implementation */ \
    (~USE_THIRDPARTY_SHARED_MUTEX + 1) || /* deprecated procedure if the good \
                                                           one is not          \
                                                available*/ \
    ( !(defined(__has_include) && __has_include(<version>)) && \
    !(defined(HAVE_CXX17) && HAVE_CXX17) &&  __cplusplus < 201703 )
...
#else
using shared_mutex = detail::debug_wrapper<detail::shared_mutex<
    detail::shared_mutex_type::PTHREAD_RWLOCK_PREFER_READER_NP>>;
#endif // NDEBUG
