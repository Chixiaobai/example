...
public:
   ...
   */
  RTPS_DllAPI static void createLocator(int32_t kindin,
                                        const std::string &address,
                                        uint32_t portin, Locator_t &locator);

  //! Sets locator's IPv4.
  RTPS_DllAPI static bool setIPv4(Locator_t &locator,
                                  const unsigned char *addr);

  //! Sets locator's IPv4.
  RTPS_DllAPI static bool setIPv4(Locator_t &locator, octet o1, octet o2,
                                  octet o3, octet o4);
  ...
  RTPS_DllAPI static bool setIPv4(Locator_t &destlocator,
                                  const Locator_t &origlocator);

  //! Copies locator's IPv4.
  RTPS_DllAPI static bool setIPv4address(Locator_t &destlocator,
  ...
  //! Copies locator's IPv4.
  RTPS_DllAPI static bool copyIPv4(const Locator_t &locator,
                                   unsigned char *dest);
  ...
  //! Sets locator's IPv6.
  RTPS_DllAPI static bool setIPv6(Locator_t &locator, uint16_t group0,
                                  uint16_t group1, uint16_t group2,
                                  uint16_t group3, uint16_t group4,
                                  uint16_t group5, uint16_t group6,
                                  uint16_t group7);
  ...
  RTPS_DllAPI static bool setIPv6(Locator_t &destlocator,
                                  const Locator_t &origlocator);
  ...
  //! Returns a string representation of the locator's IPv6 following RFC 5952
  //! recommendation.
  RTPS_DllAPI static std::string toIPv6string(const Locator_t &locator);

  //! Copies locator's IPv6.
  RTPS_DllAPI static bool copyIPv6(const Locator_t &locator,
                                   unsigned char *dest);
  ...
  RTPS_DllAPI static bool setWan(Locator_t &locator, octet o1, octet o2,
                                 octet o3, octet o4);
  ...
  //! Retrieves a string representation of the locator's WAN address (as in RTCP
  //! protocol)
  RTPS_DllAPI static std::string toWanstring(const Locator_t &locator);

  //! This method is useful in the case of having a tcp client with an initial
  //! peer pointing to a WAN locator, and receiving a locator with LAN and WAN
  //! addresses (TCP Client from TCP Server)
  RTPS_DllAPI static Locator_t WanToLanLocator(const Locator_t &locator);

  //! Sets locator's LAN ID (as in RTCP protocol)
  RTPS_DllAPI static bool setLanID(Locator_t &locator,
  ...
  //! Retrieves a string representation of the locator's LAN ID (as in RTCP
  //! protocol)
  ...
  //! Checks if a locator WAN address and IP address are the same (as in RTCP
  //! protocol).
  RTPS_DllAPI static bool ip_equals_wan(const Locator_t &locator);

  // Common
  //! Sets locator's RTCP port. Physical for UDP and logical for TCP (as in RTCP
  //! protocol)
  RTPS_DllAPI static bool setPortRTPS(Locator_t &locator, uint16_t port);

  //! Gets locator's RTCP port. Physical for UDP and logical for TCP (as in RTCP
  //! protocol)
  ...
  RTPS_DllAPI static bool compareAddress(const Locator_t &loc1,
                                         const Locator_t &loc2,
                                         bool fullAddress = false);

  //! Checks if a both locators has the same IP address and physical port  (as
  //! in RTCP protocol).
  RTPS_DllAPI static bool compareAddressAndPhysicalPort(const Locator_t &loc1,
                                                        const Locator_t &loc2);
  ...
  //! Resolve an address name by a DNS request and return the IP that this
  //! address references by a DNS server
  RTPS_DllAPI static std::pair<std::set<std::string>, std::set<std::string>>
  resolveNameDNS(const std::string &address_name);
