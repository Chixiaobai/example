...
inline int64_t Time_t2MicroSecondsInt64(const rtps::Time_t &t) {
  return (int64_t)(t.fraction() / pow(2.0, 32) * pow(10.0, 6)) +
         t.seconds() * (int64_t)pow(10.0, 6);
}
...
inline double Time_t2MicroSecondsDouble(const rtps::Time_t &t) {
  return ((double)t.fraction() / pow(2.0, 32) * pow(10.0, 6)) +
         (double)t.seconds() * pow(10.0, 6);
}
...
inline int64_t Time_t2MilliSecondsInt64(const rtps::Time_t &t) {
  return (int64_t)(t.fraction() / pow(2.0, 32) * pow(10.0, 3)) +
         t.seconds() * (int64_t)pow(10.0, 3);
}
...
inline double Time_t2MilliSecondsDouble(const rtps::Time_t &t) {
  return ((double)t.fraction() / pow(2.0, 32) * pow(10.0, 3)) +
         (double)t.seconds() * pow(10.0, 3);
}
...
inline rtps::Time_t MilliSeconds2Time_t(double millisec) {
  ...
  t.fraction((uint32_t)((millisec - (double)t.seconds() * pow(10.0, 3)) /
                        pow(10.0, 3) * pow(2.0, 32)));
  return t;
}
...
inline rtps::Time_t MicroSeconds2Time_t(double microsec) {
  ...
  t.fraction((uint32_t)((microsec - (double)t.seconds() * pow(10.0, 6)) /
                        pow(10.0, 6) * pow(2.0, 32)));
  return t;
}
 ...
 */
inline double Time_tAbsDiff2DoubleMillisec(const rtps::Time_t &t1,
                                           const rtps::Time_t &t2) {
  double result = 0;
  result += (double)abs((t2.seconds() - t1.seconds()) * 1000);
  result +=
      (double)std::abs((t2.fraction() - t1.fraction()) / pow(2.0, 32) * 1000);
  return result;
}

//! Create a random Time_t that is millisec + [-randoff,randoff]
inline rtps::Time_t MilliSecondsWithRandOffset2Time_t(double millisec,
                                                      double randoff) {
  randoff = std::abs(randoff);
  millisec = millisec + (-randoff) +
...
inline rtps::Time_t MicroSecondsWithRandOffset2Time_t(double microsec,
                                                      double randoff) {
  randoff = std::abs(randoff);
  microsec = microsec + (-randoff) +
             static_cast<double>(rand()) /
                 (static_cast<double>(RAND_MAX / (2 * randoff)));
