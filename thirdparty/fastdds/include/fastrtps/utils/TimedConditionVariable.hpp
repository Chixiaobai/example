...
/*
   NOTE: Windows implementation temporary disabled due to aleatory high CPU
 consumption when calling _Cnd_timedwait function, making some tests to fail and
 very poor performance. Related task: #6274
...
#define CV_INIT_(x)                                                            \
  pthread_condattr_init(&cv_attr_);                                            \
...
public:
  TimedConditionVariable() { CV_INIT_(&cv_); }

  template <typename Mutex>
  void wait(std::unique_lock<Mutex> &lock, std::function<bool()> predicate) {
    while (!predicate()) {
  ...
  bool wait_for(std::unique_lock<Mutex> &lock,
    ...
    while (ret_value && false == (ret_value = predicate())) {
      ret_value =
          (0 == CV_TIMEDWAIT_(cv_, lock.mutex()->native_handle(), &max_wait));
    }
  ...
  wait_until(std::unique_lock<Mutex> &lock,
             ...
             std::function<bool()> predicate) {
    auto secs =
        std::chrono::time_point_cast<std::chrono::seconds>(max_blocking_time);
    auto ns = std::chrono::time_point_cast<std::chrono::nanoseconds>(
                  max_blocking_time) -
    ...
    while (ret_value && false == (ret_value = predicate())) {
      ret_value =
          (CV_TIMEDWAIT_(cv_, lock.mutex()->native_handle(), &max_wait) == 0);
    }
  ...
  void notify_one() { CV_SIGNAL_(cv_); }

  void notify_all() { CV_BROADCAST_(cv_); }
