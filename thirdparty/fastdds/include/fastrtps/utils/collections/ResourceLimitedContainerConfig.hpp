...
struct ResourceLimitedContainerConfig {
  ...
  ResourceLimitedContainerConfig(
      ...
      size_t inc = 1u)
      : initial(ini), maximum(max), increment(inc) {}
   ...
   */
  inline static ResourceLimitedContainerConfig
  fixed_size_configuration(size_t size) {
    return ResourceLimitedContainerConfig(size, size, 0u);
  }

  /**
   * Return a resource limits configuration for a linearly growing, dynamically
   * allocated collection.
   * @param increment Number of new elements to allocate when increasing the
   * capacity of the collection.
   * @return Resource limits configuration.
   */
  inline static ResourceLimitedContainerConfig
  dynamic_allocation_configuration(size_t increment = 1u) {
    return ResourceLimitedContainerConfig(
        0u, std::numeric_limits<size_t>::max dummy_avoid_winmax(),
        increment ? increment : 1u);
  }
};

inline bool operator==(const ResourceLimitedContainerConfig &lhs,
                       const ResourceLimitedContainerConfig &rhs) {
  return lhs.maximum == rhs.maximum && lhs.initial == rhs.initial &&
