...
/**
 * A helper to calculate the block size of a memory pool given the size of the
 * node and a resource limits configuration.
 *
...
std::size_t
memory_pool_block_size(std::size_t node_size,
                       const ResourceLimitedContainerConfig &limits) {
  FASTDDS_DEPRECATED_UNTIL(3, "eprosima::fastrtps::memory_pool_block_size",
  ...
  return num_elems *
             ((node_size > MemoryPool::min_node_size
                   ? node_size
                   : MemoryPool::min_node_size) // Room for elements
#if FOONATHAN_MEMORY_DEBUG_DOUBLE_DEALLOC_CHECK
              * (fm::detail::debug_fence_size ? 3 : 1)) // Room for debug info
#else
              + (fm::detail::debug_fence_size ? 2 * fm::detail::max_alignment
                                              : 0)) // Room for debug info
#endif // if FOONATHAN_MEMORY_DEBUG_DOUBLE_DEALLOC_CHECK
#ifdef FOONATHAN_MEMORY_MEMORY_POOL_HAS_MIN_BLOCK_SIZE
         + fm::detail::memory_block_stack::implementation_offset(); // Room for
                                                                    // padding
#else
         + fm::detail::memory_block_stack::implementation_offset; // Room for
                                                                  // padding
