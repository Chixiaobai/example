...
std::array<P, sizeof...(Ints)> make_array(P &&i,
                                          std::index_sequence<Ints...> is) {
  return {(Ints == is.size() - 1 ? std::move(i) : i)...};
}

template <size_t N, class P> std::array<P, N> make_array(P &&i) {
...
template <bool, size_t N, class... Ts> struct make_array_choice {
  template <class P> static std::array<P, N> res(P &&i, Ts &&... args) {
    P tmp(i);
    return make_array<N>(std::move(i), std::move(tmp), std::move(args)...);
  }
};

template <size_t N, class... Ts> struct make_array_choice<true, N, Ts...> {
  template <class P> static std::array<P, N> res(P &&i, Ts &&... args) {
...
template <size_t N, class P, class... Ts>
std::array<P, N> make_array(P &&i, Ts &&... args) {
  return make_array_choice<N == (sizeof...(Ts) + 1), N, Ts...>::res(
      std::move(i), std::move(args)...);
}
...
template <class Proxy, std::size_t N = 4> class ProxyPool {
  ...
  public:
    void operator()(Proxy *p) const { pool_.set_back(p); }
...
public:
  ...
  ProxyPool(Proxy &&init)
      : heap_(make_array<N>(std::move(init))), deleter_(this) {
   ...
   * Note deleter is referenced not copied to avoid heap allocations on smart
   * pointer construction
   * @return unique_ptr referencing the proxy. On destruction the resource is
   * returned.
   */
