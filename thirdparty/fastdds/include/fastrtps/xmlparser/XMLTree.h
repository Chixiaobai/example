...
public:
  ...
  // Vs2013 partly support defaulted functions. Defaulted move constructors and
  // move assignment operators are not supported.
#if !defined(HAVE_CXX0X) || (defined(_MSC_VER) && _MSC_VER <= 1800)
  BaseNode(BaseNode &&other)
      : data_type_(std::move(other.data_type_)),
        parent_(std::move(other.parent_)), children(std::move(other.children)) {
  }
...
template <class T> class DataNode : public BaseNode {
public:
  ...
  DataNode(DataNode &&other)
      : BaseNode(std::move(other)), attributes_(std::move(other.attributes_)),
...
template <class T> DataNode<T>::~DataNode() {}

template <class T> T *DataNode<T>::get() const { return data_.get(); }

template <class T> std::unique_ptr<T> DataNode<T>::getData() {
  return std::move(data_);
}

template <class T> void DataNode<T>::setData(std::unique_ptr<T> data) {
  data_ = std::move(data);
}
