...
public:
   ...
   * @return XMLP_ret::XML_OK if all profiles are correct, XMLP_ret::XML_NOK if
   * some are and some are not, XMLP_ret::XML_ERROR in other case.
   */
   ...
   */
  RTPS_DllAPI static void
  library_settings(const LibrarySettingsAttributes &library_settings);
  ...
  fillParticipantAttributes(const std::string &profile_name,
                            ParticipantAttributes &atts, bool log_error = true);
   ...
   */
  RTPS_DllAPI static XMLP_ret
  fillPublisherAttributes(const std::string &profile_name,
                          PublisherAttributes &atts, bool log_error = true);

  //! Fills publisher_attributes with the default values.
  RTPS_DllAPI static void
  getDefaultPublisherAttributes(PublisherAttributes &publisher_attributes);
   ...
   */
  RTPS_DllAPI static XMLP_ret
  fillSubscriberAttributes(const std::string &profile_name,
                           SubscriberAttributes &atts, bool log_error = true);

  //! Fills subscriber_attributes with the default values.
  RTPS_DllAPI static void
  getDefaultSubscriberAttributes(SubscriberAttributes &subscriber_attributes);

  //! Add a new transport instance along with its id.
  RTPS_DllAPI static bool insertTransportById(const std::string &transport_id,
                                              sp_transport_t transport);

  //! Retrieves a transport instance by its id.
  RTPS_DllAPI static sp_transport_t
  getTransportById(const std::string &transport_id);
  ...
  RTPS_DllAPI static void
  getDefaultTopicAttributes(TopicAttributes &topic_attributes);

  //! Add a new dynamic type instance along with its name.
  RTPS_DllAPI static bool insertDynamicTypeByName(const std::string &type_name,
                                                  p_dynamictypebuilder_t type);

  //! Retrieves a transport instance by its name.
  RTPS_DllAPI static p_dynamictypebuilder_t
  getDynamicTypeByName(const std::string &type_name);
   ...
   */
  RTPS_DllAPI static XMLP_ret
  fillRequesterAttributes(const std::string &profile_name,
   ...
   */
  RTPS_DllAPI static XMLP_ret
  fillReplierAttributes(const std::string &profile_name,
   ...
   * if using XMLProfileManager outside of FastDDS, it should be called
   * manually.
   */
   ...
   */
  RTPS_DllAPI static types::DynamicPubSubType *
  CreateDynamicPubSubType(const std::string &type_name);
   ...
   */
  RTPS_DllAPI static void
  DeleteDynamicPubSubType(types::DynamicPubSubType *type);

private:
  RTPS_DllAPI static XMLP_ret extractProfiles(up_base_node_t properties,
                                              const std::string &filename);

  RTPS_DllAPI static XMLP_ret
  extractParticipantProfile(up_base_node_t &profile,
                            const std::string &filename);

  RTPS_DllAPI static XMLP_ret
  extractPublisherProfile(up_base_node_t &profile, const std::string &filename);

  RTPS_DllAPI static XMLP_ret
  extractSubscriberProfile(up_base_node_t &profile,
                           const std::string &filename);
  ...
  RTPS_DllAPI static XMLP_ret
  extractRequesterProfile(up_base_node_t &profile, const std::string &filename);

  RTPS_DllAPI static XMLP_ret
  extractReplierProfile(up_base_node_t &profile, const std::string &filename);
