...
protected:
  ...
  std::map<std::string, DynamicTypeMember *>
      member_by_name_; // Uses the pointers from "member_by_id_".
  std::string name_;
  TypeKind kind_;
...
public:
  ...
  RTPS_DllAPI ReturnCode_t add_member(MemberId id, const std::string &name,
  ...
  add_member(MemberId id, const std::string &name,
  ...
  RTPS_DllAPI ReturnCode_t
  apply_annotation_to_member(MemberId id, AnnotationDescriptor &descriptor);

  RTPS_DllAPI ReturnCode_t
  apply_annotation_to_member(MemberId id, const std::string &annotation_name,
  ...
  ReturnCode_t
  get_all_members(std::map<MemberId, DynamicTypeMember *> &members);

  RTPS_DllAPI inline TypeKind get_kind() const { return kind_; }

  RTPS_DllAPI std::string get_name() const;

  RTPS_DllAPI MemberId get_member_id_by_name(const std::string &name) const;

  const TypeDescriptor *get_type_descriptor() const { return descriptor_; }
