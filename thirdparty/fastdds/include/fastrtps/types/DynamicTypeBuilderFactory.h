...
protected:
  ...
  void build_alias_type_code(const TypeDescriptor *descriptor,
                             TypeObject &object, bool complete = true) const;
  ...
  void build_map_type_code(const TypeDescriptor *descriptor, TypeObject &object,
  ...
  void
  build_struct_type_code(const TypeDescriptor *descriptor, TypeObject &object,
                         const std::vector<const MemberDescriptor *> members,
                         bool complete = true) const;

  void
  build_union_type_code(const TypeDescriptor *descriptor, TypeObject &object,
                        const std::vector<const MemberDescriptor *> members,
                        bool complete = true) const;

  void
  build_bitset_type_code(const TypeDescriptor *descriptor, TypeObject &object,
                         const std::vector<const MemberDescriptor *> members,
                         bool complete = true) const;

  void
  build_bitmask_type_code(const TypeDescriptor *descriptor, TypeObject &object,
                          const std::vector<const MemberDescriptor *> members,
                          bool complete = true) const;
...
public:
  ...
  RTPS_DllAPI DynamicTypeBuilder *
  create_custom_builder(const TypeDescriptor *descriptor,
                        const std::string &name = "");

  RTPS_DllAPI DynamicTypeBuilder *
  create_builder_copy(const DynamicTypeBuilder *type);
  ...
  RTPS_DllAPI DynamicTypeBuilder *
  create_string_builder(uint32_t bound = MAX_STRING_LENGTH);

  RTPS_DllAPI DynamicTypeBuilder *
  create_wstring_builder(uint32_t bound = MAX_STRING_LENGTH);
  ...
  RTPS_DllAPI DynamicTypeBuilder *
  create_sequence_builder(const DynamicType_ptr type,
                          uint32_t bound = MAX_ELEMENTS_COUNT);
  ...
  RTPS_DllAPI DynamicTypeBuilder *
  create_array_builder(const DynamicType_ptr type,
                       const std::vector<uint32_t> &bounds);

  RTPS_DllAPI DynamicTypeBuilder *
  create_map_builder(DynamicTypeBuilder *key_element_type,
                     DynamicTypeBuilder *element_type,
                     uint32_t bound = MAX_ELEMENTS_COUNT);

  RTPS_DllAPI DynamicTypeBuilder *
  create_map_builder(DynamicType_ptr key_type, DynamicType_ptr value_type,
                     uint32_t bound = MAX_ELEMENTS_COUNT);
  ...
  RTPS_DllAPI DynamicTypeBuilder *
  create_alias_builder(DynamicTypeBuilder *base_type, const std::string &sName);

  RTPS_DllAPI DynamicTypeBuilder *
  create_alias_builder(DynamicType_ptr base_type, const std::string &sName);
  ...
  RTPS_DllAPI DynamicTypeBuilder *
  create_child_struct_builder(DynamicTypeBuilder *parent_type);

  RTPS_DllAPI DynamicTypeBuilder *
  create_union_builder(DynamicTypeBuilder *discriminator_type);

  RTPS_DllAPI DynamicTypeBuilder *
  create_union_builder(DynamicType_ptr discriminator_type);

  RTPS_DllAPI DynamicType_ptr
  create_annotation_primitive(const std::string &name);
  ...
  RTPS_DllAPI DynamicType_ptr
  create_string_type(uint32_t bound = MAX_STRING_LENGTH);

  RTPS_DllAPI DynamicType_ptr
  create_wstring_type(uint32_t bound = MAX_STRING_LENGTH);
