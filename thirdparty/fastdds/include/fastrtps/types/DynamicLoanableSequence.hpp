 ...
 * of DynamicData.
 *
 * @tparam _NonConstEnabler to enable data access through [] operator on
 * non-const sequences.
 */
template <typename _NonConstEnabler>
class LoanableSequence<fastrtps::types::DynamicData, _NonConstEnabler>
    : public LoanableTypedCollection<fastrtps::types::DynamicData,
                                     _NonConstEnabler> {
public:
  ...
  LoanableSequence(fastrtps::types::DynamicType_ptr dyn_type)
      : dynamic_type_support_(
            new fastrtps::types::DynamicPubSubType(dyn_type)) {}
...
private:
  ...
  void resize(size_type maximum) override {
    ...
    while (maximum_ < maximum) {
      data_[maximum_++] = static_cast<fastrtps::types::DynamicData *>(
          dynamic_type_support_->createData());
    }
  }
...
using DynamicLoanableSequence =
    LoanableSequence<fastrtps::types::DynamicData, std::true_type>;
