...
public:
   ...
   */
  PublisherHistory(const TopicAttributes &topic_att, uint32_t payloadMax,
                   rtps::MemoryManagementPolicy_t mempolicy);

  virtual ~PublisherHistory();

  /**
   * Rebuild instances loaded from DB. Does nothing if the topic doesn't have
   * key.
   */
   ...
   */
  bool
  register_instance(const rtps::InstanceHandle_t &instance_handle,
   ...
   * @param lock Lock which should be unlock in case the operation has to wait.
   * @param max_blocking_time Maximum time the operation should be waiting.
   * @return true when the last change of the instance history is acknowleged,
   * false when timeout is reached.
   */
...
private:
  ...
  //! Map where keys are instance handles and values are vectors of cache
  //! changes associated
   ...
   */
  bool find_or_add_key(const rtps::InstanceHandle_t &instance_handle,
   ...
   * @return True if added.
   */
  bool prepare_change(rtps::CacheChange_t *change,
                      std::unique_lock<RecursiveTimedMutex> &lock,
                      const std::chrono::time_point<std::chrono::steady_clock>
                          &max_blocking_time);
};
