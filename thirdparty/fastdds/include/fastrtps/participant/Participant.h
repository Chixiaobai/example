...
public:
  ...
  /**
   * Called when using a StaticEndpointDiscovery mechanism different that the
   * one included in FastRTPS, for example when communicating with other
   * implementations. It indicates to the Participant that an Endpoint from the
   * XML has been discovered and should be activated.
  ...
  bool newRemoteEndpointDiscovered(const rtps::GUID_t &partguid,
                                   uint16_t userId, rtps::EndpointKind_t kind);
