...
// Include library
    ...
    defined(EPROSIMA_LIB_DEBUG_TAG) && defined(FASTRTPS_VERSION_MAJOR) &&      \
    defined(FASTRTPS_VERSION_MINOR)
#pragma comment(                                                               \
    lib, EPROSIMA_LIB_PREFIX EPROSIMA_STRINGIZE(EPROSIMA_LIB_NAME)             \
             EPROSIMA_LIB_DEBUG_TAG                                            \
    "-" EPROSIMA_STRINGIZE(FASTRTPS_VERSION_MAJOR) "." EPROSIMA_STRINGIZE(     \
        FASTRTPS_VERSION_MINOR) ".lib")
#else
