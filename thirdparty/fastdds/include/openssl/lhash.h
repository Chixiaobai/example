...
#define DEFINE_LHASH_OF(type)                                                  \
  ...
  static ossl_inline type *lh_##type##_delete(LHASH_OF(type) * lh,             \
                                              const type *d) {                 \
    return (type *)OPENSSL_LH_delete((OPENSSL_LHASH *)lh, d);                  \
  }                                                                            \
  static ossl_inline type *lh_##type##_retrieve(LHASH_OF(type) * lh,           \
                                                const type *d) {               \
  ...
  static ossl_inline unsigned long lh_##type##_num_items(LHASH_OF(type) *      \
                                                         lh) {                 \
    return OPENSSL_LH_num_items((OPENSSL_LHASH *)lh);                          \
  }                                                                            \
  static ossl_inline void lh_##type##_node_stats_bio(                          \
      const LHASH_OF(type) * lh, BIO * out) {                                  \
    OPENSSL_LH_node_stats_bio((const OPENSSL_LHASH *)lh, out);                 \
  }                                                                            \
  static ossl_inline void lh_##type##_node_usage_stats_bio(                    \
      const LHASH_OF(type) * lh, BIO * out) {                                  \
...
#define int_implement_lhash_doall(type, argtype, cbargtype)                    \
  static ossl_inline void lh_##type##_doall_##argtype(                         \
