...
template <typename T>
using format_as_member_result = remove_cvref_t<decltype(
...
public:
  ...
  FMT_CONSTEXPR explicit format_string_checker(basic_string_view<Char> fmt,
      ...
      : types_{mapped_type_constant<T, Char>::value...}, named_args_{},
        context_(fmt, NUM_ARGS, types_), parse_funcs_{
                                             &invoke_parse<T, Char>...} {
...
public:
  ...
  iterator_buffer(iterator_buffer &&other) noexcept
      : fixed_buffer_traits(other), buffer<T>(
                                        static_cast<iterator_buffer &&>(other)),
        out_(other.out_) {
    if (this->data() != out_) {
      this->set(data_, buffer_size);
      this->clear();
...
public:
  ...
  constexpr FMT_ALWAYS_INLINE
  basic_format_args(const store<NUM_ARGS, NUM_NAMED_ARGS, DESC> &s)
