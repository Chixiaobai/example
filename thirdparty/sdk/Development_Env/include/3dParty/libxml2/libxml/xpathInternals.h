...
#define xmlXPathStackIsNodeSet(ctxt) \
    (((ctxt)->value != NULL) && (((ctxt)->value->type == XPATH_NODESET) || ((ctxt)->value->type == XPATH_XSLT_TREE)))
...
#define CHECK_TYPE(typeval) \
    if ((ctxt->value == NULL) || (ctxt->value->type != typeval)) XP_ERROR(XPATH_INVALID_TYPE)
...
#define CHECK_TYPE0(typeval) \
    if ((ctxt->value == NULL) || (ctxt->value->type != typeval)) XP_ERROR0(XPATH_INVALID_TYPE)
...
#define CHECK_ARITY(x)                               \
    if (ctxt == NULL) return;                        \
    if (nargs != (x)) XP_ERROR(XPATH_INVALID_ARITY); \
    if (ctxt->valueNr < ctxt->valueFrame + (x)) XP_ERROR(XPATH_STACK_ERROR);
...
#define CAST_TO_STRING \
    if ((ctxt->value != NULL) && (ctxt->value->type != XPATH_STRING)) xmlXPathStringFunction(ctxt, 1);
...
#define CAST_TO_NUMBER \
    if ((ctxt->value != NULL) && (ctxt->value->type != XPATH_NUMBER)) xmlXPathNumberFunction(ctxt, 1);
...
#define CAST_TO_BOOLEAN \
    if ((ctxt->value != NULL) && (ctxt->value->type != XPATH_BOOLEAN)) xmlXPathBooleanFunction(ctxt, 1);

/*
 * Variable Lookup forwarding.
 */

XMLPUBFUN void XMLCALL xmlXPathRegisterVariableLookup(xmlXPathContextPtr ctxt, xmlXPathVariableLookupFunc f,
                                                      void *data);
...
XMLPUBFUN int XMLCALL xmlXPathRegisterFuncNS(xmlXPathContextPtr ctxt, const xmlChar *name, const xmlChar *ns_uri,
                                             xmlXPathFunction f);
XMLPUBFUN int XMLCALL xmlXPathRegisterVariable(xmlXPathContextPtr ctxt, const xmlChar *name, xmlXPathObjectPtr value);
XMLPUBFUN int XMLCALL xmlXPathRegisterVariableNS(xmlXPathContextPtr ctxt, const xmlChar *name, const xmlChar *ns_uri,
                                                 xmlXPathObjectPtr value);
XMLPUBFUN xmlXPathFunction XMLCALL xmlXPathFunctionLookup(xmlXPathContextPtr ctxt, const xmlChar *name);
XMLPUBFUN xmlXPathFunction XMLCALL xmlXPathFunctionLookupNS(xmlXPathContextPtr ctxt, const xmlChar *name,
...
XMLPUBFUN xmlXPathObjectPtr XMLCALL xmlXPathVariableLookupNS(xmlXPathContextPtr ctxt, const xmlChar *name,
                                                             const xmlChar *ns_uri);
