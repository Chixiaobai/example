...
void emuc_unesc (EMUC_RAW_INFO *info, unsigned char s)
{
  ...
  /* check rbuff is CAN or CANFD format and parse rbuff */
  unsigned char FDF = (info->rbuff[2] & 0b00000100) >> 2;
  if((info->rbuff[0] == CMD_HEAD_RECV_CANFD) && (FDF == 1))
  {
    int dataLength = DecodeDLC_CANFD((info->rbuff[1] & 0b11110000) >> 4);
    if(dataLength < 8)
    {
      // printk("dataLength < 8, info->rcount: %d\n", info->rcount);
      if(info->rcount == (CANFD_RX_HEAD_SIZE + 1 + 8 + 3))
      {
        emuc_fd_bump(info);
        info->rcount = 0;
      }
    ...
    {
      if(info->rcount == (CANFD_RX_HEAD_SIZE + 1 + dataLength + 3))
      {
        emuc_fd_bump(info);
        info->rcount = 0;
      }
  ...
  else if((info->rbuff[0] == CMD_HEAD_RECV_TS) && (info->rcount == EMUC_MTU_TS))
  {
    // todo emuc_bump with timestamp format
    emuc_bump(info);
    info->rcount = 0;
  }
  else if((info->rbuff[0] == CMD_HEAD_RECV) && (info->rcount == EMUC_MTU))
  {
    // int i = 0;
    // printk("emuc_unesc(Rx): \n");
    // for(i = 0; i < EMUC_FD_MTU; i++)
    // {
    //   printk("%02X ", info->rbuff[i]);
    // }
...
void emuc_fd_bump(EMUC_RAW_INFO *info)
{
  EMUC_CAN_FRAME       frame;
  int                  i, ret;
  struct sk_buff      *skb;
  struct canfd_frame   cf_fd = {0};
  ...
  ret = EMUC_CANFD_RevHex(&frame);
  if(ret < 0)
  {
    printk("emuc : fd bump : parse fail %d.\n", ret);
    return;
  }
...
/*--------------------------------------*/
  ...
  {
    int dataLength = DecodeDLC_CANFD(frame.dlc);
    printk("%d. Data(Length: %d): ", cnt++, dataLength);
...
/*--------------------------------------*/
  ...
  *(u64 *)(&cf_fd.data) = 0;
  ...
  cf_fd.len = DecodeDLC_CANFD(frame.dlc);

  for(i = 0; i < cf_fd.len; i++)
  ...
  info->devs[frame.CAN_port - 1]->stats.rx_bytes += cf_fd.len;
...
void emuc_bump (EMUC_RAW_INFO *info)
{
  ...
  if(info->rcount == COM_RX_BUF_LEN_TS)
    ret = EMUCRevHex_TS(&frame);
  else
...
void emuc_fd_encaps(EMUC_RAW_INFO *info, int channel, struct canfd_frame *cf_fd)
{
  int               i;
  int               len = COM_CANFD_TX_BUF_LEN;
  int               actual;
  canid_t           id = cf_fd->can_id;
  EMUC_CAN_FRAME    emuc_can_frame;
  ...
  memset(&emuc_can_frame, 0, sizeof(EMUC_CAN_FRAME));
  emuc_can_frame.CAN_port = channel;
  emuc_can_frame.rtr = (cf_fd->can_id & CAN_RTR_FLAG) ? 1: 0;

  if (cf_fd->can_id & CAN_EFF_FLAG)
  {
    emuc_can_frame.id_type = EMUC_EID;
    id &= cf_fd->can_id & CAN_EFF_MASK;
  }
  else
  {
    emuc_can_frame.id_type = EMUC_SID;
    id &= cf_fd->can_id & CAN_SFF_MASK;
  }
  ...
  emuc_can_frame.dlc = EncodeDLC_CANFD(cf_fd->len);

  if(cf_fd->flags & CANFD_BRS)
    emuc_can_frame.brs = 1;
  else 
    emuc_can_frame.brs = 0;

  if(cf_fd->flags & CANFD_ESI)
    emuc_can_frame.esi = 1;
  else
    emuc_can_frame.esi = 0;

  emuc_can_frame.fdf = 1; // fdf always set 1 in emuc_fd_encaps() function

  for(i = 0; i < cf_fd->len; i++)
    emuc_can_frame.data[i] = cf_fd->data[i];

  EMUC_CANFD_SendHex(&emuc_can_frame);
  len = CANFD_TX_HEAD_SIZE + 1 + cf_fd->len + 3;
  memcpy(info->xbuff, emuc_can_frame.com_buf, len);
  ...
  info->xleft = len - actual;
  info->xhead = info->xbuff + actual;
  info->devs[channel]->stats.tx_bytes += cf_fd->len;
